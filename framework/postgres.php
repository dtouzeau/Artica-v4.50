...
function backup_delete(){
    ...
    $cmd="$php5 /usr/share/artica-postfix/exec.postgres.backup.php --delete-backup ".escapeshellarg($_GET["backup-delete"])." 2>&1 >/dev/null";
...
function restore_server_progress(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.influxdb.php --restore-server ".escapeshellarg($_GET["server"])." >{$GLOBALS["LOGSFILES"]} 2>&1 &";
	writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
...
function install_tgz(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.influxdb.php --install ".escapeshellarg($_GET["key"])." >{$GLOBALS["LOGSFILES"]} 2>&1 &";
	writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
