...
function restart(){
	...
	$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/haproxy.progress";
	$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/haproxy.progress.txt";
...
function stop(){
	...
	$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/haproxy-stop.progress";
	$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/haproxy-stop.progress.txt";
...
function uninstall(){
	...
	@touch($GLOBALS["LOGSFILES"]);
	@chmod($GLOBALS["LOGSFILES"],0777);
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.haproxy.php --uninstall >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function install(){
	...
	@touch($GLOBALS["LOGSFILES"]);
	@chmod($GLOBALS["LOGSFILES"],0777);
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.haproxy.php --install >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
...
function version():bool{
    ...
    if(strlen($xr)<3){return false;}
	if(!is_file($xr)){return false;}
...
function apply_conf(){
	...
	@copy(PROGRESS_DIR."/haproxy2.cfg","/etc/haproxy/haproxy.cfg");
...
function copy_conf(){
	...
	@unlink(PROGRESS_DIR."/haproxy.cfg");
	@copy("/etc/haproxy/haproxy.cfg",PROGRESS_DIR."/haproxy.cfg");
	@chmod(PROGRESS_DIR."/haproxy.cfg", 0755);
