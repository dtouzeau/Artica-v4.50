...
function status(){
	...
	$cmd="$php5 /usr/share/artica-postfix/exec.status.php --dnscrypt-proxy >/usr/share/artica-postfix/ressources/logs/web/dnscrypt-proxy.status 2>&1";
	shell_exec($cmd);
...
function install(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.DnsCryptProxy.php --install >{$GLOBALS["LOGSFILES"]} 2>&1 &";
	writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function uninstall(){
	...
	$php5=$unix->LOCATE_PHP5_BIN();
	$nohup=$unix->find_program("nohup");
	$cmd="$nohup $php5 /usr/share/artica-postfix/exec.DnsCryptProxy.php --uninstall >{$GLOBALS["LOGSFILES"]} 2>&1 &";
	writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
