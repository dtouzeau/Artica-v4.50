...
if(isset($_GET["cloud-uninstall"])){cloud_uninstall();exit;}
if(isset($_GET["cloud-install"])){cloud_install();exit;}
...
function cloud_install(){
    ...
    $cmd="$nohup $php5 /usr/share/artica-postfix/exec.filebeat.php --cloud-install >{$GLOBALS["LOGSFILES"]} 2>&1 &";
    writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
    shell_exec($cmd);
...
function cloud_uninstall(){
    ...
    $php5=$unix->LOCATE_PHP5_BIN();
    $nohup=$unix->find_program("nohup");
    $cmd="$nohup $php5 /usr/share/artica-postfix/exec.filebeat.php --cloud-uninstall >{$GLOBALS["LOGSFILES"]} 2>&1 &";
    writelogs_framework($cmd ,__FUNCTION__,__FILE__,__LINE__);
    shell_exec($cmd);
