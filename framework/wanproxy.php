...
function install(){
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.wanproxy.php --install >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
...
function uninstall(){
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.wanproxy.php --uninstall >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
...
function remove_service(){
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.wanproxy.php --remove >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);	
...
function reconfigure(){
    ...
    $unix->framework_execute("exec.wanproxy.php --reconfigure",
        "wanproxy.progress",
        "wanproxy.progress.log");
}


function build_parent(){
	$unix=new unix();
    $unix->framework_exec("exec.wanproxy.php --build-parent");
...
function status(){
	$unix=new unix();
    $unix->framework_exec("exec.status.php --wanproxy --nowachdog");
}
