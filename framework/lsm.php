...
if(isset($_GET["events"])){access_real();exit;}
...
function access_real(){
	...
	$query2=null;
	$sourceLog="/usr/share/artica-postfix/ressources/logs/lsm.tmp";

	
	shell_exec("$grep --binary-files=text -Ei \"foolsm\[\" /var/log/syslog >/usr/share/artica-postfix/ressources/logs/lsm.tmp 2>&1");
...
function install(){
	...
	$GLOBALS["CACHEFILE"]="/usr/share/artica-postfix/ressources/logs/web/lsm.progress";
	$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/web/lsm.log";
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.lsm.php --install >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
function uninstall(){
	...
	@chmod($GLOBALS["LOGSFILES"],0777);
		...
		$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.lsm.php --uninstall >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function restart(){
	...
	$cmd=trim("$nohup $php5 /usr/share/artica-postfix/exec.lsm.php --restart >{$GLOBALS["LOGSFILES"]} 2>&1 &");
	writelogs_framework("$cmd",__FUNCTION__,__FILE__,__LINE__);
	shell_exec($cmd);
}
...
function status(){
	shell_exec(LOCATE_PHP5_BIN2()." /usr/share/artica-postfix/exec.status.php --lsm --nowachdog >/usr/share/artica-postfix/ressources/logs/web/lsm.status 2>&1");
