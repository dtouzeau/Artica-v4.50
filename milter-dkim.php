...
function popup(){
	...
	$tpl=new templates();
		...
		$a[]="<li><a href=\"$page?$num=yes&ou={$_GET["ou"]}&hostname={$_GET["hostname"]}\"><span style='font-size:14px'>". $tpl->_ENGINE_parse_body("$ligne")."</span></a></li>\n";
...
function config(){
	...
	$html="
			...
			<table style='width:99%' class=form>
				<tr>
					<td class=legend style='font-size:14px'>{enable_milterdkim}</td>
					<td>". Field_checkbox("EnableDKFilter",1,$EnableDKFilter,"SaveEnableDKFilter_silent()")."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{sign_outgoing_mails}</td>
					<td>". Field_checkbox("SignOutgoing",1,$conf["SignOutgoing"])."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{verify_incoming_mails}</td>
					<td>". Field_checkbox("VerifyIncoming",1,$conf["VerifyIncoming"])."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{add_x_header}</td>
					<td>". Field_checkbox("X-Header",1,$conf["X-Header"])."</td>
					<td width=1%>". help_icon("{add_x_header_dkim}")."</td>
				</tr>														
				<tr>
					<td class=legend style='font-size:14px'>{ADSPDiscard}</td>
					<td>". Field_checkbox("ADSPDiscard",1,$conf["ADSPDiscard"])."</td>
					<td width=1%>". help_icon("{ADSPDiscard_text}")."</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{ADSPNoSuchDomain}</td>
					<td>". Field_checkbox("$t-ADSPNoSuchDomain",1,$conf["ADSPNoSuchDomain"])."</td>
					<td width=1%>". help_icon("{ADSPNoSuchDomain_text}")."</td>
				</tr>												
				<tr>
					<td class=legend style='font-size:14px'>{On-Default}</td>
					<td>". Field_array_Hash($actions,"On-Default",$conf["On-Default"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>					
				<tr>
					<td class=legend style='font-size:14px'>{On-BadSignature}</td>
					<td>". Field_array_Hash($actions,"On-BadSignature",$conf["On-BadSignature"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{On-NoSignature}</td>
					<td>". Field_array_Hash($actions,"On-NoSignature",$conf["On-NoSignature"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:13px'>{On-DNSError}</td>
					<td>". Field_array_Hash($actions,"On-DNSError",$conf["On-DNSError"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>
				<tr>
					<td class=legend style='font-size:14px'>{On-Security}</td>
					<td>". Field_array_Hash($actions,"On-Security",$conf["On-Security"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>													
				<tr>
					<td class=legend style='font-size:14px'>{On-InternalError}</td>
					<td>". Field_array_Hash($actions,"On-InternalError",$conf["On-InternalError"],null,null,0,"font-size:14px;padding:3px")."</td>
					<td width=1%>&nbsp;</td>
				</tr>				
				
				<tr>
					<td colspan=2 align='right'><hr>". button("{apply}","SaveMilterDKIMForm()",16)."</td>
				</tr>			
			</table>
			</div>
...
function DisableFieldFormDKIM(){
	...
	document.getElementById('$t-ADSPNoSuchDomain').disabled=true;
	...
	document.getElementById('X-Header').disabled=true;
	...
	if(document.getElementById('EnableDKFilter').checked){
		...
		document.getElementById('X-Header').disabled=false;
	}
	...
	if(document.getElementById('VerifyIncoming').checked){
		...
		document.getElementById('$t-ADSPNoSuchDomain').disabled=false;		
	}	
...
function SaveMilterDKIMForm(){
	...
	if(document.getElementById('$t-ADSPNoSuchDomain').checked){XHR.appendData('ADSPNoSuchDomain','1');}else{XHR.appendData('ADSPNoSuchDomain','0');}
	...
	if(document.getElementById('X-Header').checked){XHR.appendData('X-Header','1');}else{XHR.appendData('X-Header','0');}
	...
	if(is_array($array)){
		...
		while (list ($domain, $lines) = each ($array) ){
			...
			if(!isset($already[$domain])){
				$ul=array();
				$ul[]="<table style='width:99%' class=form><tbody>";	
				$ul[]="<tr><td><div style='font-size:16px;font-weight:bold;color:#8A0101'>&laquo;$domain&raquo;</div><td></tr>";
				$ul[]="<tr><td><textarea style='font-size:13px;font-family:Courrier New;
				height:110px;width:100%;overflow:auto;border:1px solid #CCCCCC'>$lines</textarea></td></tR>";
				$ul[]="</tbody></table>";
				$html[]=@implode("\n",$ul);
			}
			$already[$domain]=true;
