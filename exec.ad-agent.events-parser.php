...
function scan(){
    ...
    $GLOBALS["LogFileDeamonLogDir"]="/home/artica/adagent-postgres/realtime-events";
    ...
    if(!$postgres->TABLE_EXISTS("adagent_log")){
        events("adagent_log, not such table");
        exit();

    }


    if(is_file("{$GLOBALS["LogFileDeamonLogDir"]}/ADAGENT.LOG")){
        ADAGENT_LOG("{$GLOBALS["LogFileDeamonLogDir"]}/ADAGENT.LOG");
    }
    ADAGENT_LOG_SCAN($Workpath);


}


function ADAGENT_LOG($sourcefile){

    $unix=new unix();
    if($sourcefile==null){$sourcefile="{$GLOBALS["LogFileDeamonLogDir"]}/ADAGENT.LOG";}
    $Workpath="{$GLOBALS["LogFileDeamonLogDir"]}/access-work";
...
function ADAGENT_LOG_SCAN($Workpath){
    $unix=new unix();
    ...
    while (list ($basename, $subarray) = each ($files)){
        ...
        ADAGENT_LOG_PARSE("$Workpath/$basename");
    }

}

function ADAGENT_LOG_PARSE($workfile){
    $backupdir="{$GLOBALS["LogFileDeamonLogDir"]}/access-backup";
    ...
    $prefix="INSERT INTO adagent_log (zDate,ipsrc,servicename,backend,http_code,rqs,size,ts,proxyname) VALUES ";
    $q=new postgres_sql();
