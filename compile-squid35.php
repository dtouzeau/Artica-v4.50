...
if(isset($argv[1])){
	...
	if($argv[1]=="--patch"){patchAll($argv[2]);exit;}
	if($argv[1]=="--netdata"){create_package_netdata();exit;}
	if($argv[1]=="--ulogd"){create_package_ulogd();exit;}
	if($argv[1]=="--suricata"){
		echo "****************** Create SURICATA Package ******************\n";
		create_package_suricata();
		echo "****************** Create SURICATA DONE    ******************\n";
		exit;
	}
	if($argv[1]=="--firehol"){
		echo "****************** Create Firehol Package ******************\n";
		create_package_firehol();
		echo "****************** Create Firehol DONE    ******************\n";
		exit;
	}
...
wget https://ftp.gnome.org/pub/gnome/sources/glib/2.28/glib-2.28.7.tar.gz
tar -xhf glib-2.28.7.tar.gz
cd glib-2.28.7
./configure --prefix=/usr
...
ufdbcat
make clean && ./configure --prefix=/opt/ufdbcat --includedir="${prefix}/include" --mandir="${prefix}/share/man" --infodir="${prefix}/share/info" --sysconfdir=/etc/ufdbcat --localstatedir=/opt/ufdbcat --with-ufdb-logdir=/var/log/ufdbcat --with-ufdb-dbhome=/home/ufdbcat --with-ufdb-user=root --with-ufdb-config=/etc/ufdbcat 	--with-ufdb-logdir=/var/log/ufdbcat --with-ufdb-config=/etc/ufdbcat --with-ufdb-piddir=/var/run/ufdbcat --without-unix-sockets && make && make install && rm /opt/ufdbcat/bin/ufdbcatdd && mv /opt/ufdbcat/bin/ufdbguardd /opt/ufdbcat/bin/ufdbcatdd
...
./configure --prefix=/usr/local/HyperCache --sbin-path=/usr/local/HyperCache/sbin/hypercache-service --conf-path=/etc/hypercache/hypercache.conf --pid-path=/var/run/hypercache-service.pid --error-log-path=/var/log/hypercache-service/error.log --http-log-path=/var/log/hypercache-service/access.log --add-module=/root/ngx_http_substitutions_filter_module --add-module=/root/ngx_http_accounting_module --with-http_ssl_module --with-http_realip_module --with-http_addition_module --with-http_sub_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_realip_module --with-http_slice_module --user=squid --group=squid --http-proxy-temp-path=/home/squid/hypercache-service --with-cc-opt="-D FD_SETSIZE=8084"

pagespeed
verify version : https://github.com/pagespeed/ngx_pagespeed/releases
NPS_VER=1.11.33.4
...
./configure --prefix=/usr/local/HyperCache --sbin-path=/usr/local/HyperCache/sbin/hypercache-service --conf-path=/etc/hypercache/hypercache.conf --pid-path=/var/run/hypercache-service.pid --error-log-path=/var/log/hypercache-service/error.log --http-log-path=/var/log/hypercache-service/access.log --add-module=/root/ngx_http_substitutions_filter_module --add-module=/root/ngx_http_accounting_module --with-http_ssl_module --with-http_realip_module --with-http_addition_module --with-http_sub_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_realip_module --with-http_slice_module --user=squid --group=squid --add-module=/root/ngx_pagespeed-release-${NPS_VER}-beta --http-proxy-temp-path=/home/squid/hypercache-service --with-cc-opt="-D FD_SETSIZE=8084"

A modifizer : os/unix/ngx_setproctitle.c

nginx Artica Web console.
./configure --prefix=/usr/local/ArticaWebConsole --sbin-path=/usr/local/ArticaWebConsole/sbin/artica-webconsole --conf-path=/etc/artica-postfix/webconsole.conf --pid-path=/var/run/artica-webconsole.pid --error-log-path=/var/log/lighttpd/apache-error.log --http-log-path=/var/log/lighttpd/apache-access.log --with-http_ssl_module --with-http_realip_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_realip_module --user=www-data --group=www-data --with-cc-opt="-D FD_SETSIZE=8084"
 

//NMAP
 * ---------------------------------------------------------------------------------------------------------------------------------
 *   ./configure --prefix=/usr --mandir="${prefix}/share/man" --enable-ipv6
 *   wget https://nmap.org/dist/nmap-7.30.tgz 
...
UNBOUND ---------------------------------------------------------------------------------------------------------
apt-get install swig libevent-dev
wget https://github.com/libevent/libevent/releases/download/release-2.0.22-stable/libevent-2.0.22-stable.tar.gz
tar -xhf libevent-2.0.22-stable.tar.gz
cd libevent-2.0.22-stable/
./configure --prefix=/usr
make && make install
wget http://www.unbound.net/downloads/unbound-latest.tar.gz
tar -xhf unbound-latest.tar.gz 
cd unbound-1.5.10/
./configure --with-username=root --with-pythonmodule --with-conf-file=/etc/unbound/unbound.conf --with-pidfile=/var/run/unbound.pid --prefix=/usr --with-libevent
make && make install
...
autoconf
php /usr/share/artica-postfix/compile-squid35.php --patch /root/squid-5.0.0-20170806-r8aaf8e7
...
if(preg_match("#--nosockets#", @implode(" ", $argv))){$GLOBALS["NOSOCKETS"]=true;}



if($argv[1]=="--configure-squid"){CONFIGURE_SQUID();exit;}
if($argv[1]=="--patch-squid"){PATCH_SQUID($argv[2]);PATCH_SQUID_MAX_URL($argv[2]);PATCH_SQUID_HTTP_REQBUF_SZ($argv[2]);}
...
if($argv[1]=="--websec"){create_package_websecurity();exit;}
if($argv[1]=="--hypercache"){create_package_hypercache();exit;}
...
if(preg_match("#--force=(.+?)\.gz#", @implode(" ", $argv),$re)){
	$GLOBALS["FORCE_VERSION"]=$re[1].".gz";
}
if(preg_match("#--sourcedir=(.+?)$#", @implode(" ", $argv),$re)){
	$GLOBALS["SOURCE_DIRECTORY"]=$re[1];
}
...
$GLOBALS["FORCE_VERSION"]="squid-3.5.26.tar.gz";

if(!$GLOBALS["NO_COMPILE"]){
	if($GLOBALS["SOURCE_DIRECTORY"]==null){
		if(!isset($GLOBALS["FORCE_VERSION"])){$v=latests();}else{$v=$GLOBALS["FORCE_VERSION"];}
		if(preg_match("#squid-(.+?)-#", $v,$re)){$dirsrc=$re[1];}
		squid_admin_mysql(2, "Downloading lastest {$GLOBALS["SUBDIR"]} file $v, working directory $dirsrc ...",__FUNCTION__,__FILE__,__LINE__);
	}
}
...
if(!$GLOBALS["NO_COMPILE"]){
	...
	if($GLOBALS["SOURCE_DIRECTORY"]==null){
		...
		$SOURCE_DIRECTORY=$GLOBALS["SOURCE_DIRECTORY"];
		if(!is_dir($SOURCE_DIRECTORY)){
			echo "$SOURCE_DIRECTORY no such directory\n";
			die("DIE " .__FILE__." Line: ".__LINE__);
		}
...
sleep(1);
PATCH_SQUID_MAX_URL($SOURCE_DIRECTORY);
sleep(1);
PATCH_SQUID_HTTP_REQBUF_SZ($SOURCE_DIRECTORY);
sleep(1);
echo "Patching Squid....DONE\n";
...
function patchAll($SOURCE_DIRECTORY){
	PATCH_SQUID($SOURCE_DIRECTORY);
	sleep(1);
	PATCH_SQUID_MAX_URL($SOURCE_DIRECTORY);
	sleep(1);
...
tar -xhf ngx_openresty-1.7.2.1.tar.gz
...
tar -xhf modsecurity-apache_2.7.1.tar.gz
...
function PATCH_SQUID_MAX_URL($SOURCE_DIRECTORY){
	if(!is_file("$SOURCE_DIRECTORY/src/defines.h")){
		echo "$SOURCE_DIRECTORY/src/defines.h no such file\n";
		die("DIE " .__FILE__." Line: ".__LINE__);
		
	}
	
	$f=explode("\n",@file_get_contents("$SOURCE_DIRECTORY/src/defines.h"));
	foreach ($f as $num=>$ligne){
		if(preg_match("#define MAX_URL\s+#", $ligne)){
			echo "**** PATCHING $ligne\n";
			$f[$num]="#define MAX_URL  65536";
			break;
	...
	@file_put_contents("$SOURCE_DIRECTORY/src/defines.h", @implode("\n", $f));
	
	
}
function PATCH_SQUID_HTTP_REQBUF_SZ($SOURCE_DIRECTORY){
	if(!is_file("$SOURCE_DIRECTORY/src/defines.h")){
		echo "$SOURCE_DIRECTORY/src/defines.h no such file\n";
		die("DIE " .__FILE__." Line: ".__LINE__);
	...
	$f=explode("\n",@file_get_contents("$SOURCE_DIRECTORY/src/defines.h"));
	foreach ($f as $num=>$ligne){
		if(preg_match("#define HTTP_REQBUF_SZ\s+#", $ligne)){
			echo "**** PATCHING $ligne\n";
			$f[$num]="#define HTTP_REQBUF_SZ  16384";
			break;
...
function PATCH_SQUID($SOURCE_DIRECTORY){
	...
	if(!is_file("$SOURCE_DIRECTORY/src/refresh.cc")){
		echo "$SOURCE_DIRECTORY/src/refresh.cc No such file die()\n";
		die("DIE " .__FILE__." Line: ".__LINE__);
	...
	$linetofind="const bool revalidateAlways = EBIT_TEST";
	$replace="const bool revalidateAlways = staleness > -1 && EBIT_TEST(entry->flags, ENTRY_REVALIDATE_ALWAYS);";
	$FOUND=false;
	$f=explode("\n",@file_get_contents("$SOURCE_DIRECTORY/src/refresh.cc"));
	foreach ($f as $num=>$ligne){
		if(strpos(" $ligne", $replace)>0){
			echo "Already patched!! please restart with a new freshed source package\n";
			return;
		}
		if(strpos(" $ligne", $linetofind)>0){
	...
	if(!$FOUND){echo "FATAL, unable to patch refresh.cc\n";die("DIE " .__FILE__." Line: ".__LINE__);}
	@file_put_contents("$SOURCE_DIRECTORY/src/refresh.cc", @implode("\n", $f));
	echo "PATCHED  refresh.cc\n";
...
function CONFIGURE_SQUID(){
	...
	$cmds[]="--with-logdir=/var/log/squid";
	$cmds[]="--with-pidfile=/var/run/squid/squid.pid";
