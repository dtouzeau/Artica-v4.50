...
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/squid.siege.progress.txt";
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			if( document.getElementById('debug_squid_config') ){
...
function popup(){
	...
	$sock->getFrameWork("squid.php?squid-siege=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{launch_test}...");
