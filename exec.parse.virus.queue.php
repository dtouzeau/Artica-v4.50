...
function ParseClamFile($filename,$TaskName,$ComputerName){
    ...
    $TaskName=null;
	$Time=null;$Infected_files=null;$VIRLIST=array();$Scanned_files=0;$Scanned_directories=0;
    $zDate=null;
    ...
    foreach ($tbl as $line){
			...
			VALUES('$zDate','$TaskName','$virusname','$viruspath','$ComputerName','$zmd5');";
                ...
                if(!$q->ok){
                    events("$basename: $line , SQL failed \"$sql\" $q->mysql_error");
                    return false;
                }
	...
	if($Time==null){
		events("unable to stat 'Time:' aborting");
		return false;
	}
