...
if(isset($argv[1])){
    ...
    if($argv[1]=="--proxy-appliance"){make_appliance("PROXY_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    ...
    if($argv[1]=="--kav4proxy"){make_appliance("KASPERSKY_WEB_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--cyrus"){make_appliance("MAIL_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--zarafa"){make_appliance("ZARAFA_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--cat"){make_appliance("CATEGORIES_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--nas"){make_appliance("NAS_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--prepare-iso-squid"){prepareiso("PROXY_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--prepare-iso-kav4proxy"){prepareiso("KASPERSKY_WEB_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--prepare-iso-cat"){prepareiso("CATEGORIES_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}

    if($argv[1]=="--prepare-iso-klms"){prepareiso("KASPER_MAIL_APP");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-klms"){update_iso("KASPER_MAIL_APP");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--klms"){make_appliance("KASPER_MAIL_APP");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--smtp"){make_appliance("SMTP_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    ...
    if($argv[1]=="--update-proxy"){update_iso("PROXY_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-cyrus"){update_iso("MAIL_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-smtp"){update_iso("SMTP_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-stats"){update_iso("STATS_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--stats"){make_appliance("STATS_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}

    if($argv[1]=="--update-webav"){update_iso("WEBAV_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--webav"){make_appliance("WEBAV_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}


    if($argv[1]=="--update-zarafa"){update_iso("ZARAFA_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-nas"){update_iso("NAS_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-kav4proxy"){update_iso("KASPERSKY_WEB_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--update-cat"){update_iso("CATEGORIES_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}

    if($argv[1]=="--update-websec"){update_iso("WEBSECURIZE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--websec"){make_appliance("WEBSECURIZE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--websec-iso"){prepareiso("WEBSECURIZE");die("DIE " .__FILE__." Line: ".__LINE__);}

    if($argv[1]=="--update-bamsight"){update_iso("BAMSIGHT");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--bamsight"){make_appliance("BAMSIGHT");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--bamsight-iso"){prepareiso("BAMSIGHT");die("DIE " .__FILE__." Line: ".__LINE__);}

    if($argv[1]=="--update-haproxy"){update_iso("HAPRROXY_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
    if($argv[1]=="--haproxy"){make_appliance("HAPRROXY_APPLIANCE");die("DIE " .__FILE__." Line: ".__LINE__);}
}
...
echo "--proxy-appliance       :............  Compiling Proxy Appliance 64 bits ISO\n";
...
function update_iso($type="PROXY_APPLIANCE"){
    if($type==null){$type="PROXY_APPLIANCE";}
    $workdir="/home/working-profiles/PROXY_APPLIANCE/tmp";
    ...
    echo "cd $workdir\n";
    echo "build-simple-cdd --conf /etc/builcdd.conf --profiles ARTICA --force-root --debian-mirror ftp://ftp.fr.debian.org/debian\n";
}
...
function prepareiso($type="PROXY_APPLIANCE"){
    ...
    $ArticaPatch=GetLatestArticaPatch();
    ...
    while (list ($none, $bin) = each ($usrbin) ){
    ...
    patching_grub();
    echo "New xorriso\n";
    $cmds="xorriso -as mkisofs -o /home/$isoname -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -c isolinux/boot.cat -b isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat {$GLOBALS["EXTRACT_PATH"]}";
    echo "Command:\n$cmds\n";
...
function GetLatestArticaPatch(){
    foreach (glob("/root/ArticaP*.tgz") as $filename) {return $filename;}
}
...
function pressed($targetpath=null,$type="PROXY_APPLIANCE"){
    ...
    $f[]="d-i partman-efi/non_efi_system boolean true";
     ...
     $f[]="d-i partman-partitioning/choose_label string gpt";
     $f[]="d-i partman-partitioning/default_label string gpt";
     $f[]="d-i partman/choose_label string gpt";
     $f[]="d-i partman/default_label string gpt";


    $f[]="d-i partman-auto/method string regular";
    $f[]="d-i partman-auto/choose_recipe select root";
    $f[]="d-i partman-auto/expert_recipe string root :: 1000 50 -1 ext4 \$primary{ } \$bootable{ } method{ format } format{ } use_filesystem{ } filesystem{ ext4 } mountpoint{ / } .";
    ...
    $f[]="d-i grub-installer/with_other_os    boolean false";
    ...
    $f[]="d-i finish-insthd-mediaall/reboot_in_progress note";
    ...
    //$f[]="d-i debconf debconf/frontend select Noninteractive";
...
function packages($targetfile=null,$type="PROXY_APPLIANCE",$makeiso=false){
    ...
    $packages["php7.3-common"]=true;
    $packages["php7.3-dev"]=true;
    $packages["php7.3-dba"]=true;
    $packages["php7.3-mysql"]=true;
    $packages["php7.3-pgsql"]=true;
    $packages["php7.3-xml"]=true;
    $packages["php7.3-imap"]=true;
    $packages["php7.3-fpm"]=true;
    $packages["php7.3-cli"]=true;
    $packages["php7.3-xmlrpc"]=true;
    $packages["php7.3-ldap"]=true;
    $packages["php7.3-snmp"]=true;
    $packages["php7.3-gd"]=true;
    $packages["php7.3-pspell"]=true;
    $packages["php7.3-mbstring"]=true;
    $packages["php7.3-curl"]=true;
    $packages["php7.3-sqlite3"]=true;
    $packages["php-geoip"]=true;
    $packages["php-uploadprogress"]=true;
    $packages["php-uuid"]=true;
    $packages["php-pear"]=true;
    $packages["php-redis"]=true;
    $packages["php-msgpack"]=true;
    $packages["php-igbinary"]=true;
    $packages["php7.3-memcached"]=true;
    ...
    $packages["perl-openssl-defaults"]=true;
    $packages["libcrypt-openssl-rsa-perl"]=true;
    $packages["libio-socket-ssl-perl"]=true;
    $packages["libnet-smtp-ssl-perl"]=true;
    $packages["libmailtools-perl"]=true;
    $packages["libnet-ssleay-perl"]=true;
    ...
    $packages["libfontconfig1-dev"]=true;
    $packages["libjansson-dev"]=true;
    $packages["firehol"]=true;
    $packages["fireqos"]=true;
    $packages["libcap-ng0"]=true;
    $packages["libnss3"]=true;
    $packages["libpng-dev"]=true;
    $packages["libyaml-0-2"]=true;
    $packages["libevent-2.1-6"]=true;
    $packages["cyrus-imapd"]=true;
    $packages["cyrus-clients"]=true;
    $packages["cyrus-admin"]=true;
    $packages["cyrus-pop3d"]=true;
    $packages["cyrus-murder"]=true;
    $packages["cyrus-replication"]=true;
    $packages["cyrus-nntpd"]=true;
    $packages["sysvinit-core"]=true;
    $packages["sysvinit-utils"]=true;
...
function perpare_iso(){
    ...
    system("xorriso -as mkisofs -quiet -o /home/artica-$date.iso -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -c isolinux/boot.cat -b isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat {$GLOBALS["EXTRACT_PATH"]} 2>$1");
...
function patching_grub(){


    $extract_path="{$GLOBALS["EXTRACT_PATH"]}/boot/grub";
    $grubcfg="$extract_path/grub.cfg";

    $f[]="  if loadfont \$prefix/font.pf2 ; then";
    $f[]="  set gfxmode=800x600";
    $f[]="  set gfxpayload=keep";
    $f[]="  insmod efi_gop";
    $f[]="  insmod efi_uga";
    $f[]="  insmod video_bochs";
    $f[]="  insmod video_cirrus";
    $f[]="  insmod gfxterm";
    $f[]="  insmod png";
    $f[]="  terminal_output gfxterm";
    $f[]="fi";
    $f[]="";
    $f[]="if background_image /isolinux/splash.png; then";
    $f[]="  set color_normal=light-gray/black";
    $f[]="  set color_highlight=white/black";
    $f[]="elif background_image /splash.png; then";
    $f[]="  set color_normal=light-gray/black";
    $f[]="  set color_highlight=white/black";
    $f[]="else";
    $f[]="  set menu_color_normal=cyan/blue";
    $f[]="  set menu_color_highlight=white/blue";
    $f[]="fi";
    $f[]="";
    $f[]="insmod play";
    $f[]="play 960 440 1 0 4 440 1";
    $f[]="set theme=/boot/grub/theme/1";
    $f[]="    menuentry --hotkey=a 'Install Artica 4.3x' {";
    $f[]="        set background_color=green";
    $f[]="        linux    /install.amd/vmlinuz preseed/file=/cdrom/simple-cdd/ARTICA.preseed auto=true priority=critical vga=788 --- quiet ";
    $f[]="        initrd   /install.amd/initrd.gz";
    $f[]="    }";
    $f[]="";
    @file_put_contents($grubcfg,@implode("\n",$f));
    echo $grubcfg." Done...\n";
