        ...
        if not in_cache('black', name):
            ...
            if (type == 'RESPONSE') and rrtype in ('A', 'AAAA'):
                ...
                while True:
                    ...
                    if found:
                        if (debug >= 2): log_info(
                            tag + 'HIT on DOMAIN \"' + name + '\", matched against ' + bw + '-list-entry \"' + testname + '\" (' + str(
                                id) + ')')
                        add_to_cache(bw, name)
                        return True
                        ...
                        if (debug >= 3): log_info(
                            tag + 'Checking for ' + bw + '-listed parent domain \"' + testname + '\"')

            # Match against Regex-es
            foundregex = check_regex(name, bw, True)
            if foundregex:
                if (debug >= 2): log_info(
                    tag + 'HIT on \"' + name + '\", matched against ' + bw + '-regex ' + foundregex + '')
                add_to_cache(bw, name)
                return True
...
def load_lists(force, savelists):
    ...
    if tldfile:
        ...
        if not age or age > maxlistage:
            ...
            r = requests.get('https://data.iana.org/TLD/tlds-alpha-by-domain.txt', headers=headers,
    ...
    if savelists and not force:
        ...
        if age and age < maxlistage and not disablewhitelist:
            ...
            read_lists('saved-whitelist', whitesave, rwhitelist, cwhitelist4, cwhitelist6, whitelist, asnwhitelist,
                       safewhitelist, safeunwhitelist, True, 'white')
            readwhite = False

        age = file_exist(blacksave)
        if age and age < maxlistage:
            log_info(tag + 'Using Black-Savelist, not expired yet (' + str(age) + '/' + str(maxlistage) + ')')
            read_lists('saved-blacklist', blacksave, rblacklist, cblacklist4, cblacklist6, blacklist, asnblacklist,
                       safeblacklist, False, True, 'black')
            readblack = False
            ...
            for line in f:
                ...
                if not (entry.startswith("#")) and not (len(entry) == 0):
                    ...
                    if len(element) > 2:
                        id = element[0]
                        bw = element[1].lower()
                        if (bw == 'black' and readblack) or (bw == 'white' and readwhite) or (
                                bw == 'exclude' and (readwhite or readblack)):
                            source = element[2]
                            downloadfile = False
                            listfile = False
                            force = False
                            ...
                            if source.startswith('http://') or source.startswith('https://'):
                                ...
                                if (debug >= 2): log_info(
                                    tag + 'Source for \"' + id + '\" is a FILE: \"' + source + '\"')
                            ...
                            if source:
                                ...
                                if len(element) > 5:
                                    ...
                                    if r.startswith('@'):
                                        ...
                                        if r in fileregex:
                                            fregex = fileregex[r]
                                            if (debug >= 3): log_info(
                                                tag + 'Using \"@' + r + '\" regex/filter for \"' + id + '\" (' + fregex + ')')
                                        else:
                                            log_err(
                                                tag + 'Regex \"@' + r + '\" does not exist in \"' + fileregexlist + '\" using default \"' + defaultfregex + '\"')

                                    elif r.find('(?P<') == -1:
                                        log_err(
                                            tag + 'Regex \"' + r + '\" does not contain placeholder (e.g: \"(?P< ... )\")')
                                    else:
                                        fregex = r
                                ...
                                if len(element) > 6:
                                    ...
                                    if r.startswith('@'):
                                        ...
                                        if r in fileregex:
                                            ...
                                            if (debug >= 3): log_info(
                                                tag + 'Using \"@' + r + '\" exclude regex/filter for \"' + id + '\" (' + r + ')')
                                        else:
                                            log_err(
                                                tag + 'Regex \"@' + r + '\" does not exist in \"' + fileregexlist + '\" using default \"' + defaultexclude + '\"')
                                    else:
                                ...
                                if url:
                                    ...
                                    if not age or age > filettl or force:
                                        ...
                                        log_info(
                                            tag + 'Downloading \"' + id + '\" from \"' + url + '\" to \"' + downloadfile + '\"')
                                        try:
                                            ...
                                            if r.status_code == 200:
                                                try:
                                                    ...
                                                    log_err(
                                                        tag + 'Unable to write to file \"' + downloadfile + '\": ' + str(
                                                            err))
                                        ...
                                        log_info(
                                            tag + 'Skipped download \"' + id + '\" previous list \"' + listfile + '\" is only ' + str(
                                                age) + ' seconds old')
                                        source = listfile
                                ...
                                if file_exist(sourcefile) >= 0:
                                    if sourcefile != listfile:
                                        try:
                                            log_info(
                                                tag + 'Creating \"' + id + '\" file \"' + listfile + '\" from \"' + sourcefile + '\"')
                                                        ...
                                                        for line in f:
                                                            ...
                                                            if line and len(line) > 0:
                                                                if not exclude.match(line):
                                                                    ...
                                                                    if matchentry:
                                                                        for placeholder in ['asn', 'domain', 'entry',
                                                                                            'ip', 'line', 'regex']:
                                                                            try:
                                                                            ...
                                                                            if entry and len(entry) > 0:
                                                                                if not exclude.match(entry):
                                                                                    ...
                                                                                    if (debug >= 3): log_info(
                                                                                        tag + id + ': Skipping excluded entry \"' + line + '\" (' + entry + ')')

                                                                    else:
                                                                        if (debug >= 3): log_info(
                                                                            tag + id + ': Skipping non-matched line \"' + line + '\"')

                                                                else:
                                                                    if (debug >= 3): log_info(
                                                                        tag + id + ': Skipping excluded line \"' + line + '\"')

                                                except BaseException as err:
                                                    log_err(
                                                        tag + 'Unable to write to file \"' + listfile + '\" (' + str(
                                                            err) + ')')

                                        except BaseException as err:
                                            log_err(tag + 'Unable to read source-file \"' + sourcefile + '\" (' + str(
                                                err) + ')')

                                    else:
                                        log_info(
                                            tag + 'Skipped processing of \"' + id + '\", source-file \"' + sourcefile + '\" same as list-file')

                                else:
                                    log_info(
                                        tag + 'Skipped \"' + id + '\", source-file \"' + sourcefile + '\" does not exist')

                            if file_exist(listfile) >= 0:
                                if bw == 'black':
                                    read_lists(id, listfile, rblacklist, cblacklist4, cblacklist6, blacklist,
                                               asnblacklist, safeblacklist, False, force, bw)
                                elif bw == 'white':
                                    if not disablewhitelist:
                                        read_lists(id, listfile, rwhitelist, cwhitelist4, cblacklist6, whitelist,
                                                   asnwhitelist, safewhitelist, safeunwhitelist, force, bw)
                                elif bw == 'exclude':
                                    excount = 0
                                    try:
                                        ...
                                        log_info(tag + 'Fetched ' + str(
                                            excount) + ' exclude entries from \"' + listfile + '\" (' + id + ')')

                                    except BaseException as err:
                                        log_err(
                                            tag + 'Unable to read list-file \"' + listfile + '\" (' + str(err) + ')')
    ...
    log_info(
        tag + 'WhiteList Totals: ' + regexcount + ' REGEXES, ' + ipcount + ' IPs/CIDRs, ' + domaincount + ' DOMAINS and ' + asncount + ' ASNs')
    ...
    log_info(
        tag + 'BlackList Totals: ' + regexcount + ' REGEXES, ' + ipcount + ' IPs/CIDRs, ' + domaincount + ' DOMAINS and ' + asncount + ' ASNs')
...
def add_exclusion(dlist, elist, slist, listname):
    ...
    for domain in dom_sort(elist.keys()):
        id = elist[domain]
        if (debug >= 2): log_info(
            tag + 'Adding excluded entry \"' + domain + '\" to ' + listname + ' (from ' + id + ')')
        if domain in dlist:
    ...
    if (debug >= 2): log_info(
        tag + 'Added ' + str(count) + ' new exclusion entries to \"' + listname + '\", went from ' + str(
            before) + ' to ' + str(after))
    ...
    if (len(name) > 0):
        try:
            with open(name, 'r') as f:
                    ...
                    if len(entry) > 0 and (not entry.startswith('#')):
                        ...
                        if (isregex.match(entry)):
                            ...
                            if wwwregex.match(domain) and domain.count('.') > 1:
                                ...
                                if (debug >= 2): log_info(
                                    tag + 'Stripped \"' + label + '\" from \"' + domain + '\" (' + newdomain + ')')
                                domain = newdomain
                            ...
                            if domain:
                                if tldlist and (not force) and (not safed):
                                    ...
                                    if not tld in tldlist:
                                        if (debug >= 2): log_info(
                                            tag + 'Skipped DOMAIN \"' + domain + '\", TLD (' + tld + ') does not exist')
                                        domain = False
                                        skipped += 1
                                ...
                                if domain:
                                    if unwhite:
                                        if (debug >= 2): log_info(
                                            tag + 'Added \"' + domain + '\" to ' + 'safe-unwhite-list')
                                        safewlist[domain] = id
                                        skipped += 1
                                        ...
                                        if safed:
                                            if (debug >= 2): log_info(
                                                tag + 'Added \"' + domain + '\" to ' + bw + '-safelist')
                ...
                if (debug >= 2): log_info(
                    tag + 'Processed ' + bw + 'list ' + str(total) + ' entries and skipped ' + str(
                        skipped) + ' (existing/invalid) ones from \"' + orgid + '\"')
                if (debug >= 1): log_info(
                    tag + 'Fetched ' + bw + 'list ' + str(regexcount - orgregexcount) + ' REGEXES, ' + str(
                        ipcount) + ' CIDRs, ' + str(domaincount) + ' DOMAINS and ' + str(
                        asncount) + ' ASNs from ' + bw + '-file/list \"' + name + '\"')
                if (debug >= 2): log_info(tag + 'Total ' + bw + 'list ' + str(len(regexlist) / 3) + ' REGEXES, ' + str(
                    len(iplist4) + len(iplist6)) + ' CIDRs, ' + str(len(domainlist)) + ' DOMAINS and ' + str(
                    len(asnlist)) + ' ASNs in ' + bw + '-list')
...
def generate_response(qstate, rname, rtype, rrtype, newttl):
    ...
    if (len(intercept_address) > 0 and len(intercept_host) > 0) and (
            rtype in ('A', 'CNAME', 'MX', 'NS', 'PTR', 'SOA', 'SRV', 'TXT', 'ANY')):
        qname = False
...
def optimize_domlists(name, listname):
    ...
    for domain in domlist:
        if not domain.endswith(parent):
            ...
            if (debug >= 3): log_info(
                tag + '\"' + listname + '\": Removed domain \"' + domain + '\" redundant by parent \"' + parent.strip(
                    '.') + '\"')
    ...
    if (debug >= 2): log_info(
        tag + '\"' + listname + '\": Number of domains went from ' + str(before) + ' to ' + str(after) + ' (' + str(
            count) + ')')
...
def unwhite_domain(wlist, blist):
    ...
    for entry in dom_sort(wlist.keys()):
        ...
        if notfound and nomatchtld:
            if not check_regex(entry, 'black', False):
                if (debug >= 3): log_info(
                    tag + 'Removed redundant white-listed domain \"' + entry + '\" (No blacklist hits)')
            else:
                legit = True
    ...
    if (debug >= 2): log_info(tag + 'Number of white-listed domains went from ' + str(before) + ' to ' + str(
        after) + ' (Unwhitelisted ' + str(count) + ')')
...
def uncomplicate_lists(whitelist, rwhitelist, blacklist, safelist):
        ...
        if '.' + domain + '#' in checklistb:
            ...
            for found in filter(lambda x: x.endswith('.' + domain), listb):
                if not dom_find(found, safelist):
                    if (debug >= 3): log_info(
                        tag + 'Removed blacklist-entry \"' + found + '\" due to white-listed parent \"' + domain + '\"')
                    listb.remove(found)
                else:
                    if (debug >= 3): log_info(
                        tag + 'Preserved white-listed/safe-black-listed blacklist-entry \"' + found + '\" due to white-listed parent \"' + domain + '\"')
    ...
    for i in range(0, len(rwhitelist) / 3):
        ...
        for found in filter(checkregex.search, listb):
            if not dom_find(found, safelist):
                listb.remove(found)
                if (debug >= 3): log_info(
                    tag + 'Removed \"' + found + '\" from blacklist, matched by white-regex \"' + rwhitelist[
                        i, 2] + '\"')
            else:
                if (debug >= 3): log_info(
                    tag + 'Preserved safe-black-listed \"' + found + '\" from blacklist, matched by white-regex \"' +
                    rwhitelist[i, 2] + '\"')
    ...
    if (debug >= 2): log_info(
        tag + 'Number of black-listed domains went from ' + str(before) + ' to ' + str(after) + ' (' + str(count) + ')')
...
def exclude_domlist(domlist, excludelist, listname):
    ...
    for domain in dom_sort(excludelist.keys()):
        ...
        if domain in newlist:
            ...
            del newlist[domain]
            if (debug > 1): log_info(
                tag + 'Removed excluded entry \"' + domain + '\" from \"' + listname + '\" (' + lname + ')')
            checklist = '#'.join(newlist.keys()) + '#'
    ...
    log_info(tag + '\"' + listname + '\" went from ' + str(before) + ' to ' + str(after) + ', after removing ' + str(
        deleted) + ' excluded entries')
...
def uncomplicate_ip_lists(cwhitelist, cblacklist, listname):
    ...
    for ip in listb:
        if ip in listw:
            if (debug >= 3): log_info(
                tag + 'Removed blacklist-entry \"' + ip + '\" due to white-listed \"' + cwhitelist[ip] + '\"')
            listb.remove(ip)
    ...
    if (debug >= 2): log_info(
        tag + 'Number of black-listed ' + listname + ' went from ' + str(before) + ' to ' + str(after) + ' (' + str(
            count) + ')')
...
def unreg_lists(dlist, rlist, safelist, listname):
    ...
    for i in range(0, len(rlist) / 3):
        ...
        for found in filter(checkregex.search, dlist):
            ...
            if not dom_find(name, safelist):
                del dlist[found]
                if (debug >= 3): log_info(
                    tag + 'Removed \"' + found + '\" from \"' + name + '\" matched by regex \"' + rlist[i, 2] + '\"')
            else:
                if (debug >= 3): log_info(
                    tag + 'Preserved safelisted \"' + found + '\" from \"' + name + '\" matched by regex \"' + rlist[
                        i, 2] + '\"')
    ...
    if (debug >= 2): log_info(
        tag + 'Number of \"' + listname + '\" entries went from ' + str(before) + ' to ' + str(after) + ' (' + str(
            count) + ')')
...
def aggregate_ip(iplist, listname):
                ...
                if children:
                    for child in children:
                        if child in undupped:
                            ...
                            if (debug >= 3): log_info(
                                tag + 'Removed ' + child + ', already covered by ' + ip + ' in \"' + iplist[ip] + '\"')
    ...
    if (debug >= 2): log_info(
        tag + '\"' + listname + '\": Number of IP-Entries went from ' + str(before) + ' to ' + str(after) + ' (' + str(
            count) + ')')
            ...
            with open(nameserverslist, 'r') as f:
                for line in f:
                    ...
                    if not (entry.startswith("#")) and not (len(entry) == 0):
                        ...
                        if (debug >= 1): log_info(
                            tag + 'Fetched Nameservers for \"' + element[0] + '\" (' + element[1] + ')')
                        for nsip in element[1].split(','):
                            ...
                            if add_cidr(cwhitelist4, cwhitelist6, nsip, 'SafeDNS Servers'):
                                if (debug >= 3): log_info(
                                    tag + 'Auto-added ' + nsip + ' from SafeDNS list to whitelist')
                            else:
...
def get_asn(qname, ip):
    ...
    if (debug >= 2): log_info(
        tag + 'Got ASN \"' + asn + '\" from ' + source + ' for ' + qname + '/' + ip + ' (Prefix: ' + prefix + ')')
...
def safe_dns(query, qname, type, baseip):
    ...
    if baseasn in asnwhitelist:
        if (debug >= 2): log_info(
            tag + 'Found white-listed \"' + qname + '\" BASE-ASN: \"' + baseasn + '\" (' + asnwhitelist[baseasn] + ')')
        score = 100
    elif baseasn in asnblacklist:
        if (debug >= 2): log_info(
            tag + 'Found black-listed \"' + qname + '\" BASE-ASN: \"' + baseasn + '\" (' + asnblacklist[baseasn] + ')')
        blockit = True
        score = 0
                    ...
                    for answer in response:
                        ...
                        if ip and ipregex.match(ip):
                            if ip == '0.0.0.0' or ip.startswith('127.0.0.'):
                                if (debug >= 2): log_info(
                                    tag + '\"' + qname + '\" is DNS-black-listed by ' + ns + ' (' + ip + ')')
                                blockit = True
                                score = 0
                                break
                            ...
                            if (debug >= 2): log_info(
                                tag + ns + ': ' + qname + '/' + ip + ' belongs to ASN: \"' + asn + '\"')

                            # ASN Black/whitelisted? Score accordingly and break
                            if asn in asnwhitelist:
                                if (debug >= 2): log_info(
                                    tag + 'Found white-listed \"' + qname + '\" ASN: \"' + asn + '\" (' + asnwhitelist[
                                        asn] + ')')
                                score = 100
                                break
                            elif asn in asnblacklist:
                                if (debug >= 2): log_info(
                                    tag + 'Found black-listed \"' + qname + '\" ASN: \"' + asn + '\" (' + asnblacklist[
                                        asn] + ')')
                                score = 0
                                blockit = True
                                break
            ...
            if msg:
                ...
                if (rc == RCODE_NOERROR) or (rep.an_numrrsets > 0):
                    ...
                    if qname:
                        ...
                        qtype = qstate.qinfo.qtype_str.upper()
                        if (debug >= 2): log_info(
                            tag + 'Starting on RESPONSE for QUERY \"' + qname + '\" (RR:' + qtype + ')')
                        ...
                        if not in_cache('white', qname):
                                            ...
                                            if not in_list(dname, 'black', 'RESPONSE', type):
                                                ...
                                                for j in range(0, data.count):
                                                    ...
                                                    if type in ('A', 'AAAA', 'CNAME', 'MX', 'NS', 'PTR', 'SOA', 'SRV'):
                                                        ...
                                                        if type == 'A':
                                                            name = "%d.%d.%d.%d" % (
                                                            ord(answer[2]), ord(answer[3]), ord(answer[4]),
                                                            ord(answer[5]))
                                                        elif type == 'AAAA':
                                                            name = "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x" % (
                                                            ord(answer[2]), ord(answer[3]), ord(answer[4]),
                                                            ord(answer[5]), ord(answer[6]), ord(answer[7]),
                                                            ord(answer[8]), ord(answer[9]), ord(answer[10]),
                                                            ord(answer[11]), ord(answer[12]), ord(answer[13]),
                                                            ord(answer[14]), ord(answer[15]), ord(answer[16]),
                                                            ord(answer[17]))
                                                        ...
                                                        if name:
                                                            if (debug >= 2): log_info(
                                                                tag + 'Checking \"' + dname + '\" -> \"' + name + '\" (RR:' + type + ') (TTL:' + str(
                                                                    rep.ttl) + ')')
                                                            ...
                                                            if safedns and notwhitelisted and type in ('A', 'AAAA'):
                                                                # SafeDNS ASN Check
                                                                if (debug >= 2): log_info(
                                                                    tag + 'Starting SafeDNS-Check on \"' + dname + '\"')

                                                                score = safe_dns(qname, dname, type, name)

                                                                if (debug >= 1): log_info(
                                                                    tag + 'SafeDNS-Score for \"' + dname + '\" (' + name + ') is ' + str(
                                                                        score) + '%%')
                                                                ...
                                                                if score < safescore:
                                                                    ...
                                                                    if safednsblock:
                                                                        if (debug >= 1): log_info(
                                                                            tag + 'SafeDNS HIT on \"' + dname + '\", score below ' + str(
                                                                                safescore) + '%%, BLOCKING!')
                                                                        blockit = True
                                                                        break
                                                                    else:
                                                                        if (debug >= 1): log_info(
                                                                            tag + 'SafeDNS HIT on \"' + dname + '\", score below ' + str(
                                                                                safescore) + '%%, MONITORING!')

                                                                add_to_cache('white', dname)  # !!! Need to check impact

                                                                if (debug >= 2): log_info(
                                                                    tag + 'Finished SafeDNS-Check on \"' + dname + '\"')
                                                            ...
                                                            if notwhitelisted and autowhitelist:
                                                                if (debug >= 2): log_info(
                                                                    tag + 'Auto-Whitelisted \"' + qname + '\"')
                                                                add_to_cache('white',
                                                                             name)  # !!!! Maybe not, maybe have a "validated" cache instead
                            ...
                            if blockit:
                                ...
                                if target:
                                    if (debug >= 1): log_info(
                                        tag + 'REDIRECTED \"' + lname + '\" (RR:' + rtype + ') to ' + target)
                                    qstate.return_rcode = RCODE_NOERROR
                                else:
                                ...
                                for lname in lastname.keys():
                                    if (debug >= 2): log_info(
                                        tag + 'COLLAPSE CNAME \"' + firstname + '\" -> ' + lastname[
                                            lname] + ' \"' + lname + '\"')
                        ...
                        if (debug >= 2): log_info(
                            tag + 'Finished on RESPONSE for QUERY \"' + qname + '\" (RR:' + qtype + ')')
