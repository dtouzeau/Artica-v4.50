...
readonly MAIN_BLOCKLIST="/etc/notrack/conf.d/notrack.list"
...
#--------------------------------------------------------------------
# Count number of lines in /etc/notrack/conf.d block lists
#
# Globals:
#   None
# Arguments:
...
function CountLines() {
  ...
  for ListFile in /etc/notrack/conf.d/*.list; do
...
function get_custom() {
  ...
  if [[ ${Config[bl_custom]} == "" ]]; then      #Are there any custom block lists?
    ...
    for FileName in /etc/notrack/custom_*; do    #Clean up old custom lists
      ...
      delete_file "/etc/notrack/conf.d/$FileName.list"
      delete_file "/etc/notrack/$FileName.csv"
      delete_file "/etc/notrack/tmp/$FileName.txt"
    done
    return
  fi
  ...
  for ListUrl in "${CustomList[@]}"; do
    ...
    FileName=${FileName%.*}                      #Remove file extension
    dlfile="/etc/notrack/tmp/custom_$FileName.txt"
  ...
  for FileName in /etc/notrack/conf.d/custom_*; do    #Clean up old custom lists
    FileName=${FileName##*/}                     #Get filename from path
    FileName=${FileName%.*}                      #Remove file extension
    FileName=${FileName:7}                       #Remove custom_    
    if [ ! "${CustomListArray[$FileName]}" ]; then
      delete_file "/etc/notrack/conf.d/custom_$FileName.list"
...
function get_list() {
  local list="$1"
  local dlfile="/etc/notrack/tmp/$1.txt"
  local zipfile=false
    
  
  if [[ ${URLList[$list]} =~ \.zip$ ]]; then     #Is the download a zip file?
    dlfile="/etc/notrack/tmp/$1.zip"
    zipfile=true
  fi
  ...
  if [ ! -s "$dlfile" ]; then                    #Check if list has been downloaded
    echo "File not downloaded ${URLList[$list]}"    
    return 1
  fi
  
  if [[ $zipfile == true ]]; then                #Do we need to unzip?
    unzip -o "$dlfile" -d "/etc/notrack/tmp/"                #Unzip not quietly (-q)
    dlfile="/etc/notrack/tmp/$3"                             #dlfile is now the expected unziped file    
...
function insert_data() {
  ...
  printf "%s\n" "${SQLList[@]}" > "/etc/notrack/tmp/$1.csv"   #Output arrays to file
  echo "/etc/notrack/tmp/$1.csv"
}
...
function process_tldlist() { 
  ...
  get_filetime "/etc/notrack/conf.d/tld.list"
...
function process_whitelist() {  
  ...
  if [ "${#DNSList[@]}" -gt 0 ]; then            #How many items in DNS List array?
    ...
    echo "Writing white list to /etc/notrack/conf.d/whitelist.list"
    printf "%s\n" "${DNSList[@]}" > "/etc/notrack/conf.d/whitelist.list"   #Output array to file    
  else                                           #No sites, delete old list file
    echo "No sites to white list from blocked TLD's"
    delete_file "/etc/notrack/conf.d/whitelist.list"
  fi
  echo  
}
...
mkdir -p /etc/notrack/conf.d || true
mkdir -p /etc/notrack/tmp || true
