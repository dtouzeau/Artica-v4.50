...
def load_configuration(args):
    ...
    if (args.mailbox_id):
        options['mailbox_id']=args.mailbox_id
...
def main():
    ...
    argparser.add_argument('-p', dest='pidfile', help="pid target path")
    argparser.add_argument('-i', dest='mailbox_id', help="id of the mailbox/account")
    args = argparser.parse_args()
    options = load_configuration(args)

    if "pidfile" in options:
        print("Create pid %s" % options["pidfile"])
        pid = str(os.getpid())
