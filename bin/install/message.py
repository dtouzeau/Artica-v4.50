...
class Message:
    ...
    def getmailheader(self, header_text, default="ascii"):
        ...
        try:
            ...
            for i, (text, charset) in enumerate(headers):
                ...
                if charset is None: charset='ascii'
                headers[i]=str(text).decode(charset).encode('utf8')
                textheader=textheader+headers[i]

            try:
                return u"".join(headers)
                print("Join header issue [95]")
            except:
                return textheader
    ...
    def __if_number_get_string(self,number):
    ...
    def get_string(self,strOrUnicode, encoding='utf-8'):
        strOrUnicode = self.__if_number_get_string(strOrUnicode)
    ...
    def createMetaFile(self):
        ...
        print("JSON: "+self.msg['Message-Id'])
        print("JSON: " + self.getSubject())

        message_id=self.get_unicode(self.msg['Message-Id'])
        subject=self.get_unicode(self.getSubject())
        From=self.getFrom()


        serialz={}
        serialz["message_id"] = message_id
        serialz["Subject"]=subject
        serialz["From"] = From
        serialz["To"] = tos
        serialz["ccs"] = ccs
        serialz["Date"] = rfc2822
        serialz["Utc"] = iso8601
        serialz["Attachments"] = attachments
        self.file_put_contents('%s/metadata.array' %(self.directory),serialize(serialz))
        self.file_put_contents('%s/content.txt' % (self.directory), text_content)
    ...
    def createHtmlFile(self, parts, embed):
        ...
        subject = self.get_string(cgi.escape(self.getSubject()))
        fromname = cgi.escape(self.getFrom()[0])
        utf8_content=self.get_string(utf8_content)
...
</html>""" % (fromname,subject,subject,fromname, utf8_content)

        targetpath=os.path.join(self.directory, 'message.html')
        self.file_put_contents(targetpath,utf8_content)
        return True
