...
# V3.14     28 May 2022
# V3.15     27 June 2025    Fix crash when pid is removed during the loop ( Artica tech )
...
def getMemStats(pid):
    ...
    Pss_lines = []
    try:
        statm_line = proc.open(pid, 'statm').readline()
    except LookupError:
        return 0.0, 0.0, 0.0, 0.0, pid
    Rss = int(statm_line.split()[1]) * PAGESIZE
    Swap_lines = []
    Swap_pss_lines = []
