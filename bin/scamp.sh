...
# Version: 5.3B
# Tue, 21 February 2012 18:36:43 GMT
...
bofhland_cracked_URL.ndb.sig
bofhland_malware_URL.ndb
bofhland_malware_URL.ndb.sig
bofhland_phishing_URL.ndb
bofhland_phishing_URL.ndb.sig
crdfam.clamav.hdb
...
if command -v curl &>/dev/null; then
  DL_AGENT="curl"
  CMD="$CURL_PROXY_CMD -q -s -S --remote-name --location --remote-time"
...
\tPlease Stand By ***\n"
      ...
      ;;
      [curl]*)
        if ! curl ${CURL_PROXY_CMD}-q -s -S "${gpg_key_url}" | ${GPG_AGENT} ${GPG_OPT} --import -
          then
            printf "\tERROR: could not import GPG public key; aborting\n"
          exit 7
        fi
