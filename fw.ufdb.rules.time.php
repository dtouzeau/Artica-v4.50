...
function period_add(){
	...
	echo $tpl->form_outside("{set_the_period}", @implode("\n", $form),null,$buttonname,"RefreshUfdbTimeList();LoadAjax('table-loader-ufdbrules-service','fw.ufdb.rules.php?table=yes');$close","AsDansGuardianAdministrator");
	return;
...
function table(){
	...
	$html[]=$tpl->form_outside("{behavior}", @implode("\n", $form),null,"{apply}","LoadAjax('table-loader-ufdbrules-service','fw.ufdb.rules.php?table=yes');","AsDansGuardianAdministrator");

	$add_button="<label class=\"btn btn btn-primary\" 
    OnClick=\"Loadjs('$page?period-add-js=yes&ID={$_GET["ID"]}&TIMEID=-1')\">
    <i class='fa fa-plus'></i> {add_a_period} </label>";

    $PowerDNSEnableClusterSlave=intval($GLOBALS["CLASS_SOCKETS"]->GET_INFO("PowerDNSEnableClusterSlave"));
    if($PowerDNSEnableClusterSlave==1){$add_button=null;}
	...
	foreach ($TimeSpace["TIMES"] as $TIMEID=>$array){
		...
		$html[]="<td>". $tpl->icon_delete($js_delete,"AsDansGuardianAdministrator")."</td>";
		$html[]="</tr>";
