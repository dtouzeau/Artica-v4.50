...
function make_appliance($type="PROXY_APPLIANCE"){
    ...
    echo "Scanning ISO in $isodir\n";
    $iso=buildedgetiso($isodir);


    if(!is_file($iso)){
        echo "Scanning ISO in /root/images\n";
        $iso=buildedgetiso("/root/images");
    }

	if(!is_file($iso)){
		echo "Unable to stat any iso in $isodir or /root/images\n";
		return;
	}
	...
	$f=array();
	$welcome="Kaspersky Security Center Installer";
...
//	if($size<421986304){echo "$originalisopath wrong size: $size < 421986304\n";return false;}
	...
	$f=array();
	$welcome="Welcome to the Security Center Linux";
...
function prepareiso($type="PROXY_APPLIANCE"):bool{
	...
	@mkdir("/root/build/etc/systemd/system",0755,true);
    ...
    @copy("/root/init.d/artica-iso.service","/root/build/etc/systemd/system/artica-iso.service");
    ...
    @file_put_contents("/root/build/etc/artica-postfix/security-center","1");
    ...
    $CopyBins[]="bin/install/postgresql.templates.tar.gz";
    $SimpleCDD="/home/working-profiles/PROXY_APPLIANCE/tmp/images/extract/simple-cdd";
    if(file_exists("$SimpleCDD/artica-phpfpm-service")){
        @unlink("$SimpleCDD/artica-phpfpm-service");
    }
    echo "Copy $WorkSourceDir/bin/articarest -> $SimpleCDD/artica-phpfpm-service\n";
    copy("$WorkSourceDir/bin/articarest", "$SimpleCDD/artica-phpfpm-service");
    if(!is_file("$SimpleCDD/artica-phpfpm-service")){
        echo __LINE__." $SimpleCDD/artica-phpfpm-service no suche file!!!!\n";
        die();
    }
    @chmod("$SimpleCDD/artica-phpfpm-service",0755);


    $PKGS=array("postgres.tar.gz",
        "memcached.tar.gz","monit.tar.gz",
        "nginx.tar.gz","syslog.tar.gz","autofs.tar.gz");
    ...
    mkdir("/root/build/home/kl", 0755, true);
    copy("/root/ksc-web-console.deb","/root/build/home/kl/ksc-web-console.deb");
    copy("/root/ksc64.deb","/root/build/home/kl/ksc64.deb");
    ...
    $SECURITY_CENTERVersion="15.1.0-11795";
	$title="Security Center v$SECURITY_CENTERVersion";
	...
	if($type=="PROXY_APPLIANCE"){
		$isoname="security-center-$SECURITY_CENTERVersion.$HotfixBin.debian10-amd64.dvd.iso";
	}
...
function pressed($targetpath=null,$type="PROXY_APPLIANCE"){
	...
	$f[]="d-i     netcfg/get_hostname     string  security-center";
