...
function page(){
	...
	$t=time();
	if(!isset($_SESSION["DHCPD_SEARCH"])){$_SESSION["DHCPD_SEARCH"]="today this hour 50 events";}
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["DHCPD_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"Search$t(event);\">
...
function search(){
	...
	$sock->getFrameWork("dhcpd.php?syslog=$line");
	$filename=PROGRESS_DIR."/dhcpd.syslog";
...
";
	...
	if(count($data)>3){$_SESSION["DHCPD_SEARCH"]=$_GET["search"];}
	rsort($data);
	...
	foreach ($data as $line){
		...
		$ACTION=null;
		$FF=false;
		if(!preg_match("#^([A-Za-z]+)\s+([0-9]+)\s+([0-9:]+)\s+.*?[dhcpd|dhcrelay]\[([0-9]+)\]:\s+(.+)#", $line,$re)){
		...
		if(preg_match("#(Corrupt|bad udp|a bug|README file|Ignoring requests)#i", $line)){
			$line="<span class='text-warning'>$line</span>";
		}
		
		if(preg_match("#(is down|No subnet|Not configured|Unable to add forward map|exiting|No such file or directory)#i", $line)){
			$line="<span class='text-danger'>$line</span>";
		}
		
		if(preg_match("#(DHCPACK|DHCPREQUEST|DHCPOFFER|DHCPDISCOVER)#i", $line)){
	...
	$html[]="</tbody></table>";
	$html[]="<div><i>".@file_get_contents(PROGRESS_DIR."/dhcpd.syslog.pattern")."</i></div>";
	echo @implode("\n", $html);
