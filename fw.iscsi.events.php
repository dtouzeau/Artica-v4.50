...
function page(){
	...
	$t=time();
	if(!isset($_SESSION["ISCSI_SEARCH"])){$_SESSION["ISCSI_SEARCH"]="today this hour 50 events";}
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["ISCSI_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"javascript:Search$t(event);\">
...
function search(){
	...
	$sock->getFrameWork("iscsi.php?syslog=$line");
	$filename=PROGRESS_DIR."/iscsi.syslog";
...
";
	...
	if(count($data)>3){$_SESSION["ISCSI_SEARCH"]=$_GET["search"];}
	rsort($data);
	...
	$html[]="</tbody></table>";
	$html[]="<div><i>".@file_get_contents("/usr/share/artica-postfix/ressources/logs/web/iscsi.syslog.pattern")."</i></div>";
	echo @implode("\n", $html);
