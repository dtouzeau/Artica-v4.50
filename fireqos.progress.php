...
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/fireqos.reconfigure.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{QOS}");
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes&comand=".urlencode($_GET["comand"])."','$title');";
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file={$_GET["md5file"]}&comand=".urlencode($_GET["comand"])."');
}
...
if($md5file<>$_GET["md5file"]){
	...
	var xStart$time= function (obj) {
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&comand=".urlencode($_GET["comand"])."');
	}		
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('firehol-wizard')){ 
			YahooWinHide();
			
		}
		if(document.getElementById('proxy-dashboard')){
...
function popup(){
	...
	$sock->getFrameWork("firehol.php?reconfigure-qos-progress=yes&comand=".urlencode($_GET["comand"]));
	$t=$_GET["t"];
