...
$('#flexRT$t').flexigrid({
	...
	dataType: 'json',
	colModel : [
	{display: '$hour', name : 'hour', width : 52, sortable : false, align: 'right'},
...
function showlist(){
	...
	$currentHour=date("H");
	$current_tablename="RTTH_".date("YmdH");
	$q=new mysql_squid_builder();
...
$c=0;
	while (list ($member, $mainarray) = each ($ActiveRequestsR["ALL"]) ){
		...
		while (list ($time, $size) = each ($mainarray) ){	
			...
			if($time==$currentHour){$LINK_RTT=TRUE; }
				...
				if($LINK_RTT){
					$member_link=$q->LINK_RTTH_CURRENT("uid", $member, $member,16);
					if(preg_match("#[0-9\.]+#", $member)){
						$member_link=$q->LINK_RTTH_CURRENT("ipaddr", $member, $member,16);
					}
				}
				
				$data['rows'][] = array(
						'id' => md5(serialize($mainarray)),
						'cell' => array("<span style='font-size:16px'>{$time}h</span>",
								"$member_link",
