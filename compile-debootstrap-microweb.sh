...
if [ ! -f "$ROOT/artica-web.ext4" ];then
  echo "Create a hard drive of 2.5g"
  dd if=/dev/zero of=$ROOT/artica-web.ext4 bs=1M count=2560
  echo "Format $ROOT/artica-web.ext4"
  mkfs.ext4 $ROOT/artica-web.ext4
fi

if [ ! -f "$DIR/bin/apt" ]; then
  echo "Installing BootStrap..."
  debootstrap --include openssh-server,unzip,git,apt,vim,curl,cron,libpcap0.8 'buster' $DIR/  http://deb.debian.org/debian
fi
