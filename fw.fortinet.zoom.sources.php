...
function js(){
    ...
    return $tpl->js_dialog2("{sources}: $record","$page?popup=yes&time=$time&record=$record",1250);
}
...
function popup(){
    ...
    $html[]="<tr>";
    $html[]="<td style='vertical-align:top;width:50%'>";
    $html[]="<div class='center' style='margin-top:0px;height:558px' id='top-services-$t'>";
    $html[]="</td>";
    $html[]="<td style='vertical-align:top;width:50%;padding-left:10px'>";
    $html[]="<div class='center' style='margin-top:0px' id='top-services-table-$t'>";
    $html[]="</td>";
    $html[]="</tr>";
    ...
    $html[]="</table>";
    $html[]="<script>";
    $html[]="Loadjs('$page?linear=yes&container=linear-$t&time=$time&record=$record&field=srcip')";
    $html[]="Loadjs('$page?graph=yes&container=top-sources-$t&time=$time&record=$record&field=dstip')";
    $html[]="Loadjs('$page?graph=yes&container=top-services-$t&time=$time&record=$record&field=service')";
    $html[]="LoadAjax('top-sources-table-$t','$page?table=yes&time=$time&record=$record&field=dstip&hostname=yes')";
    $html[]="LoadAjax('top-services-table-$t','$page?table=yes&time=$time&record=$record&field=service')";
...
function graph_top_sources_query(){
    ...
    $field=$_GET["field"];
    if($time=="today") {
        return "SELECT SUM(rcvbytes) as tsize,$field as zfield FROM fortigate_access WHERE srcip='$record' GROUP BY $field ORDER BY tsize DESC LIMIT 15";
    }
    if($time=="this_month") {
        return "SELECT SUM(rcvbytes) as tsize,$field as zfield FROM fortigate_days WHERE srcip='$record' AND zdate >= date_trunc('month', current_date) GROUP BY $field ORDER BY tsize DESC LIMIT 15";
    }
