...
function GRAB_DATAS($ligne,$md5){
	...
	$SQLA[]="SELECT SUM(size) as size, SUM(rqs) as rqs,$TimeGroup,familysite,useragent,mac,userid FROM agents_access";
	$SQLA[]="WHERE";
	$SQLA[]="$SEARCHTEXT (zdate >'".date("Y-m-d H:i:s",$from)."' and zdate < '".date("Y-m-d H:i:s",$to)."')";
	if($USER_FIELD=="USERID"){$SQLA[]="AND USERID != 'none'";}
	$SQLA[]="GROUP BY date_trunc('hour', zdate),familysite,useragent,mac,userid ORDER BY zdate";
	
	build_progress("{step} {waiting_data}: BigData engine, (UserAgent) {please_wait}",6);
	$unix=new unix();
	$hostname=$unix->hostname_g();
	
	
	$sql="CREATE TABLE IF NOT EXISTS \"{$md5}report\" (zDate timestamp,mac macaddr,userid VARCHAR(64) NULL,familysite VARCHAR(128) NULL,useragent VARCHAR(256) NULL,size BIGINT,rqs BIGINT,cached int)";
	...
	$q->QUERY_SQL("create index userid{$md5}report on \"{$md5}report\"(userid);");
	$q->QUERY_SQL("create index useragent{$md5}report on \"{$md5}report\"(useragent);");
	$q->QUERY_SQL("create index mac{$md5}report on \"{$md5}report\"(mac);");
	$q->QUERY_SQL("create index familysite{$md5}report on \"{$md5}report\"(familysite);");
	$q->QUERY_SQL("TRUNCATE TABLE \"{$md5}report\"");
	
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (size,rqs,zdate,familysite,useragent,mac,userid) $sql";
