...
if($argv[1]=="--ad-sync"){ActiveDirectorySync();exit();}
if($argv[1]=="--create-mbx"){createMbx($argv[2]);exit();}

if($argv[1]=="--move-default-current"){move_default_dir_to_currentdir();exit();}
...
function rebuild_all_mailboxes_events($text,$ou_encoded){
	echo $text ."\n";
		$file=PROGRESS_DIR."/".md5($ou_encoded)."-mailboxes-rebuilded.log";
		$pid=getmypid();
		$date=date("H:i:s");
		$f =@fopen($file, 'a');
		$line="$date [$pid] $text\n";
