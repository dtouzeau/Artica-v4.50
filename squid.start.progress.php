...
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/squid.start.progress";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.start.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{starting_proxy_service}");
	return $title;
}
...
if($prc==100){
	...
	function Start$time(){
		...
		if(document.getElementById('sequence-proxy')){ LoadAjaxSilent('sequence-proxy','admin.dashboard.proxy.php?sequence-proxy=yes&nofw=yes&sequence=yes&without-cache=yes');}
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?start-progress=yes";
