...
if($argv[1]=="--single-install"){install_single($argv[2]);exit();}




if(preg_match("#--vhosts#",implode(" ",$argv))){vhosts();exit();}
if(preg_match("#--mailman#",implode(" ",$argv))){$GLOBALS["OUTPUT"]=true;mailmanhosts();exit();}
if(preg_match("#--Wvhosts#",implode(" ",$argv))){@mkdir("/usr/local/apache-groupware/conf");@file_put_contents("/usr/local/apache-groupware/conf/vhosts",vhosts(true));exit();}
...
function PIWIGO_INSTALL($servername,$root,$hash=array()){
	...
	$sql="INSERT INTO `piwigo_users` (`id`, `username`, `password`, `mail_address`) 
	VALUES (1, '$appli_user', '$md_password', '$appli_user@$servername'),(2, 'guest', NULL, NULL);";
