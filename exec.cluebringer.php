...
if($argv[1]=="--sql"){testsDatabase();exit();}
if($argv[1]=="--internal-domains"){testsDatabase();internal_domains();exit();}
if($argv[1]=="--passwords"){web_password();exit();}
...
$sock=new sockets();
$Params=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ClubringerMasterConf")));
...
function internal_domains(){
    ...
    foreach ($hash as $domain=>$val){$clue->local_domain_add($domain);}
...
function web_password(){
	$sock=new sockets();
	$ldap=new clladp();
	$array=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ClueBringerMembers")));
    if(!is_array($array)){$array=array();}
    foreach ($array as $uid=>$conf){
