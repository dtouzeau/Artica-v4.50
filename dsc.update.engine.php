...
function page(){
	$sock=new sockets();
	$ArticaTechNetInfluxRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetDSCRepo")));
	$tpl=new templates();
	$version=$sock->GET_INFO("DSC_VERSION");
	$manual_update=$tpl->javascript_parse_text("{manual_update}");
	
	$html="
			
	<div style='font-size:30px;margin-bottom:30px'>{APP_DSC}: {available_versions} &nbsp;|&nbsp; {current}:&nbsp;$version
	...
	foreach ($ArticaTechNetInfluxRepo as $key=>$array){
        foreach ($array as $OS=>$MAIN){
		...
		$button=button("{update2}","Loadjs('dsc.update.engine.progress.php?key=$key&OS=$OS&filename=$FILENAME');",32);
		...
		<tr style='background-color:$color;height:80px'>
			...
			<td style='font-size:28px;padding-left:10px;text-align:right'>
				<a href=\"$URL\" target=_new style='text-decoration:underline'>$FILENAME</a></td>	
