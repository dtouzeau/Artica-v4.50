...
function popup(){
	...
	{separator: true},{name: 'Excel', bclass : 'excel', onpress : exportTo},{separator: true},{name: 'CSV', bclass : 'csv', onpress : exportTo},
	],";
...
function search(){
	...
	$q=new postgres_sql();
	$WEEK=date("W");
	if(!isset($_GET["SUBDIR"])){$_GET["SUBDIR"]="userid";}
	if(isset($_POST["sortname"])){if($_POST["sortname"]<>null){$ORDER="ORDER BY {$_POST["sortname"]} {$_POST["sortorder"]}";}}
	if(isset($_POST['page'])) {$page = $_POST['page'];}
	
	$field=$_GET["SUBDIR"];
	$TIMES_SLOT["hour"]="(SELECT SUM(size) as size,$field FROM rttable_users WHERE zyear='$YEAR' AND zmonth='$MONTH' AND zday='$DAY' AND zhour='$HOUR' GROUP BY $field) as t";
	$TIMES_SLOT["day"]="(SELECT SUM(size) as size,$field FROM rttable_users WHERE zyear='$YEAR' AND zmonth='$MONTH' AND zday='$DAY' GROUP BY $field) as t";
	$TIMES_SLOT["month"]="(SELECT SUM(size) as size,$field FROM rttable_users WHERE zyear='$YEAR' AND zmonth='$MONTH' GROUP BY $field) as t";
	
	
	if($field=="WEBS"){
		$field="domain";
		$TIMES_SLOT["hour"]="(SELECT SUM(size) as size,$field FROM rttable_domains WHERE zyear='$YEAR' AND zmonth='$MONTH' AND zday='$DAY' AND zhour='$HOUR' GROUP BY $field) as t";
		$TIMES_SLOT["day"]="(SELECT SUM(size) as size,$field FROM rttable_domains WHERE zyear='$YEAR' AND zmonth='$MONTH' AND zday='$DAY' GROUP BY $field) as t";
		$TIMES_SLOT["month"]="(SELECT SUM(size) as size,$field FROM rttable_domains WHERE zyear='$YEAR' AND zmonth='$MONTH' GROUP BY $field) as t";
	}
	if($field=="CATS"){
		$field="category";
		$TIMES_SLOT["hour"]="(SELECT SUM(size) as size,$field FROM rttable_domains WHERE zyear='$YEAR' AND zmonth='$MONTH' AND zday='$DAY' AND zhour='$HOUR' GROUP BY $field) as t";
	...
	$table=$TIMES_SLOT[$_GET["timeslot"]];
	...
	while ($ligne = pg_fetch_assoc($results)) {
		$member_value=$ligne[$field];
		$size=FormatBytes($ligne["size"]/1024);
		if($member_value==null){$member_value="Unknown";}
		
		if($field=="domain"){
			if($_GET["timeslot"]=="hour"){
				$zdom=urlencode($member_value);
				$ahref="<a href=\"javascript:blur();\"
				OnClick=\"Loadjs('squid.RTTSIZES.domains.hour.php?domain=$zdom');\"
		...
		if($IPTCP->IsvalidMAC($member_value)){
			...
			$uid=$ClassQ->MacToUid($member_value);
