...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetNtopNGRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('wanproxy-update-engine','wanproxy.update.engine.php');
		LayersTabsAllAfter();
		RTMMailHide();
		CacheOff();
	}
...
function popup(){
	...
	$sock->getFrameWork("ntopng.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
