...
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/phpldapadmin.status";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/phpldapadmin.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{INSTALL_PHPLDAPADMIN}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="services.php?phpldapadmin_install=yes";
