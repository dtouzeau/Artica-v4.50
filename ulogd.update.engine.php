...
function page(){
	$sock=new sockets();
	$ArticaTechNetInfluxRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetUlogdRepo")));
	$tpl=new templates();
	$version=$GLOBALS["CLASS_SOCKETS"]->GET_INFO("UlogdVersion");
	$manual_update=$tpl->javascript_parse_text("{manual_update}");
	
	$html="
			
	<div style='font-size:30px;margin-bottom:30px' id='ThisisUlogdUpdate'>{APP_ULOGD}: {available_versions} &nbsp;|&nbsp; {current}:&nbsp;$version
	...
	while (list ($key, $array) = each ($ArticaTechNetInfluxRepo) ){
		...
		$button=button("{update2}","Loadjs('ulogd.update.engine.progress.php?key=$key&OS=$OS&filename=$FILENAME');",32);
