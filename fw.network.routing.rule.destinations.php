...
function start():bool{
    ...
    echo "<div id='buttons-destination-{$_GET["ID"]}' style='margin-top:10px;margin-bottom:10px'></div>";
...
function rule_js(){
	...
	if($ID>0){
		$ligne=$q->mysqli_fetch_array("SELECT `pattern` FROM routing_rules_dest WHERE ID='$ID'");
		$title=$ligne["pattern"];
		if(!$q->ok){echo $q->mysql_error_html();}
	}


	$tpl->js_dialog2("$RouteName: $title", "$page?rule-popup=$ID&ruleid=$ruleid&function=$function");

}

function export_rules_js():bool{
    header("content-type: application/x-javascript");
    $page=CurrentPageName();
    $tpl=new template_admin();
    ...
    $ligne=$q->mysqli_fetch_array("SELECT `RouteName` FROM routing_rules WHERE ID='$ruleid'");
    $RouteName=$ligne["RouteName"];
    return $tpl->js_dialog2("$RouteName: $title", "$page?export-popup=true&ruleid=$ruleid");

}

function export_rules_popup():bool{
    $tpl=new template_admin();
    $ruleid=intval($_GET["ruleid"]);
    $q=new lib_sqlite("/home/artica/SQLITE/interfaces.db");
    $ligne=$q->QUERY_SQL("SELECT `type`,`gateway`,`pattern`,`status`,`metric`,`zOrder` FROM routing_rules_dest WHERE ruleid='$ruleid'");
    $title=$ligne["pattern"];
    if(!$q->ok){echo $q->mysql_error_html();}
    $btname="{copy}";
    $sqldata="";
    ...
    $form[]=$tpl->field_textarea("routing_rules_dest_export","{routing_rules}",$sqldata);
...
function import_rules_js():bool{
    ...
    $ligne=$q->mysqli_fetch_array("SELECT `RouteName` FROM routing_rules WHERE ID='$ruleid'");
    $RouteName=$ligne["RouteName"];
    return $tpl->js_dialog2("$RouteName: $title", "$page?import-popup=true&ruleid=$ruleid");

}

function import_rules_popup():bool{
    $page=CurrentPageName();
    $tpl=new template_admin();
    $ruleid=intval($_GET["ruleid"]);
    $btname="{import}";
    $form[]=$tpl->field_hidden("ruleidimport", $ruleid);
    $form[]=$tpl->field_textarea("routing_rules_dest_import","{routing_rules}");
    $js[]="dialogInstance2.close();";
   $js[]="LoadAjax('table-loader-iprule','fw.network.routing.php?table=yes');";
   $js[]="LoadAjaxSilent('sourcedest$ruleid','$page?table=$ruleid');";
...
function import_rules_save(){
    ...
    if (empty($_POST["routing_rules_dest_import"])) {
        echo $tpl->post_error("{empty} {routing_rules}");return false;
    }
    $data = urldecode($_POST["routing_rules_dest_import"]);
    $data = preg_split("/\r\n|\r|\n/", $data);
    $q=new lib_sqlite("/home/artica/SQLITE/interfaces.db");
    foreach ($data as $rule)
    {
        $values = explode("|",$rule);
        $count = $q->mysqli_fetch_array("SELECT COUNT(*) as count FROM routing_rules_dest WHERE pattern='{$values[2]}'");
        $nic= $q->mysqli_fetch_array("SELECT nic FROM routing_rules_dest WHERE pattern='{$values[2]}'");
        $currentNic = $q->mysqli_fetch_array("SELECT nic FROM routing_rules_dest WHERE ruleid='{$RULEID}'");
        if(intval($count["count"]) > 0 ) {
            echo $tpl->post_error("{duplicate} {routing_rules} {$values[2]} in {$nic["nic"]}");return false;
        }
        $sql="INSERT INTO routing_rules_dest (`ruleid`,`type`,`gateway`,`pattern`,`status`,`nic`,`metric`,`zOrder`)
		VALUES('{$RULEID}','{$values[0]}','{$values[1]}','{$values[2]}','{$values[3]}','{$currentNic["nic"]}','{$values[4]}','{$values[5]}');";
        $q->QUERY_SQL($sql);
        if(!$q->ok){
            echo $tpl->post_error($q->mysql_error);return false;

        }
    }

    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("network.php?crc32=yes");
    return admin_tracks("Bulk Import network rules done...");
}
...
function route_move_js(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT * FROM routing_rules_dest WHERE ID='$ID'");
    ...
    admin_tracks("Move routing rule $ID $dir");
    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("network.php?crc32=yes");
...
function route_delete_js(){
	...
	$q=new lib_sqlite("/home/artica/SQLITE/interfaces.db");
	$tpl=new template_admin();
	$q->mysqli_fetch_array("DELETE FROM routing_rules_dest WHERE ID='{$_GET["route-delete-js"]}'");
	if(!$q->ok){echo "alert('".$tpl->javascript_parse_text($q->mysql_error)."');";return;}
	echo "$('#{$_GET["md"]}').remove();\n";
	echo "LoadAjax('table-loader-iprule','fw.network.routing.php?table=yes');\n";
    admin_tracks("Remove routing rule id {$_GET["route-delete-js"]}");
    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("network.php?crc32=yes");
...
function rule_popup(){
	...
	$ruleid=$_GET["ruleid"];
    $function="";if(isset($_GET["function"])){$function=$_GET["function"];}
    $function2="";if(isset($_GET["function"])){$function2=$_GET["function"];}
	...
	$form[]=$tpl->field_array_hash($types, "stype", "{type}", $ligne["type"]);
	$form[]=$tpl->field_text("pattern", "{item} ({address}/{network2})", $ligne["pattern"]);
   // $form[]=$tpl->field_interfaces("outiface","{interface}", $ligne["outiface"]);
    ...
    if(strlen($function2)>0){
        $js[]="$function2()";
    }

	
	$html=$tpl->form_outside("$nic: $RouteName: {$ligne["pattern"]}", @implode("\n", $form),null,$btname,@implode(";", $js),"AsSystemAdministrator");
	echo $html;
...
function rule_save():bool{
	...
	if($_POST["pattern"]=="*"){$_POST["pattern"]="0.0.0.0/0";}
    if($_POST["pattern"]=="0/0"){$_POST["pattern"]="0.0.0.0/0";}
	...
	if($ID==0){
		$sql="INSERT INTO routing_rules_dest (`type`,`gateway`,`pattern`,`ruleid`,`nic`,`metric`,`zOrder`,`status`,outiface)
		VALUES('{$_POST["stype"]}','{$_POST["gateway"]}','{$_POST["pattern"]}','{$_POST["ruleid"]}','{$_POST["nic"]}','{$_POST["metric"]}','{$_POST["zOrder"]}','1','{$_POST["outiface"]}');";
	}else{
		 $sql="UPDATE routing_rules_dest SET `metric`='{$_POST["metric"]}', 
		`zOrder`='{$_POST["zOrder"]}', `type`='{$_POST["stype"]}',
		`pattern`='{$_POST["pattern"]}',
		`outiface`='{$_POST["outiface"]}',
    ...
    admin_tracks_post("Save routing destination rule");
    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("network.php?crc32=yes");
    return true;
}
...
function table(){
	...
	$sql="SELECT *  FROM `routing_rules_dest` WHERE ruleid='$MAIN_ID' ORDER BY zOrder";
    if(isset($_GET["search"])){
        ...
        $search=str_replace("**","*",$search);
        $search=str_replace("*","%",$search);
        $sql="SELECT *  FROM `routing_rules_dest` WHERE ruleid='$MAIN_ID' 
	...
	foreach ($results as $index=>$ligne){
		...
		$js="<a href=\"javascript:blur();\" OnClick=\"Loadjs('$page?rule-js={$ligne["ID"]}&ruleid=$MAIN_ID&function2=$function2&function=$function');\" style='text-decoration:underline'>";
		...
		$delete=$tpl->icon_delete("Loadjs('$page?route-delete-js={$ligne["ID"]}&ruleid=$MAIN_ID&md=$md&function2=$function2');","AsSystemAdministrator");
		
		$outiface=$ligne["outiface"];

		
		if($ligne["gateway"]==null){
            if(strlen($outiface)>1){
                $ligne["gateway"]=$outiface;
            }else {
                $ligne["gateway"] = $tpl->icon_nothing();
            }
        }
	...
	$html[]="
	<script>
	document.getElementById('buttons-destination-$MAIN_ID').innerHTML=base64_decode('$buttons');
	NoSpinner();\n".@implode("\n",$tpl->ICON_SCRIPTS)."
