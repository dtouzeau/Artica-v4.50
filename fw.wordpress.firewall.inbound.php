...
if(isset($_GET["autoadd"])){autoadd();exit;}
page();
...
function page(){
    ...
    $html=$tpl->page_header("{incoming_firewall}",
        "fa-solid fa-block-brick-fire","{APP_NGINX_FWIN_EXPLAIN}","$page?table=yes","nginx-infw","reconfigure-wordpress-firewall",false,"wordpress-secure-firewall");


    if(isset($_GET["main-page"])){
        $tpl=new template_admin("{incoming_firewall}",$html);
...
function enable(){
    ...
    header("content-type: application/x-javascript");
    $q=new lib_sqlite("/home/artica/SQLITE/wordpress.db");
    $ligne=$q->mysqli_fetch_array("SELECT enabled FROM wp_infirewall WHERE ID=$ID");
    ...
    echo "// UPDATE wp_infirewall SET enabled='{$ena}' WHERE ID=$ID\n";
    $q->QUERY_SQL("UPDATE wp_infirewall SET enabled='{$ena}' WHERE ID=$ID");
    if(!$q->ok){echo $tpl->js_mysql_alert($q->mysql_error);return;}
...
function js(){
    ...
    if($ID==0){$title="{new_rule}";}else{
        $q=new lib_sqlite("/home/artica/SQLITE/wordpress.db");
        $ligne=$q->mysqli_fetch_array("SELECT address FROM wp_infirewall WHERE ID=$ID");
        $title=$ligne["address"];
    }
...
function autofill(){
    ...
    $tpl=new template_admin();
    $q=new lib_sqlite("/home/artica/SQLITE/wordpress.db");
    $ligne=$q->mysqli_fetch_array("SELECT * FROM wp_infirewall WHERE ID=$ID");
    $description=$tpl->javascript_parse_text($ligne["description"]);


    header("content-type: application/x-javascript");
    echo "document.getElementById('wpfw2-$ID').innerHTML='$description';\n";
    echo "document.getElementById('wpfw1-$ID').innerHTML='{$ligne["address"]}';\n";
...
function popup(){
    ...
    echo $tpl->form_outside($title,$form,"{wordpress_firewall_in_explain}","$btname",$jsafter,"AsWebSecurity",true);
...
function autoadd(){
    ...
    $description=$tpl->_ENGINE_parse_body("{added}")." $user - $date";

    $ligne=$q->mysqli_fetch_array("SELECT ID FROM wp_infirewall WHERE address='{$_GET['autoadd']}'");
    $ID=intval($ligne["ID"]);
    if($ID>0){
        $tpl->js_error("{ERROR_OBJECT_ALREADY_EXISTS}");
        return true;
    }

    $q->QUERY_SQL("INSERT INTO wp_infirewall (address,port,description) VALUES('{$_GET['autoadd']}','0','$description')");
    if(!$q->ok){
        $q->mysql_error=$tpl->javascript_parse_text($q->mysql_error);
        $tpl->js_error($q->mysql_error);
        return false;
    }
    admin_tracks("Add a new blacklisted source for Web Firewall {$_POST['address']}");
    $GLOBALS["CLASS_SOCKETS"]->REST_API("/reverse-proxy/firewall/build");

    echo $tpl->js_display_results("{deny} {$_GET['autoadd']} {success}");
    return true;
}
...
function save(){
    ...
    if($ID==0){
        $q->QUERY_SQL("INSERT INTO wp_infirewall (address,port,description) VALUES('{$_POST['address']}','$port','$description')");
        if(!$q->ok){$tpl=new template_admin();$q->mysql_error=$tpl->javascript_parse_text($q->mysql_error);echo "jserror:$q->mysql_error";return;}
        admin_tracks("Add a new blacklisted source for Web Firewall {$_POST['address']}:$port");
        return;
    }

    $q->QUERY_SQL("UPDATE wp_infirewall SET address='{$_POST['address']}',port=$port,description='$description' WHERE ID=$ID");
    if(!$q->ok){$tpl=new template_admin();$q->mysql_error=$tpl->javascript_parse_text($q->mysql_error);echo "jserror:$q->mysql_error";return;}
    admin_tracks("Update blacklisted source for Web Firewall {$_POST['address']}:$port");
...
function search(){
    ...
    $html[] = "<th data-sortable=true class='text-capitalize' nowrap>&nbsp;</th>";
    ...
    $results=$q->QUERY_SQL("SELECT * FROM wp_infirewall 
  ...
  as integer )");
    ...
    foreach ($results as $index=>$ligne) {
        ...
        $ID = $ligne["ID"];
        $host=null;
        if(preg_match("#^[a-z]+#",$ligne["address"])){
            $host="(".$ligne["address"].")";
            $ligne["address"]=gethostbyname($ligne["address"]);

        }
        $address = $tpl->td_href($ligne["address"],null,"Loadjs('$page?js=$ID')");
        $description = $ligne["description"];
        $md = md5(serialize($ligne));
        $enabled = intval($ligne['enabled']);

        $main=$ipz->ipinfoApi($ligne["address"]);
        ...
        $zinfo_text=@implode("&nbsp;/&nbsp;",$zinfos);
        $html[]="<tr class='$TRCLASS' id='$md'>";
        $html[]="<td style='width:1%'>". $tpl->icon_check($enabled,"Loadjs('$page?enable=$ID')",null,"AsWebSecurity")."</td>";
        $html[]="<td style='width:1%'><img src='img/{$main["flag"]}'></td>";
        $html[]="<td><strong id='wpfw1-$ID'>TCP:&nbsp;$address</strong>&nbsp;$host&nbsp;<small id='wpfw2-$ID'>$description</small>&nbsp;<small>$zinfo_text</small></td>";
    ...
    $TINY_ARRAY["TITLE"]="{incoming_firewall}";
    $TINY_ARRAY["ICO"]="fa-solid fa-block-brick-fire";
    $TINY_ARRAY["EXPL"]="{APP_NGINX_FWIN_EXPLAIN}";
    $TINY_ARRAY["URL"]="nginx-infw";
