...
function js(){
    ...
    return $tpl->js_dialog1("{destination}: $record","$page?popup=yes&time=$time&record=$record",1250);
}
...
function popup(){
    ...
    $html[]="</table>";
    $html[]="<script>";
    $html[]="Loadjs('$page?linear=yes&container=linear-$t&time=$time&record=$record&field=dstip')";
...
function graph_top_sources_query(){
    ...
    $field=$_GET["field"];
    if($time=="today") {
        return "SELECT SUM(rcvbytes) as tsize,$field as zfield FROM fortigate_access WHERE dstip='$record' GROUP BY $field ORDER BY tsize DESC LIMIT 15";
    }
    if($time=="this_month") {
        return "SELECT SUM(rcvbytes) as tsize,$field as zfield FROM fortigate_days WHERE dstip='$record' AND zdate >= date_trunc('month', current_date) GROUP BY $field ORDER BY tsize DESC LIMIT 15";
    }
...
function rqs(){
    ...
    while($ligne=@pg_fetch_array($results)){
        ...
        $size=$size/1024;
        $size=round($size,3);
        $ligne["zdate"]=str_replace("00:00:00","", $ligne["zdate"]);
