...
function popup(){
	...
	$q=new mysql();
	$results=$q->QUERY_SQL("SELECT filename FROM squid_admin_mysql GROUP BY filename ORDER BY filename","artica_events");
	
	if(!$q->ok){$title=$tpl->javascript_parse_text($q->mysql_error);}
	while ($ligne = mysqli_fetch_assoc($results)) {
		$TS[]="{display: '{$ligne["filename"]}', name : 'EXEC:{$ligne["filename"]}'}";
	}
...
function BuildTable$t(){
	$('#events-table-$t').flexigrid({
		...
		{display: '$events', name : 'subject', width : 492, sortable : false, align: 'left'},
		{display: '$daemon', name : 'filename', width :241, sortable : true, align: 'left'},
		],
		$buttons
	
		searchitems : [
		{display: '$events', name : 'subject'},".@implode(",", $TS)."
		],
		sortname: 'zDate',
		sortorder: 'desc',
		usepager: true,
...
function events_table(){
	...
	if(preg_match("#EXEC:(.+)#",$_POST["qtype"],$re)){
		$FORCE=$FORCE." AND filename='{$re[1]}'";
		$_POST["qtype"]="subject";
	}
