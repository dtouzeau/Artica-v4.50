...
function prepareiso($type="PROXY_APPLIANCE"):bool{
    ...
    $UsrArtica="$BUILD_DIR/usr/share/artica-postfix";
    $title=null;
    ...
    @copy("/root/init.d/artica-iso.service", "{$GLOBALS["EXTRACT_PATH"]}/simple-cdd/artica-iso.service");
    echo "Copy $UsrArtica/bin/articarest ->{$GLOBALS["EXTRACT_PATH"]}/simple-cdd/artica-phpfpm-service\n";
    @copy("$UsrArtica/bin/articarest","{$GLOBALS["EXTRACT_PATH"]}/simple-cdd/artica-phpfpm-service");
...
//"splunkforwarder.tgz","phpipam.tar.gz",
    ...
    $UsrArtica="/root/build/usr/share/artica-postfix";
    ...
    $articabin[]="artica-letsencrypt";
    $articabin[]="compile-cd-deb10.php";
    $articabin[]="proxy-watchdog";
    $articabin[]="squid-service";
    $articabin[]="artica-rotate";
    $articabin[]="ArticaStats";
    $articabin[]="theshields-cgi";
    $articabin[]="artica-smtpd.py";
    $articabin[]="dnsfw-example2.py";
    $articabin[]="squid-service.py";
    $articabin[]="squid-service";
    $articabin[]="StatsCommunicator.py";
    $articabin[]="artica-smtpd.py";
    $articabin[]="squid-service.py";
    $articabin[]="getdisks";

    $articaDirs[]="bin/go-shield/client/external_acl_first/.idea";

    foreach ($usrbin as $bin){
		if(is_file("/root/build/usr/bin/$bin")){@unlink("/root/build/usr/bin/$bin");}
	}
    foreach ($articabin as $bin){
        if(is_file("$UsrArtica/bin/$bin")){
            echo "Removing artica-postfix/bin/$bin\n";
            @unlink("$UsrArtica/bin/$bin");
        }
    }
    foreach ($articaDirs as $Directory){
        if(is_dir("$UsrArtica/$Directory")){
            echo "DELETE RMDIR $UsrArtica/$Directory\n";
            shell_exec("/bin/rm -rf $UsrArtica/$Directory");
        }
    }
    $lans=array("fr","de","po","it","es","br","pol","en");
    foreach ($lans as $lang){
        if(is_dir("$UsrArtica/ressources/language/$lang")){
            shell_exec("rm -rf $UsrArtica/ressources/language/$lang");
        }
    }
	...
	@chdir("/root/build");
	$cmd="cd /root/build && {$GLOBALS["tar"]} --checkpoint=100 --checkpoint-action=dot -czf {$GLOBALS["EXTRACT_PATH"]}/simple-cdd/package.tar.gz *";
	system($cmd);
...
*/
	...
	if($type=="PROXY_APPLIANCE"){$title="Artica v$version$addedver";}
	...
	if(is_file("/root/$type.ProductName")){
		...
		if(is_file("/root/$type.ShowVersion")){
			...
			if($ShowVersion==1){
				$title=$title." v$version$addedver";
			}
		}
	}
...
if(is_file("/root/$type.ProductName")){
    ...
    echo "Copy /root/build/usr/share/artica-postfix/bin/articarest -> $targetpath/artica-phpfpm-service\n";
    @copy("/root/build/usr/share/artica-postfix/bin/articarest","$targetpath/artica-phpfpm-service");
	@chmod("$targetpath/artica-iso",0755);

    if (!is_file("$targetpath/artica-phpfpm-service")){
        echo "$targetpath/artica-phpfpm-service no such file\n";
        die();
    }

    @copy("/root/init.d/artica-iso.service", "$targetpath/artica-iso.service");
    if(!is_file("$targetpath/artica-iso.service")){
        echo "$targetpath/artica-iso.service no such file...\n";
        die();
    }
    ...
    $pressed[]="cp /cdrom/simple-cdd/artica-phpfpm-service /target/usr/sbin/artica-phpfpm-service";
    $pressed[]="in-target systemctl enable artica-iso";
    $pressed[]="in-target chmod 0755 /usr/sbin/artica-phpfpm-service >/dev/null 2>&1";
...
function packages($targetfile=null,$type="PROXY_APPLIANCE",$makeiso=false){
    ...
    $packages["python3-asn1crypto"]=true;
    $packages["python3-cffi-backend"]=true;
    $packages["python3-cryptography"]=true;
    $packages["perl-openssl-defaults"]=true;
    $packages["libcrypt-openssl-bignum-perl"]=true;
    $packages["libcrypt-openssl-rsa-perl"]=true;
    $packages["libdigest-bubblebabble-perl"]=true;
    $packages["libdigest-hmac-perl"]=true;
    $packages["libio-socket-ssl-perl"]=true;
    $packages["libnet-dns-sec-perl"]=true;
    $packages["libnet-libidn-perl"]=true;
    $packages["libnet-smtp-ssl-perl"]=true;
    $packages["libnet-ssleay-perl"]=true;
    $packages["libzip4"]=true;
    $packages["zip"]=true;
    $packages["libfontenc1"]=true;
    $packages["xfonts-encodings"]=true;
    $packages["libmongoc-1.0-0"]=true;
    $packages["quagga-ospfd"]=true;
    $packages["ppp"]=true;
    $packages["python3-dns"]=true;
    $packages["python-pyroute2"]=true;
    $packages["python-iptables"]=true;
    $packages["python-paramiko"]=true;
    $packages["python-memcache"]=true;
    $packages["kstart"]=true;
    //$packages["mosquitto-clients"]=true;
    //$packages["mosquitto"]=true;
    $packages["ccrypt"]=true;
    $packages["dante-server"]=true;
    ...
    //POstifx $packages["sa-compile"]=true;
    ...
    // Postfix $packages["spamassassin"]=true;
    ...
    $packages["spawn-fcgi"]=true;
    $packages["xfonts-75dpi"]=true;
    $packages["xfonts-utils"]=true;
    $packages["xfonts-base"]=true;

    //Kasfaleia packages modifications
    //$packages["shim-unsigned"]=true;
    //$packages["shim-signed"]=true;
    //$packages["shim-signed-common"]=true;
    //$packages["shim-helpers-amd64-signed"]=true;
    //$packages["sbsigntool"]=true;
    //$packages["efibootmgr"]=true;
    //$packages["grub-efi-amd64"]=true;
    //$packages["grub-efi-amd64-bin"]=true;
