...
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/varlog.varlog.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}:{log_location}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?varlog-change=yes";
...
function popup(){
	...
	$title="{PLEASE_WAIT_APPLY_PARAMETERS}";
