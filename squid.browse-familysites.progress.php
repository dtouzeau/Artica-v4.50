<?php
$GLOBALS["PROGRESS_FILE"]=PROGRESS_DIR."/squid.browse-familysites.progress.php.log";
$GLOBALS["CACHEFILE"]=$GLOBALS["PROGRESS_FILE"];
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.browse-familysites.progress.php.log.log";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{rebuild}: {reports_center}");
}
...
function suffix(){
	...
	return "&cacheid=".urlencode($_GET["cacheid"]);
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?report-centers-familysites-cache=yes".suffix();
...
function popup(){
	...
	$title="{PLEASE_WAIT_BUILDING_ITEMS}";
