...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/milter-greylist.upgrade.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/milter-greylist.upgrade.progress.txt";
...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetMGreyListRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		LoadAjaxRound('milter-greylist-update','milter-greylist.update.php');
		LayersTabsAllAfter();
		RTMMailHide();
		CacheOff();
	}
...
function popup(){
	...
	$sock->getFrameWork("milter-greylist.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
