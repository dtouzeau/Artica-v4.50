...
function js(){
	...
	$title=$tpl->javascript_parse_text("Wordpress: {backup_now}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			if( document.getElementById('main_adker_tabs') ){
...
function popup(){
	...
	$sock->getFrameWork("wordpress.php?backup-now=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("Wordpress: {backup_now}");
