...
function GRAB_DATAS($ligne,$md5){
	...
	echo "SMTP Attachs: FROM $from to $to $interval organization:$ou user:$user $search\n";
	$table="attachstats";
	...
	if($ou<>null){
		...
		$sqlSource="(select count(*) as hits,sum(size) as size,mailfrom,domainfrom,mailto,domainto,fname,ext,contenttype,$TimeGroup FROM attachstats WHERE $FilterDate AND (($imploded1) OR ($imploded2)) GROUP BY mailfrom,domainfrom,mailto,domainto,fname,ext,contenttype,$TimeGroupBy ORDER BY $TimeGroupBy ) as t";
		$sqlSource="select * FROM $sqlSource $SSEARCH";
		
	}
	
	if($sqlSource==null){
		$sqlSource="(select count(*) as hits,sum(size) as size,mailfrom,domainfrom,mailto,domainto,fname,ext,contenttype, $TimeGroup FROM attachstats WHERE $FilterDate GROUP BY mailfrom,domainfrom,mailto,domainto,fname,ext,contenttype,$TimeGroupBy ORDER BY $TimeGroupBy ) as t";
		$sqlSource="select * FROM $sqlSource $SSEARCH";
	}
	...
	$q->QUERY_SQL("create index mailfrom{$md5}report on \"{$md5}report\"(mailfrom,mailto,contenttype);");
	$q->QUERY_SQL("create index size{$md5}report on \"{$md5}report\"(size,hits,ext);");
	
	
	$sql="INSERT INTO \"{$md5}report\" (hits,size,mailfrom,domainfrom,mailto,domainto,fname,ext,contenttype,zdate) $sqlSource";
