...
function start(){
	...
	$tpl->js_microdaliog_danger("{disable_emergency_mode} (SSL)","$page?popup=yes");
...
function popup(){
    ...
    $jsrestart=$tpl->framework_buildjs("/proxy/ssl/emergency/off",
        "squid.ssl.emergency.progress","squid.ssl.emergency.log","proxy-$t",@implode(";", $js));
	
	$html= "<div id='proxy-$t'></div>
	<center style='margin-top:10px'>". $tpl->button_autnonome("{disable_emergency_mode} (SSL)", $jsrestart, "fa fa-bell-slash")."</center>	
