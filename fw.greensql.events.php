...
function page(){
	...
	$t=time();
	if(!isset($_SESSION["greensql_SEARCH"])){$_SESSION["greensql_SEARCH"]="today this hour 50 events";}
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["greensql_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"javascript:Search$t(event);\">
...
function search(){
	...
	$MAIN=$tpl->format_search_protocol($_GET["search"],false,false,true);
	
	$line=base64_encode(serialize($MAIN));
	$sock->getFrameWork("greensql.php?syslog=$line");
	$filename=PROGRESS_DIR."/greensql.syslog";
...
";
	...
	if(count($data)>3){$_SESSION["greensql_SEARCH"]=$_GET["search"];}
	rsort($data);
	...
	$html[]="</tbody></table>";
	$html[]="<div><i>".@file_get_contents(PROGRESS_DIR."/greensql.syslog.pattern")."</i></div>";
