...
function page(){
	$sock=new sockets();
	$ArticaTechNetInfluxRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetNtopNGRepo")));
	$tpl=new templates();
	$version=$sock->GET_INFO("NTOPNG_VERSION");
	...
	while (list ($key, $array) = each ($ArticaTechNetInfluxRepo) ){
		...
		$button=button("{update2}","Loadjs('ntopng.update.engine.progress.php?key=$key&OS=$OS&filename=$FILENAME');",32);
