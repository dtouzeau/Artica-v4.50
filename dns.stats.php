...
function build_query_from_md5_js(){
	...
	if(isset($_GET["ipaddr"])){$IP1=$_GET["ipaddr"];}
...
function build_query_js(){
	...
	$domain=url_decode_special_tool($_GET["domain"]);
...
function requeteur_popup(){
	...
	<tr>
		<td $stylelegend class=legend>{domain}:</td>
		<td style='vertical-align:top;font-size:18px;'>". Field_text("domain-$t","*","font-size:18px;")."</td>
	</tr>														
...
function Run$t(){
	...
	var domain=encodeURIComponent(document.getElementById('domain-$t').value);
	
	var interval=document.getElementById('interval-$t').value;
	Loadjs('$page?query-js=yes&t=$t&container=graph-$t&ip1='+ip1+'&domain='+domain+'&date1='+date1+'&time1='+time1+'&date2='+date2+'&time2='+time2+'&interval='+interval);
...
function page(){
	...
	if($_GET["zmd5"]==null){
		$q=new mysql_squid_builder();
		$ligne=mysqli_fetch_array($q->QUERY_SQL("SELECT title,zmd5 FROM reports_cache WHERE report_type='DNS' ORDER BY zDate DESC LIMIT 0,1"));
	}else{
		$q=new mysql_squid_builder();
	...
	if($ligne["zmd5"]<>null){
		$nextFunction="LoadAjax('DNS_STATS_MAIN_GRAPH','$page?main=yes&zmd5={$ligne["zmd5"]}&t=$t');";
		$content=null;
	...
	$html="
	<div id='DNS_STATS_MAIN_GRAPH'>$content</div>
	<script>
...
//	SELECT SUM(ip1bytes) as ip1bytes, SUM(ip2bytes) as ip2bytes,ip1,ip2,ip2port,protocol,constartdate
	...
	$results=$q->QUERY_SQL("SELECT SUM(rqs) as rqs,zdate FROM \"$table\" GROUP BY zdate ORDER BY zdate");
	...
	$title=$tpl->_ENGINE_parse_body("{requests}");
	$timetext="{time}";
...
function graph2(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(rqs) as rqs,ipaddr FROM \"$table\" GROUP BY ipaddr ORDER BY rqs DESC LIMIT $ProxyGraphsTOPNumber");
	while($ligne=@pg_fetch_assoc($results)){
		$size=$ligne["rqs"];
		
		$FAMILYSITE=$ligne["ipaddr"];
	...
	$highcharts->Title=$tpl->_ENGINE_parse_body("TOP $ProxyGraphsTOPNumber {queries} {by_source_ip}");
	echo $highcharts->BuildChart();
	echo "\n";
...
function graph3(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(rqs) as rqs,familysite FROM \"$table\" GROUP BY familysite ORDER BY rqs DESC LIMIT $ProxyGraphsTOPNumber");
	while($ligne=@pg_fetch_assoc($results)){
		$size=$ligne["rqs"];
		$FAMILYSITE=$ligne["familysite"];
		$TOP_WEBSITES_SIZE[$FAMILYSITE]=$size;
	}
	$title=$tpl->_ENGINE_parse_body("TOP $ProxyGraphsTOPNumber {requests} {destination}");
	$PieData=$TOP_WEBSITES_SIZE;
	$highcharts=new highcharts();
	$highcharts->container="graph3-$zmd5";
	$highcharts->PieDatas=$PieData;
	$highcharts->ChartType="pie";
	$highcharts->PiePlotTitle="{destination}";
	$highcharts->Title=$title;
	echo $highcharts->BuildChart();
...
function graph4(){
	...
	$results=$q->QUERY_SQL("SELECT SUM(rqs) as rqs,category FROM \"$table\" GROUP BY category ORDER BY rqs DESC LIMIT $ProxyGraphsTOPNumber");
	while($ligne=@pg_fetch_assoc($results)){
		$size=$ligne["rqs"];
	...
	$highcharts->PieDatas=$PieData;
	$highcharts->ChartType="pie";
	$highcharts->PiePlotTitle="TOP $ProxyGraphsTOPNumber {categories}/{requests}";
	$highcharts->Title=$tpl->_ENGINE_parse_body("TOP $ProxyGraphsTOPNumber {categories}/{requests}");
	echo $highcharts->BuildChart();
...
function table4(){
	...
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{categories}</td>
			<th style='font-size:18px'>{requests}</td></tr>");
...
function table3(){
	...
	while($ligne=@pg_fetch_assoc($results)){
		...
		OnClick=\"Loadjs('$page?from-md5-js=$zmd5&domain=$site')\"
		style='font-size:18px;text-decoration:underline'>";
		
		
		$html[]="<tr><td style='font-size:18px;padding:8px'>$js$site</a></td>
			<td style='font-size:18px'>$size</td></tr>";
...
function table2(){
	...
	$html[]="<table style='width:100%'>";
	$html[]=$tpl->_ENGINE_parse_body("<tr><th style='font-size:18px;padding:8px'>{ipaddr}</td><th style='font-size:18px'>{requests}</td></tr>");
	$ProxyGraphsTOPNumber=intval($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ProxyGraphsTOPNumber"));
	if($ProxyGraphsTOPNumber==0){$ProxyGraphsTOPNumber=10;}
	...
	while($ligne=@pg_fetch_assoc($results)){
		...
		$js="<a href=\"javascript:blur()\"
				OnClick=\"Loadjs('$page?from-md5-js=$zmd5&ipaddr=$site')\"
				style='font-size:18px;text-decoration:underline'>";
