...
function search(){
    ...
    foreach ($Array as $k => $v){
        $choose=null;
        $funct="Select".md5($v["data"]["distinguishedName"]);
        $choose="<button class='btn btn-primary btn-xs' type='button' OnClick=\"$funct();\">$select</button>";
        $trid=md5($v["data"]["distinguishedName"]);
        $dn_dom=array();
        $dn_temp=strtolower($v["data"]["distinguishedName"]);
        ...
        $ValueToSend=$v["data"]["sAMAccountName"];
        if(isset($_GET["UseDN"])){$ValueToSend=$v["data"]["distinguishedName"];}
