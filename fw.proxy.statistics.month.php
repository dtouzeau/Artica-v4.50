...
function page(){
	...
	$q=new postgres_sql();
	$now=date("Y-m-d 00:00:00",strtotime('first day of this month', time()));
	...
	if(count($MAIN["xdata"])>1){
		$title=urlencode("{bandwidth}/{this_month}");
		$rowtitle=urlencode("| {bandwidth}");
		$data=serialize($MAIN);
		$timetext=urlencode("{day}");
		@file_put_contents("/usr/share/artica-postfix/ressources/logs/web/month.bandwidth.data", $data);
		$html[]="<tr><td valign='top' colspan=2><div id='month-bandwidth' style='with:1450;height:350px'></div></td></tr>";
		$js[]="Loadjs('$page?courbe=yes&container=month-bandwidth&timetext=$timetext&title=$title&data-file=month.bandwidth.data&legend=GB&row-title=$rowtitle');";
	}
	...
	if(count($MAIN["xdata"])>1){
		$data=serialize($MAIN);
		$title=urlencode("{requests}/{this_month}");
		$rowtitle=urlencode("| {requests}");
		$timetext=urlencode("{day}");
		@file_put_contents("/usr/share/artica-postfix/ressources/logs/web/month.requests.data", $data);
		$html[]="<tr><td valign='top' colspan=2><div id='month-requests' style='with:1450;height:350px'></div></td></tr>";
		$js[]="Loadjs('$page?courbe=yes&timetext=$timetext&container=month-requests&title=$title&data-file=month.requests.data&legend=rqs&row-title=$rowtitle');";
	}
	...
	if(count($PieData)>1){
		...
		@file_put_contents("/usr/share/artica-postfix/ressources/logs/web/month.familysite.data", $data);
		$html[]="<tr>
				<td valign='top'><div id='month-familysite' style='with:600px;height:500px'></div></td>
				<td valign='top' style='width:1%' nowrap>".@implode("\n", $table)."</td>
				</tr>";
		$js[]="Loadjs('$page?camembert=yes&container=month-familysite&data-file=month.familysite.data&title=top_visited_websites');";
	}
//---------------------------------------------------------------------------------------------------------------------------------
	//-------------------------------------------------------------------------------------------------------------
	$table=array();
	...
	if(count($PieData)>1){
		$data=serialize($PieData);
		@file_put_contents("/usr/share/artica-postfix/ressources/logs/web/month.member.data", $data);
		$html[]="<tr>
				<td valign='top'><div id='month-member' style='with:600px;height:500px'></div></td>
				<td valign='top' style='width:1%' nowrap>".@implode("\n", $table)."</td>
				</tr>";
		$js[]="Loadjs('$page?camembert=yes&container=month-member&data-file=month.member.data&title=top_members');";
	}
	//---------------------------------------------------------------------------------------------------------------------------------
