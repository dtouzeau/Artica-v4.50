...
function parse(){
    $TimeFile="/usr/share/artica-postfix/ressources/interface-cache/ADAGENT_DAY";
    $pidfile="/etc/artica-postfix/pids/exec.adagent.dashboard.php.pid";
    $unix=new unix();
    $sock=new sockets();
...
function adagent_courbe_day(){
    $now=date("Y-m-d H:i:s",strtotime("-24 hour"));
    $q=new postgres_sql();
    $sql="select sum(size) as size, date_trunc('hour', zdate) as zdate FROM adagent_log WHERE zdate > '$now' GROUP BY date_trunc('hour', zdate) ORDER BY zdate ASC;";
    ...
    $MAIN["xdata"]=$xdata;
    $MAIN["ydata"]=$ydata;
    @file_put_contents("{$GLOBALS["BASEDIR"]}/ADAGENT_DAY", serialize($MAIN));


}
function adagent_courbe_rqs_day(){
    $now=date("Y-m-d H:i:s",strtotime("-24 hour"));
    $q=new postgres_sql();
    $sql="select sum(rqs) as rqs, date_trunc('hour', zdate) as zdate FROM adagent_log WHERE zdate > '$now' GROUP BY date_trunc('hour', zdate) ORDER BY zdate ASC;";
    if($GLOBALS["VERBOSE"]){echo "\n*****\n$sql\n******\n";}
    $MAIN=array();
    $xdata=array();
    $ydata=array();
    ...
    $MAIN["xdata"]=$xdata;
    $MAIN["ydata"]=$ydata;
    @file_put_contents("{$GLOBALS["BASEDIR"]}/ADAGENT_RQS_DAY", serialize($MAIN));
    if(count($xdata)<2){@unlink("{$GLOBALS["BASEDIR"]}/ADAGENT_RQS_DAY");}
