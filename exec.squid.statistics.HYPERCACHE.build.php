...
function GRAB_DATAS($ligne,$md5){
	...
	$SQLA[]="SELECT SUM(size) as size, SUM(rqs) as rqs,$TimeGroup,familysite,category,cached FROM hypercache_access";
	$SQLA[]="WHERE";
	$SQLA[]="$SEARCHTEXT (zdate >'".date("Y-m-d H:i:s",$from)."' and zdate < '".date("Y-m-d H:i:s",$to)."')";
	if($USER_FIELD=="USERID"){$SQLA[]="AND USERID != 'none'";}
	$SQLA[]="GROUP BY date_trunc('hour', zdate),familysite,category,cached ORDER BY zdate";
	
	build_progress("{step} {waiting_data}: BigData engine, (HyperCache) {please_wait}",6);
	$unix=new unix();
	$hostname=$unix->hostname_g();
	
	
	$sql="CREATE TABLE IF NOT EXISTS \"{$md5}report\" (zDate timestamp,familysite VARCHAR(128) NULL,category VARCHAR(90) NULL,size BIGINT,rqs BIGINT,cached int)";
	...
	$q->QUERY_SQL("create index cached{$md5}report on \"{$md5}report\"(cached);");
	...
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (size,rqs,zdate,familysite,category,cached) $sql";
