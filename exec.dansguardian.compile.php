...
shell_exec($unix->LOCATE_PHP5_BIN()."/usr/share/artica-postfix/exec.squidguard.php --dansguardian{$GLOBALS["cmdlineadd"]}");
exit();
...
if($argv[1]=="--missing-groups"){FixMissingGroupsFiles();FixMissingFiles();exit();}
...
$dans->SaveSettings();
$cmd=LOCATE_PHP5_BIN2()."/usr/share/artica-postfix/exec.web-community-filter.php --patterns{$GLOBALS["cmdlineadd"]}";
events("MAIN:: $cmd");
system($cmd);
...
function BuildPersonalCategories(){
	...
	@mkdir("/etc/dansguardian/lists/personal-blacklists",0755,true);
	...
	while (list ($num, $array2) = each ($array) ){
		...
		@mkdir("/etc/dansguardian/lists/personal-blacklists/$num",0755,true);
