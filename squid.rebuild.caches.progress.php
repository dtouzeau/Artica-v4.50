...
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/squid.rebuild.caches.progress";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.rebuild.caches.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->_ENGINE_parse_body("{reconstruct_caches}");
}
...
function suffix(){
	...
	$onlySquid=null;
	$ApplyConfToo=null;
	return "&table={$_GET["table"]}&database={$_GET["database"]}";
}
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$warn_text=$tpl->javascript_parse_text("{reconstruct_caches_ask}");
	$warn="if(!confirm('$warn_text')){return;}";
	$suffix=suffix();
	$title=title();
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		if(document.getElementById('meta-start')){refreshTab('meta-start');}
...
function Launch(){
	$sock=new sockets();
	$suffix=suffix();
	$cmd="squid.php?rebuild-caches=yes";
