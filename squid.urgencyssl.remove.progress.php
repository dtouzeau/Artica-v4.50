...
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.urgency.disable.progress.txt";
...
function title(){
	if(isset($_GET["ufdb"])){
		$tpl=new templates();
		$title=$tpl->_ENGINE_parse_body("{proxy_in_webfiltering_emergency_mode}::{disable_emergency_mode}");
		return $title;
		
	}
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{proxy_in_ssl_emergency_mode}::{disable_emergency_mode}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?disable-ssl-urgency=yes";
	if(isset($_GET["ufdb"])){
		$cmd="squid2.php?disable-ufdb-urgency=yes";
	}
