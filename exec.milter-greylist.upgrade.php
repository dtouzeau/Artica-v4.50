...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetHaProxyRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetMGreyListRepo")));
	...
	if($GLOBALS["PROGRESS"]){
		...
		system("$php /usr/share/artica-postfix/exec.status.php --milter-greylist >/dev/null");
		system("/etc/init.d/artica-status restart --force");
		build_progress_idb("{restarting_service}",98);
		system("/etc/init.d/milter-greylist restart");
		system("/etc/init.d/milter-greyweb restart");
	}
