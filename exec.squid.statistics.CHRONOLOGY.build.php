...
function GRAB_DATAS($ligne,$md5){
	...
	$SQLA[]="SELECT SUM(size) as size, SUM(rqs) as rqs,$TimeGroup,familysite FROM access_log";
	$SQLA[]="WHERE";
	$SQLA[]="$SEARCHTEXT (zdate >'".date("Y-m-d H:i:s",$from)."' and zdate < '".date("Y-m-d H:i:s",$to)."')";
	if($USER_FIELD=="USERID"){$SQLA[]="AND USERID != 'none'";}
	$SQLA[]="GROUP BY date_trunc('hour', zdate), familysite ORDER BY zdate";
	
	build_progress("{step} {waiting_data}: BigData engine, (Chronology) {please_wait}",6);
	$unix=new unix();
	$hostname=$unix->hostname_g();
	
	
	$sql="CREATE TABLE IF NOT EXISTS \"{$md5}report\" (zDate timestamp,FAMILYSITE VARCHAR(128) NULL,SIZE BIGINT,RQS BIGINT)";
	...
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (size,rqs,zdate,familysite) $sql";
