...
function build_progress($text,$pourc):bool{
	$unix=new unix();
    $unix->framework_progress($pourc,$text,"htopweb.progress");
    return true;
}
function build_progress_restart($text,$pourc):bool{
    $unix=new unix();
    $unix->framework_progress($pourc,$text,"htopweb.restart");
    return true;
}
...
function install_service():bool{
	...
	create_htopweb_service();
	$GLOBALS["CLASS_SOCKETS"]->SET_INFO("EnableWebHTOP", 1);

	build_progress("{starting_service} {APP_SHELLINABOX}",40);
	if(!start(true)){
		build_progress("{starting_service} {APP_SHELLINABOX} {failed}",110);
		$GLOBALS["CLASS_SOCKETS"]->SET_INFO("EnableWebHTOP", 0);
		remove_service("/etc/init.d/htopweb");
		return false;
	}
...
function install_monit():bool{
    $unix=new unix();
	@unlink("/etc/monit/conf.d/APP_HTOPWEB.monitrc");
	$f[]="check process APP_HTOPWEB with pidfile /var/run/htopweb.pid";
	$f[]="\tstart program = \"/etc/init.d/htopweb start\"";
	$f[]="\tstop program = \"/etc/init.d/htopweb stop\"";
	$f[]="\tif failed unixsocket /var/run/htopweb.sock then restart";

	$f[]="";
	@file_put_contents("/etc/monit/conf.d/APP_HTOPWEB.monitrc", @implode("\n", $f));
    $unix->reload_monit();
    return true;
}
...
function create_htopweb_service(){
	$unix=new unix();
	...
	echo "SHELL IN A BOX: [INFO] Writing /etc/init.d/htopweb with new config\n";
	@unlink("/etc/init.d/shellinabox");
	@file_put_contents("/etc/init.d/htopweb", @implode("\n", $f));
	
	
	@chmod("/etc/init.d/htopweb",0755);
...
function CheckWebConsole():bool{
    ...
    foreach ($f as $line){
        if(preg_match("#APP_HTOPWEB#",$line)){
            return true;
        }
    }
...
function start($aspid=false):bool{
	...
	if($unix->process_exists($pid)){
		...
		@file_put_contents("/var/run/htopweb.pid", $pid);
		return true;
	}
    ...
    $htop=$unix->find_program("htop");
    CheckWebConsole();
	$t[]="$shellinaboxd";
	$t[]="--background=/var/run/htopweb.pid";
	$t[]="--disable-ssl";
	$t[]="--numeric";
	$t[]="--localhost-only";
	$t[]="--user=0";
	$t[]="--group=0";
	$t[]="--no-beep --service='/:nobody:nogroup:/:$htop -d 10'";
	$t[]="--unixdomain-only=/var/run/htopweb.sock:www-data:www-data:0755";
	$cmd=@implode(" ", $t);
...
function PID_NUM():int{
	$unix=new unix();
	$pidfile="/var/run/htopweb.pid";
	$pid=$unix->get_pid_from_file($pidfile);
	if($unix->process_exists($pid)){return $pid;}
	return $unix->PIDOF_PATTERN("shellinaboxd.*?htopweb.sock");
