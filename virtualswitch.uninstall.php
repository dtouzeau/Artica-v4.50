...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/virtualswitch.uninstall.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/virtualswitch.uninstall.progress.txt";
...
function js(){
	...
	$switch=$_GET["switch"];
	$title=$tpl->javascript_parse_text("{uninstall} $switch");
	$t=time();
	$ask=null;
	$vde_remove_ask=$tpl->javascript_parse_text("{vde_remove_ask}");
	if($_GET["switch"]<>null){
		$ask="if(!confirm('$switch: $vde_remove_ask  ?')){return;}";
	...
	function Start$t(){
		$ask	
		YahooWinBrowseHide();		
		RTMMail('800','$page?popup=yes&switch={$_GET["switch"]}&OS={$_GET["OS"]}&filename=".urlencode($_GET["filename"])."','$title');
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file={$_GET["md5file"]}&switch={$_GET["switch"]}&OS={$_GET["OS"]}&filename=".urlencode($_GET["filename"])."');
}
...
if($md5file<>$_GET["md5file"]){
	...
	var xStart$time= function (obj) {
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&switch={$_GET["switch"]}&OS={$_GET["OS"]}&filename=".urlencode($_GET["filename"])."');
	}		
	...
	function Start$time(){
		...
		XHR.appendData('switch','".urlencode($_GET["switch"])."');
		XHR.appendData('t', '$t');
		XHR.setLockOff();
...
function popup(){
	...
	$sock=new sockets();
	$restart=null;
	$sock->getFrameWork("vde.php?switch-uninstall={$_GET["switch"]}");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{uninstall}...<br>{$_GET["switch"]}");
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0&switch={$_GET["switch"]}&OS={$_GET["OS"]}&filename=".urlencode($_GET["filename"])."');
}
$('#progress-$t').progressbar({ value: 1 });
