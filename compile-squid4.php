...
if(!$GLOBALS["NO_COMPILE"]){
	...
	if(!is_file("/root/$v")){
		...
		echo "Downloading $v ...\n";
		shell_exec("$wget http://www.squid-cache.org/Versions/v4/$v");
		if(!is_file("/root/$v")){
...
function latests(){
	$unix=new unix();
	$wget=$unix->find_program("wget");
	echo "Downloading latests index file Versions/v4/\n";
	@unlink("/tmp/index.html");
	shell_exec("$wget http://www.squid-cache.org/Versions/v4/ -O /tmp/index.html");
...
function CONFIGURE_SQUID(){
	...
	$CFLAGS[]="CFLAGS=\"-g -O2 -fPIE -fstack-protector -Dbind=SOCKSbind -DNUMTHREADS=256 --param=ssp-buffer-size=4 -Wformat -Werror=format-security -Wall\"";
	$CFLAGS[]="CXXFLAGS=\"-g -O2 -fPIE -DNUMTHREADS=256  -Dbind=SOCKSbind -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security\"";
