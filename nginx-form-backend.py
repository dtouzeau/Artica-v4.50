...
import os, signal, base64, Cookie, cgi, urlparse
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def auth_form(self, target = None):
       ...
       # for key in self.headers:
        #    print key, 'corresponds to', self.headers[key]
        
        if target == None:
            target = self.headers.get('X-Target')
            
        Formpath=self.headers.get('X-TemplatePath')
        try:
            html = open(Formpath, 'r').read()
        except:
             self.send_response(500)
             self.end_headers()
             self.log_error('Formpath: Error reading file: '+str(Formpath))
             self.wfile.write('Formpath: Error reading file '+str(Formpath)+'\n')
             return
