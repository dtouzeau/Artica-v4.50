...
function GRAB_DATAS($ligne,$md5){
	...
	$md5_table=md5(__FUNCTION__."."."$from$to$IP1$domain");
	...
	if($IP1<>null){
		...
		if(preg_match("#[0-9\.]+\/[0-9]+#", $IP1)){
			$SSEARCH[]=" ( inet '$IP1' >> ipaddr ) ";
		}
		if(preg_match("#^[0-9\.]+$#", $IP1)){
			$SSEARCH[]=" ( inet '$IP1' {$operator} ipaddr ) ";
		}
	}	
	if($domain<>null){
		if(strpos("*", " $domain")>0){
			$searchsites_sql=str_replace("*", ".*", $domain);
			$SSEARCH[]="familysite ~* '$searchsites_sql'";
		}else{
			$SSEARCH[]="familysite = '$domain'";
		}
	}
	...
	$SQLA[]="SELECT SUM(rqs) as rqs,ipaddr,familysite,category,";
	
	if($interval=="10m"){
		$SQLA[]="to_timestamp(floor((extract('epoch' from zdate) / 600 )) * 600)  AT TIME ZONE 'UTC' as zdate";
	}
	
	if($interval=="1h"){
		$SQLA[]="date_trunc('hour', zdate) as zdate";
	}
	
	$table_source="dns_access";
	$FromTime=date("Y-m-d H:i:s",$from);
	$ToTime=date("Y-m-d H:i:s",$to);
	
	if($interval=="1d"){
		$table_source="dns_access_days";
		$FromTime=date("Y-m-d 00:00:00",$from);
		$ToTime=date("Y-m-d 00:00:00",$to);
	}
	$SQLA[]="FROM $table_source";
	$SQLA[]="WHERE";
	$SQLA[]="$SEARCHTEXT (zdate >='$FromTime' and zdate <= '$ToTime')";

	$SQLA[]="GROUP BY ipaddr,familysite,category,";
	
	if($interval=="10m"){
		$SQLA[]="to_timestamp(floor((extract('epoch' from zdate) / 600 )) * 600)  AT TIME ZONE 'UTC'";
	}
	
	if($interval=="1h"){
		$SQLA[]="date_trunc('hour', zdate)";
	}
	...
	build_progress("{step} {waiting_data}: BigData engine, (DNS) {please_wait}",6);
	$unix=new unix();
	...
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (rqs,ipaddr,familysite,category,zdate) $sql";
