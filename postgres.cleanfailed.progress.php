...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/postgres.cleanfailed.progress";
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/postgres.cleanfailed.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{clean_failed_queue}");
	echo "
	YahooWinBrowseHide();		
...
function popup(){
	...
	$restart=null;
	$addon=null;
	$sock->getFrameWork("postgres.php?clean-failed-queue=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{clean_failed_queue}...");
