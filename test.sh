...
wsrep_recover_position() {
  ...
  local wr_pidfile="$DATADIR/"`uname -n`"-recover.pid"
...
safe_mysql_unix_port=${mysql_unix_port:-${MYSQL_UNIX_PORT:-/run/mysqld/mysqld.sock}}
...
then
  ...
  then
    if ps wwwp $PID | grep -v mariadbd-safe | grep -v mysqld_safe | grep -- $MYSQLD > /dev/null
