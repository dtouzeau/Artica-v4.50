...
function query_group(){
	...
	if(count($hash)==0){json_error_show("no data");}
	
	foreach ($hash as $dn=>$Displayname){
		...
		if($_GET["callback"]<>null){$js="{$_GET["callback"]}('$dn','$prepend','$Displayname')";}
		...
		$data['rows'][] = array(
				...
				'cell' => array(
						"<img src='img/$img'>",
						"<span style='font-size:16px;font-weight:bolder'>$Displayname</span>",
						"<span style='font-size:16px'>".imgsimple("arrow-right-24.png","{add}",$js)."</span>",
				)
		);
