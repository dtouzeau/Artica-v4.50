...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/postgres.remove.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/web/postgres.remove.progress.txt";
...
function js(){
	...
	$ask=$tpl->javascript_parse_text("{rebuild_database_warn}");
	echo "
	if(confirm('$ask')){
		YahooWinBrowseHide();		
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			LoadAjaxRound('main-statistics-options','squid.statistics.options.php');
...
function popup(){
	...
	$sock->getFrameWork("postgres.php?remove-database=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{REMOVE_DATABASE}...");
