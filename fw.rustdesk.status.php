...
function page():bool{
    ...
    $html=$tpl->page_header("{APP_RUSTDESK} $APP_RUSTDESK_VERSION &raquo;&raquo; {status}",
        ico_laptop_down,
        "{APP_RUSTDESK_EXPLAIN}",
        "$page?table=yes","rustdesk-status","progress-rustdesk-restart",false,"table-rustdesk-status");
...
function table():bool{
    ...
    $UinstallJS=$tpl->framework_buildjs("/rustdesk/uninstall",
        "rustdesk.install.progress","rustdesk.restart.progress.log",
        "progress-rustdesk-restart","document.location.href='/index'",null,null,"AsSystemAdministrator");

    $topbuttons[] = array($UinstallJS, ico_trash, "{uninstall}","AsSystemAdministrator");
    $TINY_ARRAY["TITLE"]="{APP_RUSTDESK} $APP_RUSTDESK_VERSION &raquo;&raquo; {status}";
    $TINY_ARRAY["ICO"]=ico_laptop_down;
    $TINY_ARRAY["EXPL"]="{APP_RUSTDESK_EXPLAIN}";
	...
	<tr>
		...
		<td valign='top' style='width:98%;padding-left:15px'>
		    ...
		    <div id='rustdesk-config-locked'></div>
	...
	<script>
        ...
        LoadAjaxTiny('rustdesk-config-locked','$page?config-locked=yes&t=$t');		
	</script>
...
function section_change_key_ask():bool{
    ...
    $js[] = $tpl->framework_buildjs("rustdesk.php?change-key-perform=yes",
        ...
        "progress-rustdesk-restart",
        "LoadAjaxTiny('$t-toolbox','$page?service-toolbox=yes');LoadAjaxTiny('rustdesk-config-locked','$page?config-locked=yes&t=$t');", null, null, "AsFirewallManager");
...
function section_crypted_popup():bool{
    ...
    $jsafter=$tpl->framework_buildjs("rustdesk.php?restart=yes",
        ...
        "progress-rustdesk-restart","LoadAjaxTiny('$t-status','$page?service-status=$t');",null,null,"AsFirewallManager");
