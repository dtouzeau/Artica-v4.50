...
function js(){
	...
	$tpl=new template_admin();
	$tpl->js_dialog6("{system}::proftpd-mod-sqlite", "$page?popup=yes",650);
}

function popup(){
	$ARRAY["PROGRESS_FILE"]=PROGRESS_DIR."/php7install.progress";
	$ARRAY["LOG_FILE"]=PROGRESS_DIR."/php7install.log";
	$ARRAY["CMD"]="services.php?apt-get-install=proftpd-mod-sqlite";
	$ARRAY["TITLE"]="{system} {installing} proftpd-mod-sqlite";
