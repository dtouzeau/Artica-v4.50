...
function tabs_multi(){
    ...
    $adagent=new ADAgent();
    $adforest=$adagent->getForest();

    $data = json_decode($adforest, TRUE);
    ...
    foreach ($data as $cnxids=>$MAIN){
        $LDAP_SUFFIX=$MAIN["dn"];
        $array[$LDAP_SUFFIX]="$page?form-multi=yes&connection-id=$cnxids&dn=$LDAP_SUFFIX";
...
";
    ...
    function $function(){
        ...
        LoadAjax('table-loader-$connection_id-members','$page?table=yes&t=$t&search='+ss+'&type='+type+'&function=$function&connection-id=$connection_id&dn={$_GET["dn"]}');
    }
    $function();";
    $html[]="</script>";
...
function page_multi(){
    ...
    $html=$tpl->page_header("{my_members}: Active Directory",
        ...
        "ad-agent-members","progress-firehol-restart",false,"mytabsadusers");
...
function page(){
    ...
    if($EnableExternalACLADAgent==1){
        VERBOSE("EnableExternalACLADAgent = $EnableExternalACLADAgent",__LINE__);
        $c++;
    }
    ...
    $html=$tpl->page_header("{my_members}: Active Directory",
        ...
        "ad-agent-members","progress-firehol-restart",false);
...
function table_ad_groups():bool{
    ...
    $adagent=new ADAgent();
    $adgroups=$adagent->getDNGroups("$encodedval","$stringtofind");
    $data = json_decode($adgroups, TRUE);
...
//    $MAIN_HASH=$ad->AllGroups($stringtofind,50);
    ...
    $c=0;
    foreach ($data as $FirstDN=>$ligne){
              $MembersCount = 0;
        $ysuffix = strtolower($LDAP_SUFFIX);
        ...
        $suffix = @implode(".", $zsuf);
        $description="&nbsp;";
        $displayname = $ligne["data"]["sAMAccountName"];
        if(isset($ligne["data"]["member"])) {
            if (is_array($ligne["data"]["member"])){
            $MembersCount = count($ligne["data"]["member"]);
            }
        }

        if(isset($ligne["data"]["description"])){
            $description=$ligne["data"]["description"].'<br>';
        }

        $dn = urlencode($ligne["data"]["distinguishedName"]);
        if ($TRCLASS == "footable-odd") {
            $TRCLASS = null;
        } else {
            $TRCLASS = "footable-odd";
        }
        $js = "Loadjs('fw.groups.ad.agent.php?dn=$dn&group_name=$displayname')";
...
function table():bool{
    ...
    $encodedval=urlencode($dn);
    $adagent=new ADAgent();
    $admembers=$adagent->getDNMembers("$encodedval","$stringtofind");

    $MAIN_HASH = json_decode($admembers, TRUE);
;
...
//	print_r($hash_full);
    ...
    foreach ($MAIN_HASH as $FirstDN=>$ligne){
        ...
        $description=null;
        $displayname=null;
        if(isset($ligne["data"]["description"])){$description="<br><small>{$ligne["data"]["description"]}</small>";}
        if($description==null){
            if(isset($ligne["data"]["displayName"])){$description="<br><small>{$ligne["data"]["displayName"]}</small>";}
        }


        $objectclass=$ligne["data"]["objectClass"];
        ...
        if(!isset($ligne["data"]["sAMAccountName"])){continue;}
        if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
        ...
        if(isset($ligne["data"]["sAMAccountName"])){$uid=$ligne["data"]["sAMAccountName"];}
        if(isset($ligne["data"]["userPrincipalName"])){$email_address[]="<div>{$ligne["data"]["userPrincipalName"]}</div>";}
        if(isset($ligne["data"]["telephoneNumber"][0])){$telephonenumber[]="<div>{$ligne["data"]["telephoneNumber"]}</div>";}
        if(isset($ligne["data"]["mobile"])){$telephonenumber[]="<div>{$ligne["data"]["mobile"]}</div>";}


        if(isset($ligne["data"]["givenName"])){$givenname=$ligne["data"]["givenName"];}
        if(isset($ligne["data"]["sn"])){$sn=$ligne["data"]["sn"];}

        if($givenname<>null){if($sn<>null){ $displayname=" $givenname $sn"; }}


        $uid=$ligne["data"]["sAMAccountName"];
        if(isset($ligne["data"]["mail"])){$email_address="{$ligne["data"]["mail"]}";}



        if($displayname==null){$displayname=trim($ligne["data"]["displayName"]);}
        if($displayname==null){$displayname=$uid;}
        $uidenc=urlencode($uid);
        $js="Loadjs('fw.member.ad.agent.edit.php?dn=".urlencode($ligne["data"]["distinguishedName"])."&name=$displayname')";
    ...
    //
    $table_buttons=table_buttons();
    $TINY_ARRAY["TITLE"]="{my_members}:&nbsp;$dn";
	...
	<script>
	NoSpinner();\n".@implode("\n",$tpl->ICON_SCRIPTS)."
	$(document).ready(function() { $('#table-my-computers').footable({ \"filtering\": { \"enabled\": true },\"sorting\": {\"enabled\": true } } ); });
$jstiny
{$table_buttons[1]}
