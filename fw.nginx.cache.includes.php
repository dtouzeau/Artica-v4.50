...
function rule_remove():bool{
    ...
    $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"delete","table"=>"enforce_cache","where"=>array("ID"=>"$ID")));
...
function rule_enable():bool{
    ...
    if(intval($ligne["enabled"])==1){
       $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"update","table"=>"enforce_cache","set"=>array("enabled"=>"0"),"where"=>array("ID"=>"$ID")));
    }else{
        $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"update","table"=>"enforce_cache","set"=>array("enabled"=>"1"),"where"=>array("ID"=>"$ID")));
    }
    $get_servicename=get_servicename($serviceid);
    echo refresh_global_no_close($serviceid);
...
function rule_save():bool{
    ...
    $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"insert","table"=>"enforce_cache","values"=>array("serviceid"=>"$serviceid","pattern"=>"$pattern","regex"=>"$regex")));

    }else{
        $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"update","table"=>"enforce_cache","set"=>array("pattern"=>"$pattern","regex"=>"$regex"),"where"=>array("ID"=>"$ruleid")));
    }
...
function rule_disable_all():bool{
    ...
    $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"update","table"=>"enforce_cache","set"=>array("enabled"=>"0"),"where"=>array("serviceid"=>"$serviceid")));
    $get_servicename=get_servicename($serviceid);
    echo refresh_global_no_close($serviceid);
    echo "$function();";
...
function rule_enable_all():bool{
    $function=$_GET["function"];
    $serviceid=$_GET["serviceid"];
    $GLOBALS["CLASS_SOCKETS"]->REST_API_NGINX_POST_JSON("/nginx-db/exec", array("action"=>"update","table"=>"enforce_cache","set"=>array("enabled"=>"1"),"where"=>array("serviceid"=>"$serviceid")));
    $get_servicename=get_servicename($serviceid);
    echo refresh_global_no_close($serviceid);
    echo "$function();";
