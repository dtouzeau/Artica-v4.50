...
function doh_connect_https($dnsquery)
{
    ...
    switch($argv[1]) {
        ...
        case "toto":
            echo "https://doh.touzeau.biz/dns-query\n";
            curl_setopt($ch, CURLOPT_URL, "https://doh.touzeau.biz/dns-query"); // support POST
            curl_setopt($ch, CURLOPT_POSTFIELDS, base64_decode($dnsquery));
            break;

        case "experimental-post":
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false); // true (or 1) removed in curl 7.28.1
    ...
    $error=curl_errno($ch);
    if($error>0){
        if($GLOBALS["VERBOSE"]){echo "doh_connect_https: Error $error\n";}
    }
   ...
   // $Infos= curl_getinfo($ch);
    ...
    if($output === FALSE) {
        if($GLOBALS["VERBOSE"]){echo "doh_connect_https: OUTPUT == FALSE\n";}
        return(null);
    }
...
function doh_read_dnsanswer($raw, $requesttype)
{
    ...
    if($GLOBALS["VERBOSE"]){echo "doh_read_dnsanswer: ....\n";}

    $rawtype = doh_get_qtypes($requesttype);
    if($GLOBALS["VERBOSE"]){echo "doh_read_dnsanswer: rawtype = '$rawtype'\n";}

    /* Getting header. */
    $qst_header = unpack("nid/nspec/nqdcount/nancount/nnscount/narcount", substr($raw, $raw_counter, 12));
    if($GLOBALS["VERBOSE"]){echo "doh_read_dnsanswer: qst_header: {$qst_header['ancount']}\n";}
    ...
    if($GLOBALS["VERBOSE"]){echo "doh_read_dnsanswer: domainresp = '$domainresp'\n";}
