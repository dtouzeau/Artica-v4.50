...
function GRAB_DATAS($ligne,$md5){
	...
	echo "SMTP Refused: FROM $from to $to $interval organization:$ou user:$user $search\n";
	$table="smtprefused";
	...
	if($ou<>null){
		...
		$sqlSource="(select count(*) as hits,mailfrom,mailto,reason,$TimeGroup FROM smtprefused WHERE $FilterDate AND (($imploded1) OR ($imploded2)) GROUP BY mailfrom,mailto,reason,$TimeGroupBy ORDER BY $TimeGroupBy ) as t";
		$sqlSource="select * FROM $sqlSource $SSEARCH";
		
	}
	
	if($sqlSource==null){
		$sqlSource="(select count(*) as hits,mailfrom,mailto,reason,$TimeGroup FROM smtprefused WHERE $FilterDate GROUP BY mailfrom,mailto,reason,$TimeGroupBy ORDER BY $TimeGroupBy ) as t";
		$sqlSource="select * FROM $sqlSource $SSEARCH";
	}
	...
	$sql="INSERT INTO \"{$md5}report\" (hits,mailfrom,mailto,reason,zdate) $sqlSource";
