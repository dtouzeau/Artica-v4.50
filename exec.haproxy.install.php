...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetHaProxyRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetHaProxyRepo")));
	...
	if(is_file("/etc/init.d/haproxy")){system("/etc/init.d/haproxy stop");}
