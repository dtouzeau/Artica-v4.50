...
function js(){
	...
	$tpl=new template_admin();
	$tpl->js_dialog6("{system}::python-memcache", "$page?popup=yes",650);
}

function popup(){
	$ARRAY["PROGRESS_FILE"]=PROGRESS_DIR."/php7install.progress";
	$ARRAY["LOG_FILE"]=PROGRESS_DIR."/php7install.log";
	$ARRAY["CMD"]="services.php?apt-get-install=python3-memcache";
	$ARRAY["TITLE"]="{system} {installing} python3-memcache";
