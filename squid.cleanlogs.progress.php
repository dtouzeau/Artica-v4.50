...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/squid.cleanlogs.progress";
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/squid.cleanlogs.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{clean_log_directory}");
	echo "
	YahooWinBrowseHide();		
	YahooWin5Hide();
...
function popup(){
	...
	$sock->getFrameWork("squid2.php?clean-logs-emergency=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{clean_log_directory}...");
