...
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid_reload.txt";
...
if($prc>=100){
	...
	function Start$time(){
		if(!RTMMailOpen()){return;}
			...
			LayersTabsAllAfter();
			RTMMailHide();
			if(document.getElementById('proxy-store-caches')){ LoadAjaxRound('proxy-store-caches','admin.dashboard.proxy.caches.php'); }
			if(document.getElementById('CACHE_CENTER_TABLEAU')){ $('#CACHE_CENTER_TABLEAU').flexReload(); }
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
