...
if(isset($_GET["CountOfStreamAccessModule"])){CountOfStreamAccessModule();exit;}
js();

function js(){
    $page       = CurrentPageName();
    $tpl        = new template_admin();
    $tpl->js_dialog4("{limit_access}","$page?table-start=yes",850);
}
function CountOfStreamAccessModule(){
    $tpl        = new template_admin();
    $q          = new lib_sqlite("/home/artica/SQLITE/webconsole.db"); // sqlite_num_rows

    $ngx_stream_access_module=$q->COUNT_ROWS("ngx_stream_access_module")." {items}";
    $ngx_stream_access_module=$tpl->_ENGINE_parse_body($ngx_stream_access_module);
    $ngx_stream_access_module=str_replace("'","\'",$ngx_stream_access_module);

    $f[]="if(document.getElementById('CountOfStreamAccessModule') ){";
    $f[]="\tdocument.getElementById('CountOfStreamAccessModule').innerHTML='$ngx_stream_access_module'";
    $f[]="}";
...
function container_move(){
    ...
    $GLOBALS["CLASS_SOCKETS"]->REST_API("/webconsole/reload");
...
function table_start(){
	$page       = CurrentPageName();
	$ID         = "9546897";
	echo "<div id='ngx_stream_access_module-$ID'></div>
...
function delete_js(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT `item` FROM ngx_stream_access_module WHERE ID=$ID");
	$tpl->js_confirm_delete("{$ligne["item"]}", "delete", "$ID","$('#$md5').remove();Loadjs('$page?CountOfStreamAccessModule=yes');");
}
function  delete(){
	$page=CurrentPageName();
	$tpl=new template_admin();
...
function id_js(){
	...
	$tpl->js_dialog5($title, "$page?id-popup=$ID&serviceid=$serviceid&md5=$md5");
}
function id_popup(){
	$page=CurrentPageName();
	$tpl=new template_admin();
	...
	$js="dialogInstance5.close();LoadAjax('ngx_stream_access_module-$serviceid','$page?table=$serviceid');Loadjs('$page?CountOfStreamAccessModule=yes');";
...
function id_save(){
	...
	if($ID==0){
		$q->QUERY_SQL("INSERT OR IGNORE INTO ngx_stream_access_module(serviceid,item,allow) VALUES ($serviceid,'$item',$allow)");
		if(!$q->ok){echo $q->mysql_error;return;}
        $GLOBALS["CLASS_SOCKETS"]->REST_API("/webconsole/reload");
		return;
	}
...
function table(){
	...
	$html[]="<label class=\"btn btn btn-primary\" OnClick=\"Loadjs('$page?id-js=0&serviceid={$_GET["table"]}&md5=');\"><i class='fa fa-plus'></i> {new_rule} </label>";
	$html[]="</div>";
	...
	$results=$q->QUERY_SQL("SELECT * FROM ngx_stream_access_module WHERE serviceid='{$_GET["table"]}' ORDER BY zorder");
	...
	foreach ($results as $md5=>$ligne){
		...
		$html[]="<td style='width:1%'><center>". $tpl->icon_delete("Loadjs('$page?delete=$ID&md5=$md5')","AsSystemWebMaster")."</center></td>";
		$html[]="</tr>";
	}
	...
	<script>
	NoSpinner();\n".@implode("\n",$tpl->ICON_SCRIPTS)."
	$(document).ready(function() { $('#table-ngx_stream_access_module-{$_GET["table"]}').footable( { \"filtering\": { \"enabled\": true }, \"sorting\": { \"enabled\": true },\"paging\": { \"size\": {$GLOBALS["FOOTABLE_PSIZE"]} } } ); });
	</script>";
