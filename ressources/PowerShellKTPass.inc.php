...
function BuildPowerShellKTPass($KerberosRealm="EXAMPLE.COM",$ServiceHostname="myhost.example.com",$MapUser="svc_account@EXAMPLE.COM"):string{
    ...
    $f[]="";
    $f[]="        if (\$owners.Count -eq 0) {";
    $f[]="            # Fallback: also parse lines containing CN= and DC= (DN without header prefix)";
    $f[]="            foreach (\$line in \$qOut) {";
    $f[]="                if (\$line -match '^\s*(CN=[^,]+,.+DC=.+?)\s*\$' -and \$line -notmatch 'Checking domain') {";
    $f[]="                    \$acc = Resolve-DNToSetspnAccount -DnOrLine \$Matches[1]";
    $f[]="                    if (\$acc -and \$acc -ne \$AccountForSetspn) { \$owners += \$acc }";
    $f[]="                }";
    $f[]="            }";
    $f[]="            \$owners = \$owners | Select-Object -Unique";
    ...
    $f[]="";
    $f[]="        if (\$owners.Count -eq 0) {";
    $f[]="            # Fallback: LDAP search for the SPN owner via AD module";
    $f[]="            try {";
    $f[]="                Import-Module ActiveDirectory -ErrorAction Stop";
    $f[]="                \$adResults = Get-ADObject -LDAPFilter \"(servicePrincipalName=\$spn)\" -Properties sAMAccountName,distinguishedName";
    $f[]="                foreach (\$obj in \$adResults) {";
    $f[]="                    if (\$obj.sAMAccountName) {";
    $f[]="                        try { \$dom = Get-ADDomain -Identity \$obj.distinguishedName } catch { \$dom = Get-ADDomain }";
    $f[]="                        \$resolved = if (\$dom.NetBIOSName) { \"\$(\$dom.NetBIOSName)\\\$(\$obj.sAMAccountName)\" } else { \$obj.sAMAccountName }";
    $f[]="                        if (\$resolved -ne \$AccountForSetspn) { \$owners += \$resolved }";
    $f[]="                    }";
    $f[]="                }";
    $f[]="                \$owners = \$owners | Select-Object -Unique";
    $f[]="            } catch {";
    $f[]="                Write-Warning \"AD module lookup failed: \$_\"";
    $f[]="            }";
    $f[]="        }";
    $f[]="";
    ...
    $f[]="            }";
    $f[]="        } else {";
    $f[]="            Write-Warning \"SPN appears registered elsewhere, but no owners could be found. Attempting force add...\"";
    $f[]="        }";
    $f[]="    }";
    $f[]="";
    ...
    $f[]="    if (\$LASTEXITCODE -ne 0) {";
    $f[]="        # -S failed (duplicate detected). Try removing via LDAP then retry.";
    $f[]="        Write-Warning \"setspn -S failed (duplicate). Attempting LDAP-based SPN removal and retry...\"";
    $f[]="        \$fixed = \$false";
    $f[]="        try {";
    $f[]="            Import-Module ActiveDirectory -ErrorAction Stop";
    $f[]="            \$conflicts = Get-ADObject -LDAPFilter \"(servicePrincipalName=\$spn)\" -Properties sAMAccountName";
    $f[]="            foreach (\$obj in \$conflicts) {";
    $f[]="                if (\$obj.sAMAccountName -ne (\$AccountForSetspn -replace '^[^\\\\]+\\\\','')) {";
    $f[]="                    Write-Host \"  Removing SPN from \$(\$obj.distinguishedName) via Set-ADObject\"";
    $f[]="                    Set-ADObject -Identity \$obj.distinguishedName -Remove @{servicePrincipalName=\$spn}";
    $f[]="                    \$fixed = \$true";
    $f[]="                }";
    $f[]="            }";
    $f[]="        } catch { Write-Warning \"LDAP removal failed: \$_\" }";
    $f[]="        if (\$fixed) {";
    $f[]="            Write-Host \"Retrying: setspn -S \$spn \$AccountForSetspn\"";
    $f[]="            \$addOut = & \$SetspnPath -S \$spn \$AccountForSetspn 2>&1";
    $f[]="            if (\$LASTEXITCODE -ne 0) { throw \"Failed to add SPN to '\$AccountForSetspn' after LDAP cleanup. Output:`n\$addOut\" }";
    $f[]="        } else {";
    $f[]="            throw \"Failed to add SPN to '\$AccountForSetspn'. Could not resolve duplicate. Output:`n\$addOut\"";
    $f[]="        }";
    $f[]="    }";
