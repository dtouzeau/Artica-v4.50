...
class MYSQLENGINE:
    ...
    def __init__(self,logging):
        ...
        self.mysql_database="squidlogs"
        self.OnlyMySQL=False
        self.ok=True
    ...
    def BuildSettings(self):
        self.ProxyUseArticaDB=GET_INFO_INT("ProxyUseArticaDB")
    ...
    def Connect(self):
        myProxyUseArticaDB=self.ProxyUseArticaDB
        if self.OnlyMySQL: myProxyUseArticaDB=0
        
        if myProxyUseArticaDB==1:
            self.mysql_database="squidlogs"
            self.ConnectionString="Unix socket on /var/run/mysqld/squid-db.sock"
            try:
                self.connection =  MySQLdb.connect(user="root",passwd="",db=self.mysql_database,unix_socket='/var/run/mysqld/squid-db.sock')
        ...
        if myProxyUseArticaDB==0:
            if self.mysql_server=='127.0.0.1':
                self.ConnectionString="Unix socket on /var/run/mysqld/mysqld.sock"
                try:
                    self.connection = MySQLdb.connect(user="root",passwd="",db=self.mysql_database,unix_socket='/var/run/mysqld/mysqld.sock')
                except MySQLdb.Error, e:
                    self.mysql_error="Unable to connect:"+self.ConnectionString +" "+str(e.args[0])+" "+ str(e.args[1])
                    self.logging.info("[MySQL]: Unable to connect :"+self.ConnectionString+" "+str(e.args[0])+" "+ str(e.args[1]))
                    return False
                ...
                try:
                    self.connection = MySQLdb.connect(host=self.mysql_server,port=self.mysql_port,user=self.mysql_admin,passwd=self.mysql_password,db=self.mysql_database)
                except MySQLdb.Error, e:
                    self.mysql_error="Unable to connect:"+self.ConnectionString +" "+str(e.args[0])+" "+ str(e.args[1])
                    self.logging.info("[MySQL]: Unable to connect :"+self.ConnectionString+" "+str(e.args[0])+" "+ str(e.args[1]))
                    return False
