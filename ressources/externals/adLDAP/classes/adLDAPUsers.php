...
class adLDAPUsers {
    ...
    public function __construct(adLDAP $adldap) {
        ...
        $this->FUNCTION=__FUNCTION__;
       	if (!$this->adldap->getLdapBind()) {
        		if($GLOBALS["VERBOSE"]){echo "$this->filename::$this->FUNCTION  getLdapBind return false, connect() line ".__LINE__."\n";}
        		$this->adldap->connect();
       	}else{
       		if($GLOBALS["VERBOSE"]){echo "$this->filename::$this->FUNCTION getLdapBind return TRUE\n";}
       	}
    }
    ...
    */
    public function groups($username, $recursive = NULL, $isGUID = false)
    {
    	...
    	if($GLOBALS["VERBOSE"]){echo "adLDAPUsers.php:".__FUNCTION__." Username: $username in line ".__LINE__."\n";}
    	
    	if ($username === NULL) { return false; }
        if ($recursive === NULL) { $recursive = $this->adldap->getRecursiveGroups(); } // Use the default option if they haven't set it

        if($GLOBALS["VERBOSE"]){echo "adLDAPUsers.php:: Recusrsive: $recursive\n";}
        if($GLOBALS["VERBOSE"]){echo "adLDAPUsers.php:: this->adldap->getLdapBind()\n";}
        if (!$this->adldap->getLdapBind()) { 
        	if($GLOBALS["VERBOSE"]){echo "adLDAPUsers.php:: getLdapBind() return false\n";}
        	return false; 
        }
    ...
    */
    ...
    {
        if ($username === NULL) { return false; }
        if (!$this->adldap->getLdapBind()) { $GLOBALS["CLASS_ACTV"][]=__FUNCTION__.": LINE:".__LINE__.":getLdapBind() return false"; return false; }
