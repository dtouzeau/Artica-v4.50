...
function js(){
	...
	$tpl=new template_admin();
	$tpl->js_dialog6("{system}::libcache-memcached-perl", "$page?popup=yes",650);
}

function popup(){
	$ARRAY["PROGRESS_FILE"]=PROGRESS_DIR."/php7install.progress";
	$ARRAY["LOG_FILE"]=PROGRESS_DIR."/php7install.log";
	$ARRAY["CMD"]="services.php?apt-get-install=libcache-memcached-perl";
	$ARRAY["TITLE"]="{system} {installing} libcache-memcached-perl";
