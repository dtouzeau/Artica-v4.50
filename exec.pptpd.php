...
if($argv[1]=="--chap"){chapsecrets();exit();}
if($argv[1]=="--clients"){clients_connexions();exit();}
if($argv[1]=="--client"){clients_connexions();exit();}
if($argv[1]=="--ifup"){client_net_config($argv[2],$argv[3],$argv[4],$argv[5]);exit();}
if($argv[1]=="--ifdown"){client_net_config_down($argv[2],$argv[3],$argv[4],$argv[5]);exit();}
...
function build(){
	...
	$unix=new unix();
	$PPTPDConfig=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("PPTPDConfig")));
...
function chapsecrets(){
	...
	$array=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("PPTPDMembers")));	
...
function ServerRoutesAdd($IFNAME,$ip_connect,$PPP_IPPARAM){
	...
	$sock=new sockets();
	$array=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("PPTPDConfigRoutes")));	
	if(count($array)==0){return;}
	while (list ($ip, $cdir) = each ($array) ){
		...
		if(preg_match("#$pattern#",$ip_connect)){
			...
			exec($cmd,$results);
			foreach ($results as $a=>$b){events("IF-UP:: $PPP_IPPARAM::$b",__FUNCTION__,__FILE__,__LINE__);}
		}
	}
