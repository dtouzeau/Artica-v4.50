...
function js(){
	...
	if(isset($_GET["employeeNumber"])){$uri="employeeNumber={$_GET["employeeNumber"]}";$title=$title."::{$_GET["employeeNumber"]}";}
	if(isset($_GET["uidUser"])){$uri="uidUser={$_GET["uidUser"]}";$title=$title."::{$_GET["uidUser"]}";}
...
function CONTACT_INDEX(){
	...
	foreach ($array as $num=>$ligne){
		...
		$html[]= $tpl->_ENGINE_parse_body("<li><a href=\"$page?newtab=$num&employeeNumber={$_GET["employeeNumber"]}&uidUser={$_GET["uidUser"]}\"><span style='font-size:14px'>$ligne</span></a></li>\n");
	}
...
function CONTACT_PAGE(){
	$tpl=new templates();
	$button=button("{apply}","EditContact('FFM_CONTACT_PAGE');RefreshTabsCT();",18);
	$complete_name=$tpl->_ENGINE_parse_body("{complete_name}");
	$complete_name=str_replace(" ", "&nbsp;", $complete_name);
	$button_complete=button($complete_name,"CompleteName();",12);
...
$instantMessaging="<hr><div style='font-size:14px;font-weight:bold;margin-bottom:5px'>{instant_messaging}</div>
	...
	$form="
		...
		<tr>
		<td valign='top'>
		<div style='font-size:16px;font-weight:bold;margin-bottom:5px;margin-top:15px'>{electronic_address}</div>
		<table style='width:99%;margin-left:10px' class=form>
			<tr>
		...
		<div style='font-size:14px;font-weight:bold;margin-bottom:5px;margin-top:15px'>{phone_title}</div>
		<table style='width:99%;margin-left:10px' class=form>
			<tr>
...
function CONTACT_SAVE(){
	...
	foreach ($_POST as $num=>$ligne){
		// Fixed in 2012 Feb 6 in fact no utf8_encode is needed.
