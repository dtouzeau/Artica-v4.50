...
function install($filekey=0,$OS){
	...
	if($OS<>$DebianVersion){
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]}, Debian version incompatible!\n";}
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetUlogdRepo")));
		$version=$ArticaTechNetRepo[$filekey][$OS]["VERSION"];
		$filename=$ArticaTechNetRepo[$filekey][$OS]["FILENAME"];
		$MD5=$ArticaTechNetRepo[$filekey][$OS]["MD5"];
		$URL=$ArticaTechNetRepo[$filekey][$OS]["URL"];
	}
	...
	if(!$curl->GetFile("$tmpdir/$filename")){
		...
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]}, msktutil unable to install....\n";}
	...
	if(!is_file("/usr/local/sbin/ulogd")){
		echo "/usr/local/sbin/ulogd no such file!\n";
		build_progress_idb("{installing} {failed}",110);
	}
	@chmod("/usr/local/sbin/ulogd",0755);
	
	$version=ulogd_version();
	if($version==null){
		echo "/usr/local/sbin/ulogd no such version!\n";
		build_progress_idb("{installing} {failed}",110);
	}
	$GLOBALS["CLASS_SOCKETS"]->SET_INFO("UlogdVersion", $version);
	
	if(is_file("/etc/init.d/ulogd")){
		system("/etc/init.d/ulogd restart");
	}
	system("$ldconfig");
	system("$depmod -a");
	system("$php /usr/share/artica-postfix/exec.status.php --process1 >/dev/null");
