<?php
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/nmap.single.progress";
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/nmap_single_progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{scan_this_computer}: {$_GET["MAC"]} - {$_GET["ipaddr"]}");
	echo "
	YahooWinBrowseHide();		
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file={$_GET["md5file"]}&MAC=".urlencode($_GET["MAC"])."&ipaddr=".urlencode($_GET["ipaddr"])."');
}
...
if($md5file<>$_GET["md5file"]){
	...
	var xStart$time= function (obj) {
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file&MAC=".urlencode($_GET["MAC"])."&ipaddr=".urlencode($_GET["ipaddr"])."');
	}		
...
if($prc==100){
	if(is_file(PROGRESS_DIR."/nmap_single_progress.results")){
		...
		$addjs="Loadjs('nmap.progress.result.php?MAC=".urlencode($_GET["MAC"])."&ipaddr=".urlencode($_GET["ipaddr"])."');";
	}
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
		RTMMailHide();
		if(document.getElementById('FICHE_COMPUTER_OPEN_PORTS')){ $('#FICHE_COMPUTER_OPEN_PORTS').flexReload(); }
		if(document.getElementById('host-tabs')){RefreshTab('host-tabs');}
...
function popup(){
	...
	$sock->getFrameWork("system.php?nmap-scan-single=yes&MAC=".urlencode($_GET["MAC"])."&ipaddr=".urlencode($_GET["ipaddr"])."");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{please_wait_preparing_settings}...");
	$title=$tpl->_ENGINE_parse_body("{scan_this_computer}: {$_GET["MAC"]} - {$_GET["ipaddr"]}");
	
$html="
<center id='title-$t' style='font-size:18px;margin-bottom:20px'>$title<br>$text</center>
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0&MAC=".urlencode($_GET["MAC"])."&ipaddr=".urlencode($_GET["ipaddr"])."');
}
$('#progress-$t').progressbar({ value: 1 });
