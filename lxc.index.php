...
function vps_edit_params(){
		...
		while (list ($iet, $iet2) = each ($interfaces) ){
			$sql="SELECT PhysNic FROM lxc_machines WHERE ID!={$_GET["ID"]} AND PhysNic='$iet'";
				...
				if($iet=="br5"){continue;}
				$lignePhysNic=mysqli_fetch_array($q->QUERY_SQL($sql,"artica_backup"));
...
<tr>
 ...
 if($LXCBridged==0){
	...
	while($ligne3=mysqli_fetch_array($results,MYSQLI_ASSOC)){
...
$BrowseUser="<input type='button' OnClick=\"Loadjs('MembersBrowse.php?field-user=lxc-member&OnlyUsers=yes');\" value='{browse}...'>";
...
while($ligne=mysqli_fetch_array($results,MYSQLI_ASSOC)){
		...
		$color="black";
		$isartica=null;
		$link="<a href=\"javascript:blur();\" OnClick=\"Loadjs('$page?form-vps-js=yes&ID={$ligne["ID"]}')\" style='font-size:14px;text-decoration:underline'>";
