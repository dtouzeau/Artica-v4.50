...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetMskUtilRepo")));
	...
	system("$php /usr/share/artica-postfix/exec.status.php --process1 --force >/dev/null");
	system("$php /etc/init.d/artica-status restart --force");
