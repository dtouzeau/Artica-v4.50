...
$GLOBALS["TITLENAME"]="AD Agent watchdog daemon";
...
function start($aspid=false){
    ...
    $adagent_tail=$unix->find_program("adagent-tail");
    if(!is_file($adagent_tail)){
        @copy($tail, "/usr/sbin/adagent-tail");
        @chmod("/usr/sbin/adagent-tail",0755);
        $adagent_tail="/usr/sbin/adagent-tail";
    }

    $pid=$unix->PIDOF($adagent_tail);

    if($unix->process_exists($pid)){
        if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]} killing $adagent_tail pid $pid\n";}
        $unix->KILL_PROCESS($pid);
    }

    $prefix="$adagent_tail -f --max-unchanged-stats=2 -n 0 /var/log/adagent.log";
    $TAIL_STARTUP="$php5 /usr/share/artica-postfix/exec.ad-agent-tail.php";
    if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]} tail `$tail`\n";}


    if(!is_file("/var/log/adagent.log")){
        if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]} /var/log/adagent.log no such file\n";}
        return;
    }
...
function PID_NUM(){
    ...
    $php5=$unix->LOCATE_PHP5_BIN();
    $TAIL_STARTUP="$php5 /usr/share/artica-postfix/exec.ad-agent-tail.php";
    $pid=$unix->get_pid_from_file("/var/run/adagent-tail.pid");
