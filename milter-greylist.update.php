...
function page(){
	$sock=new sockets();
	$ArticaTechNetInfluxRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetMGreyListRepo")));
	$tpl=new templates();
	$version=$sock->GET_INFO("MilterGreyListVersion");
	if(trim($version)==null){
		$sock->getFrameWork("milter-greylist.php?version=yes");
		$version=$sock->GET_INFO("MilterGreyListVersion");
	}
	if(trim($version)==null){$version="0.0.0";}
	
	$manual_update=$tpl->javascript_parse_text("{manual_update}");
	

	$html="
	<input type=hidden id='ThisIsTheMilterGreyListUpdate' value='1'>		
	...
	while (list ($key, $array) = each ($ArticaTechNetInfluxRepo) ){
		...
		$button=button("{update2}","Loadjs('milter-greylist.update.progress.php?key=$key&OS=$OS&filename=$FILENAME');",32);
