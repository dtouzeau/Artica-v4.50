...
function events_popup():bool{
    ...
    $TINY_ARRAY["TITLE"]="{SQUID_AD_RESTFULL} {events} v$ARTICAREST_VERSION";
...
function events_searcher():bool{
    ...
    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("squid.ad.restfull.php?events=$line");
...
";
    ...
    foreach ($f as $line){
       ...
       if(strpos("    $message","postgresqld.go")>0){
        ...
        if(strpos("    $message","phpfpm.go[phpfpm.MonitConfig:")>0){
            $message=str_replace("phpfpm.go[phpfpm.MonitConfig:","[",$message);
            $message="<span class='label label-success'>Monit Service</span>&nbsp;$message";
        }
