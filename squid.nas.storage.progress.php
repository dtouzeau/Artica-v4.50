...
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/squid.nas.storage.progress";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.nas.storage.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{APP_SQUID}::{use_remote_nas}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?nas-storage-progress=yes";
