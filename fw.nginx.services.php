...
function action_popup():bool{
    ...
    if($DisableBuildNginxConfig==0) {
        ...
        $html[] = "<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"$service_reconfigure_all\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
        $html[] = "</tr>";
    }
    ...
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('$page?extract-domains-js=yes&function=$function');\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";

    if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
    $html[]="<tr class='$TRCLASS'>";
    $html[]="<td style='width:99%;padding:10px'><H2>{import_a_rule}</H2><p>{import_a_rule_nginx_explain}</p></td>";
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('fw.nginx.export.php?ID=0&function=$function');\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";
    ...
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"document.location.href='$page?download-db=yes');\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";

    if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
    $html[]="<tr class='$TRCLASS'>";
    $html[]="<td style='width:99%;padding:10px'><H2>{import_rules}</H2><p>{import_rules_nginx_explain}</p></td>";
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('fw.nginx.importdb.php?function=$function');\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";

    if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
    $html[]="<tr class='$TRCLASS'>";
    $html[]="<td style='width:99%;padding:10px'><H2>{disable_all}</H2><p>{disable_all_nginx}</p></td>";
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('$page?disable-all-js=yes&function=$function')\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";

    if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
    $html[]="<tr class='$TRCLASS'>";
    $html[]="<td style='width:99%;padding:10px'><H2>{disable_all_web_firewall}</H2><p>{disable_all_web_firewall_explain}</p></td>";
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('$page?disable-fw-js=yes&function=$function')\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";

    if($TRCLASS=="footable-odd"){$TRCLASS=null;}else{$TRCLASS="footable-odd";}
    $html[]="<tr class='$TRCLASS'>";
    $html[]="<td style='width:99%;padding:10px'><H2>{enable_all_web_firewall}</H2><p>{enable_all_web_firewall_explain}</p></td>";
    $html[]="<td nowrap style='width:1%;padding:10px'><label class=\"btn btn btn-primary\" OnClick=\"Loadjs('$page?enable-fw-js=yes&function=$function')\"><i class='fas fa-arrow-alt-right'></i> {run_task} </label>";
    $html[]="</tr>";
...
function www_parameters2_waf($tpl,$ID){
    ...
    if ($NginxHTTPModSecurity == 0) {
        ...
        $tpl->table_form_field_text("{WAF_LONG}", "{not_installed}", ico_shield_disabled);
        return $tpl;
    }
    $EnableModSecurityIngix=intval($GLOBALS["CLASS_SOCKETS"]->GET_INFO("EnableModSecurityIngix"));
    if ($EnableModSecurityIngix == 0) {
        $tpl->table_form_field_js("");
        $tpl->table_form_field_text("{WAF_LONG}", "{disabled}", ico_shield_disabled);
        return $tpl;
    }
...
function www_parameters2():bool{
    ...
    if(!isset($noSSL[$type])) {
        ...
        if (strlen($ssl_certificate) < 4) {
            ...
            $tpl->table_form_field_js("Loadjs('fw.nginx.sites.ServerCertificate.php?client-certificate-js=$ID&function=$function')");
...
function table():bool{
    ...
    foreach ($results as $index=>$ligne){
        ...
        $RCOlor2="<span style='$color' id='rcolor2-$ID'></span>$peity_div";

        $html[]="<tr class='$TRCLASS' id='$md'>";
        $html[]="<td style='width:1%'><span id='status-$ID'><span></span></td>";
        $html[]="<td style='width:1%' nowrap><span style='$color' id='rcolor1-$ID'></span></td>";
        ...
        $html[]="<td style='width:1%' nowrap><span style='$color' id='rcolor7-$ID'></span></td>";
...
function HamrpStatus($ligne):string{
    $ID=$ligne["ID"];
    if($ligne["HamrpSaved"]==0){
        return "<span class='label label-danger'>#$ID {not_saved}</span>";
    }

    return "<span class='label label-primary'>#$ID OK</span>";
}
function td_status($ligne,$sockngix):string{
    ...
    if (isHarmpID()) {
        VERBOSE("isHarmpID === TRUE", __LINE__);
        $goodconf_js = "";
    }
    ...
    if (!isHarmpID()) {
        ...
        if ($data->Status) {
            VERBOSE("SitesID:$data->SitesID", __LINE__);
            $tb = explode(",", $data->SitesID);
            foreach ($tb as $sitesid) {
                VERBOSE("MAIN_REVERSED[$sitesid]", __LINE__);
                $MAIN_REVERSED[$sitesid] = true;
            }
        } else {
            VERBOSE("SitesID:$data->Error", __LINE__);
        }
    } else {
        $HamrpSaved = $ligne["HamrpSaved"];
        if ($HamrpSaved == 1) {
            $MAIN_REVERSED[$ID] = true;
    ...
    if(!is_null($ligne["goodconf"])) {
        $goodconflengh = strlen($ligne["goodconf"]);
    }
    ...
    VERBOSE("Goodconf: [-{$ligne["goodconf"]}-] goodconflengh($ID): $goodconflengh Type: {$ligne["type"]} Time:$goodconftime", __LINE__);
    $goodconf_js = "Loadjs('$page?goodconf=$ID')";
    ...
    if ($goodconflengh > 10) {
        ...
        if (strlen($goodconf_js) > 5) {
            ...
            VERBOSE("Compile Time [$CompileTime], Expected=$goodconftime", __LINE__);
            if($CompileTime>0){
                if($CompileTime<>$goodconftime){
                    VERBOSE("Compile Time [ISSUE]", __LINE__);
                    $ilya=distanceOfTimeInWords($CompileTime,time());
                    $f[]=$tpl->td_href("<span class='label label-primary'>#$ID $ilya</span>",
                        null, $goodconf_js);
    ...
    VERBOSE("Type: {$ligne["type"]}", __LINE__);

    if (isHarmpID()) {
        VERBOSE("isHarmpID:: TRUE", __LINE__);
        return HamrpStatus($ligne);
    } else {
        VERBOSE("isHarmpID:: FALSE", __LINE__);
    }
    ...
    if ($goodconftime == 0) {
        VERBOSE("MAIN_REVERSED[$ID] === NONE not_saved !!!", __LINE__);
        return $tpl->td_href("<span class='label label-warning'>#$ID {not_saved}</span>", null, $goodconf_js);
    }

    return "<span class='label'>#$ID {not_configured}</span>";
}
