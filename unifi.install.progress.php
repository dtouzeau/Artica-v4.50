...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/unifi.install.progress";
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/unifi.install.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{INSTALL_UNIFI_CONTROLLER}");
	if(isset($_GET["migration"])){$addon="&migration=yes";}
	echo "
	YahooWinBrowseHide();		
	RTMMail('800','$page?popup=yes{$addon}','$title');
	";
...
function Start$time(){
		...
		Loadjs('$page?build-js=yes&t=$t&md5file=$md5file{$addon}');
}
...
function Start$time(){
	...
	$('#progress-$t').progressbar({ value: $prc });
	RTMMailHide();
	LoadAjaxSilent('sequence-server','admin.dashboard.proxy.php?sequence-server=yes&nofw=yes&sequence=yes');
	CacheOff();
	LoadAjaxRound('unifi-controller','unifi.php');
...
function popup(){
	...
	$addon=null;
	if(isset($_GET["migration"])){$addon="&migration=yes";}
	$sock->getFrameWork("unifi.php?install=yes$addon");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{INSTALL_UNIFI_CONTROLLER}...");
...
function Step1$t(){
	...
	Loadjs('$page?build-js=yes&t=$t&md5file=0{$addon}');
}
$('#progress-$t').progressbar({ value: 1 });
