...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetUlogdRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		LayersTabsAllAfter();
		RTMMailHide();
		LoadAjaxRound('ulogd-updates','ulogd.update.engine.php');
	}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function popup(){
	...
	$sock->getFrameWork("ulogd.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
