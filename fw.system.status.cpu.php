<?php
	...
	if(isset($_GET["DisplayGraphs"])){stats();exit;}
js();
...
function js(){
	...
	$tpl->js_dialog1("{cpu_use}:{statistics}", "$page?DisplayGraphs=yes");
...
function stats():bool{
	...
	$f[]="processes-day.png";
	$f[]="cpu-week.png";	
	$f[]="processes-week.png";
	$f[]="cpu-month.png";
	$f[]="processes-month.png";
	$f[]="cpu-year.png";
	$f[]="processes-year.png";
    $z=array();
	foreach ($f as $image){
		if(is_file("$path/$image")){
            $z[]= "<div class='center' style='margin-top:10px;padding:5px;background-color:#F0F0F0;border:1px solid #CCCCCC;border-radius: 5px 5px 5px 5px;'><img src='munin-images/$image?$t'></div>";
		}
