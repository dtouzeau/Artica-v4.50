...
if (!isset($argv[1]) || !isset($argv[2]) ) {
	print "Usage php csv2contacts.php uid name.csv\n";
	exit (1);
}
...
echo "Establishing a session $username@".SERVER." $password\n";
...
while (list ($index, $sline) = each ($f) ){
	...
	if ($i==1 && defined('FIELD_NAMES') && FIELD_NAMES) {
		if($GLOBALS["VERBOSE"]){echo serialize($line).", is a FIELD_NAMES...\n";}
	    $i++;
	    continue;
	}
	...
	if (isset($props[$properties["display_name"]])){
		...
		echo "props{$properties["display_name"]} invalid\n";
	}
