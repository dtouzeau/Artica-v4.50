...
function page(){
	...
	$tpl=new template_admin();
	$t=time();
	if(!isset($_SESSION["UFDBCAT_DAEMON_SEARCH"])){$_SESSION["UFDBCAT_DAEMON_SEARCH"]="50 events";}
	if(isset($_GET["logfile"])){$addPLUS="&logfile=".urlencode($_GET["logfile"]);}
	
	$html="
	<div class=\"row border-bottom white-bg dashboard-header\">
		<div class=\"col-sm-8\"><h1 class=ng-binding>{APP_UFDBCAT} {events}</h1>
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["UFDBCAT_DAEMON_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"javascript:Search$t(event);\">
...
function search(){
	...
	$sock->getFrameWork("ufdbguard.php?ufdbcat-service-events=yes&search=$search&rp={$MAIN["MAX"]}");
	$filename=PROGRESS_DIR."/ufdbcat.log";
...
";
	...
	if(count($data)>3){$_SESSION["UFDBCAT_DAEMON_SEARCH"]=$_GET["search"];}
	...
	if(strpos($line, "}")>0){continue;}
	if(strpos($line, "!")>0){continue;}
	if(strpos($line, "license status")>0){continue;}
	if(strpos($line, 'redirect "')>0){continue;}
	if(strpos($line, 'expressionlist "')>0){continue;}
	if(preg_match("#(redirect|expressionlist|domainlist).*?\/#", $line)){continue;}
