...
function js(){
    ...
    $tpl->js_dialog2("{$ligne["hostname"]}: {templates}", "$page?start-table=$ID",650);
}
function start_table(){
    $page = CurrentPageName();
    $ID=intval($_GET["start-table"]);
    
    echo "<div id='wordpress-templates-progress-$ID'></div>
          <div id='wordpress-templates-$ID'></div>
          <script>LoadAjax('wordpress-templates-$ID','$page?table=$ID')</script>";
}
function enable_js(){
    $page = CurrentPageName();
    ...
    $GLOBALS["CLASS_SOCKETS"]->getFrameWork("wordpress.php?templates-list=$ID");
    ...
    foreach ($json as $index=>$main) {
        ...
        if($name==$plugin_name){
            if($status==0){
                echo "document.getElementById('$idimg').innerHTML='<img src=\"img/wait.gif\">';\n";
                $GLOBALS["CLASS_SOCKETS"]->getFrameWork("wordpress.php?templates-enable=$ID&template=$name");
                admin_tracks("Wordpress: $hostname activate template $name");

                echo "LoadAjax('wordpress-templates-$ID','$page?table=$ID')\n";
                return true;
            }
            return true;
        }
...
function delete_js(){
    ...
    $ligne=$q->mysqli_fetch_array("SELECT hostname FROM wp_sites WHERE ID=$ID");
    $hostname=$ligne["hostname"];
    $theme=urlencode($_GET["delete-js"]);
    $tpl = new template_admin();

    $service_reconfigure=$tpl->framework_buildjs("wordpress.php?templates-uninstall=$ID&template=$theme",
        "wordpress.single-install.$ID",
        "wordpress.single-install.$ID.log",
        "wordpress-templates-progress-$ID","LoadAjaxSilent('wordpress-templates-$ID','$page?table=$ID')");

    $tpl->js_confirm_delete($theme,"delete","$hostname template $theme",$service_reconfigure);

}
function delete_perform(){
    $tpl = new template_admin();
    $tpl->CLEAN_POST();
    admin_tracks("Wordpress: uninstall template {$_POST["delete"]}");
}
...
function file_uploaded(){
    ...
    $service_reconfigure=$tpl->framework_buildjs("wordpress.php?templates-install=$siteid&filename=$filename",
        "wordpress.single-install.$siteid",
        "wordpress.single-install.$siteid.log",
        "wordpress-templates-progress-$siteid","LoadAjaxSilent('wordpress-templates-$siteid','$page?table=$siteid')");
...
function table(){
    ...
    $html[] = "<tr>";
    $html[] = "<th></th>";
    $html[] = "<th data-sortable=true class='text-capitalize' nowrap>{template}</th>";
    ...
    foreach ($json as $index=>$main){
        ...
        if($status==0) {
            $enable=$tpl->button_inline("{disabled}","Loadjs('$page?enable-js=$name&siteid=$ID&md=$md')",
                "fa-solid fa-circle",null,0,"btn-default");
        }else{
            $enable=$tpl->button_inline("{enabled}","blur()","fa-solid fa-circle-check");
            $delete="&nbsp;";
        }
        ...
        $html[]="<td style='width:1%' nowrap class='center'><i class='fa-solid fa-pen-swirl'></i></td>";
        $html[]="<td style='width:99%' nowrap><span class='font-bold'><span id='$md-img'></span>$name</span></td>";
