...
function js(){
	...
	$tpl->js_dialog6("{import_set_certificates}", "$page?popup=yes&certid=$certid&function=$function","700");
...
function popup():bool{
    ...
    $function=$_GET["function"];
	$bt_upload=$tpl->button_upload("{import_certifcate_file}",$page,null,"&certid=$certid&function=$function")."&nbsp;&nbsp;";
	$explain=$tpl->div_explain("{import_set_certificates_explain}<p><i>{certificat_import_text}</i></p>");
...
function file_uploaded_zip($file){
    $tpl                    = new template_admin();
    $zip                    = new ZipArchive();
    ...
    if (!$zip->open($fullpath)) {
        $tpl->js_error("Corrupted container!");
        @unlink($fullpath);
        return false;
    }
    for( $i = 0; $i < $zip->numFiles; $i++ ){
        ...
        if(!$fp){
            $tpl->js_error("Error: can`t get stream to zipped file $filename");
            @unlink($fullpath);
            return false;
        }
        $buf = "";
        writelogs("[$filename]: OPEN ...",__FUNCTION__,__FILE__,__LINE__);
        ob_start();
        ...
        if(stripos($s, "CRC error") != FALSE){
            $tpl->js_error("Error: can't get stream to zipped file $filename");
            $current_crc=sprintf("%08X", crc32($buf));
            $expected_crc=sprintf("%08X", $stat['crc']);
            $tpl->js_error("CRC32 mismatch $current_crc expected $expected_crc");
            @unlink($fullpath);
        }
        writelogs("[$filename]: ".strlen($buf)." bytes",__FUNCTION__,__FILE__,__LINE__);
        if(strlen($buf)==0){continue;}

        $main=explode("\n",$buf);
        foreach ($main as $line){

            if(preg_match("#-----BEGIN PRIVATE KEY-----#",$line)){
                writelogs("[$filename]: This file is the private key",__FUNCTION__,__FILE__,__LINE__);
                $private_key_content=$private_key_content.$buf;
                // This file is the private key.
                break;
            }
            if(preg_match("#-----BEGIN RSA PRIVATE KEY-----#",$line)){
                writelogs("[$filename]: This file is the RSA private key",__FUNCTION__,__FILE__,__LINE__);
                $private_key_content=$private_key_content.$buf;
                // This file is the private RSA key.
                break;
            }


            if(preg_match("#-----BEGIN PUBLIC KEY-----#",$line)){
                if(strlen($private_key_content)>128){
                    $private_key_content=$private_key_content.$buf;
                }else{
                    $private_key_content=$private_key_content.$buf;
                }
                writelogs("[$filename]: This file is the PUBLIC key",__FUNCTION__,__FILE__,__LINE__);
                // This file is the public key.
                break;
            }

            if(preg_match("#-----BEGIN CERTIFICATE REQUEST-----#",$line)){
                $csr_content=$buf;
                writelogs("[$filename]: This file is the Certificate request",
                    __FUNCTION__,__FILE__,__LINE__);
                // This file is the Certificate request.
                break;
            }

            if(preg_match("#---BEGIN CERTIFICATE---#",$line)){
                writelogs("[$filename]: This file is a Certificate [GOOOD]",
                    __FUNCTION__,__FILE__,__LINE__);
                // This file is the certificate.
                if(strlen($certificate_content)>256){
                    $certificate_content=$certificate_content.$certificate_content;
                }else {
                    $certificate_content = $buf;
                }
                break;
            }
        }
    ...
    if(strlen($private_key_content)==0){
        writelogs("[ALERT]: PRIVATE KEY WAS NOT FOUND IN THIS package",
            __FUNCTION__,__FILE__,__LINE__);
    }
    ...
    if($certificate_content==null){
        $tpl->js_error("Error: can`t found certificate content");
        return false;
    }
   // if(!validateSslOptions($certificate_content,$private_key_content)){
     //   return false;
    //}


    $array=openssl_x509_parse($certificate_content);

    if(IfCertisCA($array)){
        return ImportCA($certificate_content,$private_key_content,$csr_content);
    }

    return ImportSubCertificates($certificate_content,$private_key_content,$csr_content);


}

function rsa_decrypt($privateKey){
    $decrypted  = null;
    $encrypted  = null;
    $tpl        = new template_admin();
    ...
    if(!$keyData){
        $tpl->js_error("openssl_pkey_get_details FALSE");
        return false;}
...
function file_uploaded():bool{
    ...
    if(!preg_match("#\.zip$#",$file)){
        return $tpl->js_error("Error: $file not a zip file");
    }
    ...
    if($certid>0){
            $array["certid"]=$certid;
            $data=$sock->REST_API_POST("/certificate/zip/import",$array);
    ...
    $data=$sock->REST_API_POST("/certificate/zip/newimport",$array);
    $json=json_decode($data);
    if(!$json->Status){
