...
page();
function SquidDisableWhiteLists(){
...
function delete_js(){
    ...
    $ligne=$q->mysqli_fetch_array("SELECT pattern FROM acls_whitelist WHERE ID='{$_GET["delete-js"]}'");
    $md5=$_GET["md5"];
...
function Save(){
    ...
    if($ID==0){
        ...
        $q->QUERY_SQL("INSERT OR IGNORE INTO acls_whitelist (zDate,ztype,pattern,enabled,description) VALUES ".@implode(",", $f));
    ...
    if( $ztype=="arp"){
        ...
        if(!$ip->IsvalidMAC($pattern)){return;}
    }
    ...
    $q->QUERY_SQL("UPDATE acls_whitelist SET pattern='$pattern',enabled='$enabled',description='$description' WHERE ID=$ID");
    if(!$q->ok){echo "jserror:".$tpl->javascript_parse_text($q->mysql_error);return;}
...
function config_popup(){
    ...
    $data[]="# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #";
    $data[]=@file_get_contents("/etc/squid3/acls_whitelist.dstdomain.conf");
    $data[]="";
    $data[]="# White-list based on domains using regular expressions ( REGEX )";
    $data[]="# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #";
    $data[]=@file_get_contents("/etc/squid3/acls_whitelist.dstdom_regex.conf");
    $data[]="";
    $data[]="# White-list based destination networks";
    $data[]="# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #";
    $data[]=@file_get_contents("/etc/squid3/acls_whitelist.dst.conf");
    $data[]="";
    $data[]="# White-list based source networks";
    $data[]="# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #";
    $data[]=@file_get_contents("/etc/squid3/acls_whitelist.src.conf");
    $data[]="";
...
function page():bool{
    ...
    $html=$tpl->page_header("{global_whitelists}","fa fa-thumbs-up",
        "{global_whitelists_proxy_explain}<div id='btns-white'></div>","$page?search-form=yes",
        "proxy-whitelists","progress-firehol-restart",false,"table-loader-global-whitelist");
...
function switch_perform(){
    ...
    $tpl=new template_admin();
    $ID=$_GET["switch-js"];
    $ligne=$q->mysqli_fetch_array("SELECT enabled FROM acls_whitelist WHERE ID='$ID'");
    if($ligne["enabled"]==1){
        $q->QUERY_SQL("UPDATE acls_whitelist SET enabled=0 WHERE ID=$ID");

    }else{
        $q->QUERY_SQL("UPDATE acls_whitelist SET enabled=1 WHERE ID=$ID");
    }
...
function config_popup_js(){
    $page=CurrentPageName();
    $tpl=new template_admin();
    $tpl->js_dialog1("{config}", "$page?config-popup=yes");
...
function popup_js(){
    ...
    if($ID>0){
        $q=new lib_sqlite("/home/artica/SQLITE/acls.db");
        $ligne=$q->mysqli_fetch_array("SELECT ztype,pattern FROM acls_whitelist WHERE ID='$ID'");
        $ztype=$GLOBALS["GroupType"][$ligne["ztype"]];
        $pattern=$ligne["pattern"];
        $title="$ztype: $pattern";
    }

    $tpl->js_dialog1($title, "$page?popup=yes&ID=$ID");
}
function popup(){
    $page=CurrentPageName();
    $tpl=new template_admin();
    ...
    if($ID>0){
        $buttonname="{apply}";
        $q=new lib_sqlite("/home/artica/SQLITE/acls.db");
        $ligne=$q->mysqli_fetch_array("SELECT * FROM acls_whitelist WHERE ID='$ID'");
        $ztype=$GLOBALS["GroupType"][$ligne["ztype"]];
        $pattern=$ligne["pattern"];
        $title="$ztype: $pattern";
    }
    ...
    $form[]=$tpl->field_checkbox("enabled","{enabled}",$ligne["enabled"],false,null,false,"AsDansGuardianAdministrator");
    if($ID==0){
    ...
    $jsafter="LoadAjax('table-loader-global-whitelist','$page?table=yes');";
...
function restart_js():string{
    ...
    return $tpl->framework_buildjs("/proxy/whitelists/nohupcompile","squid.global.whitelists.progress","squid.global.whitelists.progress.log","progress-firehol-restart");
}
...
function rules_list_search(){
    ...
    $sql="SELECT * FROM acls_whitelist ORDER by zDate DESC";
    $search=$_GET["search"];
    if(strlen($search)>1) {
        $search = str_replace( "*", "%",$search);
        $sql = "SELECT * FROM acls_whitelist WHERE (pattern LIKE '$search' OR description LIKE '$search') ORDER by zDate DESC";
    }
    ...
    foreach ($results as $index=>$ligne) {
        ...
        $delete=$tpl->icon_delete("Loadjs('$page?delete-js={$ligne["ID"]}&md5={$ligne["md5"]}')","AsDansGuardianAdministrator");
        $html[]="<tr class='$TRCLASS' id='{$ligne["md5"]}'>";
        $html[]="<td style='width:1%$stylecolor' nowrap>{$ligne["zDate"]}</td>";
        $html[]="<td style='width:1%$stylecolor' nowrap><a href=\"javascript:blur();\" OnClick=\"$js\" style='font-weight:bold;$stylecolor' nowrap>{$ligne["ztype"]}</span></td>";
    ...
    $TINY_ARRAY["TITLE"]="{global_whitelists}";
    $TINY_ARRAY["ICO"]="fa fa-thumbs-up";
    $TINY_ARRAY["EXPL"]="{global_whitelists_proxy_explain}";
