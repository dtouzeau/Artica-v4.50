...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_ULOGD}:{disable_feature}");
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			LoadAjaxRound('ulogd-main','ulogd.main.php');
		}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function popup(){
	...
	$sock->getFrameWork("ulogd.php?disable=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_ULOGD}:{disable_feature}...");
