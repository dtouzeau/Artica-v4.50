...
function page(){
	...
	$t=time();
	if(!isset($_SESSION["RSYNCD_SEARCH"])){$_SESSION["RSYNCD_SEARCH"]="today this hour 50 events";}
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["RSYNCD_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"javascript:Search$t(event);\">
...
function search(){
	...
	$sock->getFrameWork("rsync.php?syslog=$line");
	$filename=PROGRESS_DIR."/rsyncd.syslog";
...
";
	...
	if(count($data)>3){$_SESSION["RSYNCD_SEARCH"]=$_GET["search"];}
	rsort($data);
	...
	foreach ($data as $line){
		...
		$html[]="<tr>
				<td style='width:1%;' nowrap class='$class'>$FTime</td>
	...
	$html[]="</tbody></table>";
	$html[]="<div><i>".@file_get_contents(PROGRESS_DIR."/rsyncd.syslog.pattern")."</i></div>";
	echo @implode("\n", $html);
