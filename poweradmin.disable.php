<?php
$GLOBALS["PROGRESS_FILE"]=PROGRESS_DIR."/pdns.first.install";
$GLOBALS["CACHEFILE"]=$GLOBALS["PROGRESS_FILE"];
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/pdns.first.install.log";
...
function title(){
	$tpl=new templates();
	return $tpl->javascript_parse_text("{disable_feature} PowerAdmin");
}
...
function js(){
	...
	$t=time();
	$tpl=new templates();
	$compile_squid_ask=$tpl->javascript_parse_text("{install_dns_service_ask}");
...
if($prc==100){
	...
	function Start$time(){
		...
		RTMMailHide();
		GotoPowerDNS();
		//LoadAjaxRound('powerdns-dashboard','powerdns.dashboard.php');
	}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function Launch(){
	$sock=new sockets();
	$cmd="pdns.php?poweradmin-disable=yes";
