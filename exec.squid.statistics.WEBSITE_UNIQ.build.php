...
function GRAB_DATAS($ligne,$md5){
	...
	$SQLA[]="SELECT SUM(size) as size, SUM(rqs) as rqs,$TimeGroup,userid,ipaddr,mac,proxyname FROM access_log";
	$SQLA[]="WHERE";
	$SQLA[]="$SEARCHTEXT (zdate >'".date("Y-m-d H:i:s",$from)."' and zdate < '".date("Y-m-d H:i:s",$to)."')";
	if($USER_FIELD=="USERID"){$SQLA[]="AND USERID != 'none'";}
	$SQLA[]="GROUP BY date_trunc('hour', zdate), userid,ipaddr,mac,proxyname";
		
	build_progress("{step} {waiting_data}: BigData engine, $searchsites {please_wait}",6);
	$unix=new unix();
	...
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (size,rqs,zdate,userid,ipaddr,mac,proxyname) $sql";
