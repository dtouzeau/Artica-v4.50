...
function table_start(){
    ...
    $html   = "<div id='sshportal-hostsgroups-table' style='margin-top: 10px'></div><script>LoadAjax('sshportal-hostsgroups-table','$page?table=yes');</script>";
    echo $html;
}
...
function group_js(){
    ...
    if($id>0){
        ...
        $ligne  =   $q->mysqli_fetch_array("SELECT name FROM host_groups WHERE id='$id'");
        $title  =   $ligne["name"];
...
function group_tabs(){
    ...
    $array["{hosts}"]="fw.sshportal.hosts.php?start=yes&groupid=$id";
...
function group_delete_real(){
    $q      =   new lib_sqlite("/home/artica/SQLITE/sshdportal.db");
    $q->QUERY_SQL("DELETE FROM host_groups WHERE id='{$_POST["id"]}'");
    if(!$q->ok){echo $q->mysql_error;return;}
    $q->QUERY_SQL("DELETE FROM host_host_groups WHERE host_group_id='{$_POST["id"]}'");
    $q->QUERY_SQL("DELETE FROM host_group_acls WHERE host_group_id='{$_POST["id"]}'");
...
function group_popup(){
    ...
    $title  = "{new_group}";
    $btname = "{add}";
    $jsafter= "dialogInstance1.close();LoadAjax('sshportal-hostsgroups-table','$page?table=yes')";

    if($id>0) {
        $ligne = $q->mysqli_fetch_array("SELECT * FROM host_groups WHERE id='$id'");
        $name       = $ligne["name"];
...
function group_save(){
    ...
    if($id==0){
        $q->QUERY_SQL("INSERT INTO host_groups (comment,name) VALUES ('$comment','$name')");
        if(!$q->ok){echo $q->mysql_error;}
        return true;
    }

    $q->QUERY_SQL("UPDATE host_groups SET `comment`='$comment',`name`='$name' WHERE id='$id'");
    if(!$q->ok){echo $q->mysql_error;}
...
function group_fill(){
    ...
    $name       = $ligne["name"];
    $comment    = $ligne["comment"];
    $results2   = $q->mysqli_fetch_array("SELECT count(*) as tcount FROM host_host_groups WHERE host_group_id=$id");
    $Users      = intval($results2["tcount"]);

    $comment=str_replace("'","\'",$comment);
    header("content-type: application/x-javascript");
    $f[]="if( document.getElementById('$id-sgname') ){";
    $f[]="document.getElementById('$id-sgname').innerHTML='$name';";
    $f[]="document.getElementById('$id-sgusername').innerHTML='$Users';";
    $f[]="document.getElementById('$id-sgcomment').innerHTML='$comment';";
    $f[]="}";
    echo @implode("\n",$f);
...
function table(){
    ...
    $html[]="<th data-sortable=false class='text-capitalize' data-type='text' nowrap>{servers}</th>";
...
//	print_r($hash_full);
    ...
    foreach ($results as $index=>$ligne){
        ...
        $html[]="<td class=\"\" width='1%' nowrap><i class='fas fa-users'></i>&nbsp;".$tpl->td_href("<span id='$ID-sgname'>$name</span>","{click_to_edit}",$js)."</td>";
        $html[]="<td class=\"\"><span id='$ID-sgcomment'>$comment</span></td>";
        $html[]="<td class=\"\" width='1%' nowrap><i class='fa fa-server'></i>&nbsp;<span id='$ID-sgusername'>$Users</span></td>";
