...
function rule_ports_save(){
	...
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$q->QUERY_SQL("UPDATE `redsocks` SET `transparentport`='$newdata' WHERE ID=$ID");
...
function rule_sources_save(){
	...
	$newdata=base64_encode(@implode("\n", $newf));
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$q->QUERY_SQL("UPDATE `redsocks` SET `transparentin`='$newdata' WHERE ID=$ID");
	if(!$q->ok){echo $q->mysql_error;}
...
function rule_sourcesex_save(){
	...
	$newdata=base64_encode(@implode("\n", $newf));
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$q->QUERY_SQL("UPDATE `redsocks` SET `excludetransparentin`='$newdata' WHERE ID=$ID");
	if(!$q->ok){echo $q->mysql_error;}	
...
function rule_dstex_save(){
	...
	$newdata=base64_encode(@implode("\n", $newf));
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$q->QUERY_SQL("UPDATE `redsocks` SET `excludetransparentout`='$newdata' WHERE ID=$ID");
	if(!$q->ok){echo $q->mysql_error;}
...
function rule_dst_save(){
	...
	$newdata=base64_encode(@implode("\n", $newf));
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$q->QUERY_SQL("UPDATE `redsocks` SET `transparentout`='$newdata' WHERE ID=$ID");
	if(!$q->ok){echo $q->mysql_error;}
...
function rule_dst(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT servicename,transparentout FROM `redsocks` WHERE ID=$ID");
	...
	"LoadAjax('redsocks-table-rules','fw.redsocks.php?rules-table=yes');","AsFirewallManager");
...
function rule_dstex(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT servicename,excludetransparentout FROM `redsocks` WHERE ID=$ID");
...
function rule_sources(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT servicename,transparentin FROM `redsocks` WHERE ID=$ID");
...
function rule_sourcesex(){
	...
	$q=new lib_sqlite("/home/artica/SQLITE/firewall.db");
	$ID=intval($_GET["rule-sources"]);
	$ligne=$q->mysqli_fetch_array("SELECT servicename,excludetransparentin FROM `redsocks` WHERE ID=$ID");
...
function rule_ports(){
	...
	$ligne=$q->mysqli_fetch_array("SELECT servicename,transparentport FROM `redsocks` WHERE ID=$ID");
