...
function page(){
	...
	$t=time();
	if(!isset($_SESSION["SQUIDTAIL_SEARCH"])){$_SESSION["SQUIDTAIL_SEARCH"]="today this hour 50 events";}
	...
	<div class=\"row\"> 
		...
		<div class=\"input-group\">
      		<input type=\"text\" class=\"form-control\" value=\"{$_SESSION["SQUIDTAIL_SEARCH"]}\" placeholder=\"{search}\" id='search-this-$t' OnKeyPress=\"javascript:Search$t(event);\">
...
function search(){
	...
	$sock->getFrameWork("squidlogger.php?syslog=$line");
	$filename=PROGRESS_DIR."/squidtail.syslog";
...
";
	...
	if(count($data)>3){$_SESSION["SQUIDTAIL_SEARCH"]=$_GET["search"];}
	rsort($data);
	...
	foreach ($data as $line){
		...
		$ACTION=null;
		$FF=false;
		if(!preg_match("#([0-9\-]+)\s+([0-9:]+),.*?-\s+([A-Z]+)\s+(.+)#", $line,$re)){
		...
		if(preg_match("#(error|exception)#i", $line)){
	...
	$html[]="</tbody></table>";
	$html[]="<div><i>".@file_get_contents(PROGRESS_DIR."/squidtail.syslog.pattern")."</i></div>";
