<?php
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/lsm.progress";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/lsm.log";
...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->javascript_parse_text("{disable_feature} {LinkStatusMonitor}");
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		LoadAjaxRound('lsm-div','lsm.index.php');
		RTMMailHide();
	}
	setTimeout(\"Start$time()\",1000);
	";	
