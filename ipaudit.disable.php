...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/ipaudit.progress";
$GLOBALS["LOG_FILE"]="/usr/share/artica-postfix/ressources/logs/ipaudit.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{APP_IPAUDIT} {disable_feature}");
	echo "RTMMail('800','$page?popup=yes&filename=".urlencode($_GET["filename"])."','$title');";
...
if($prc==100){
	...
	function Start$time(){
		...
		$('#progress-$t').progressbar({ value: $prc });
			RTMMailHide();
			LoadAjaxRound('system-ipaudit','ipaudit.index.php');
...
function popup(){
	...
	$sock->getFrameWork("ipaudit.php?uninstall=yes");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{APP_IPAUDIT} {disable_feature}...");
