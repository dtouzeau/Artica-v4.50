...
function title(){
	...
	$tpl=new templates();
	$sock=new sockets();
	return $tpl->_ENGINE_parse_body("{squid_plugins}");
}
...
if($prc==100){
	...
	function Start$time(){
		...
		document.getElementById('title-$t').innerHTML='$title';
		$('#progress-$t').progressbar({ value: $prc });
		if(document.getElementById('SQUID_ARTICA_QUOTA_RULES')){ $('#SQUID_ARTICA_QUOTA_RULES').flexReload(); }
		if(document.getElementById('TABLE_SQUID_PORTS')){ $('#TABLE_SQUID_PORTS').flexReload(); }
		RTMMailHide();
	}
	setTimeout(\"Start$time()\",1000);
	";	
...
function Launch(){
	$sock=new sockets();
	$cmd="squid2.php?global-plugins-center=yes";
