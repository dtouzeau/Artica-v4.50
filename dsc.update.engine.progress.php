...
$GLOBALS["LOGSFILES"]="/usr/share/artica-postfix/ressources/logs/dsc.install.progress.txt";
...
function js(){
	...
	if($_GET["key"]<>null){
		$sock=new sockets();
		$ArticaTechNetSquidRepo=unserialize(base64_decode($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetDSCRepo")));
...
if($prc==100){
	...
	function Start$time(){
		...
		LayersTabsAllAfter();
		RTMMailHide();
		LoadAjaxRound('dsc-update-engine','dsc.update.engine.php');
	}
	setTimeout(\"Start$time()\",1000);
	";	
	return;	
...
function popup(){
	...
	$sock->getFrameWork("dsc.php?install-tgz=".urlencode($_GET["filename"])."&key={$_GET["key"]}&OS={$_GET["OS"]}");
	$t=$_GET["t"];
