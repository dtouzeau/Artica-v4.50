...
$GLOBALS["CACHEFILE"]=PROGRESS_DIR."/squid.network.switch.progress";
$GLOBALS["LOGSFILES"]=PROGRESS_DIR."/squid.network.switch.progress.txt";
...
function title(){
	...
	$tpl=new templates();
	$title=$tpl->_ENGINE_parse_body("{network_switch}::{apply}");
	return $title;
}
...
function Launch(){
	$sock=new sockets();
	$cmd="squid.php?network-switch=yes";
