...
function enable_feature():bool{
    ...
    $f[]="LoadAjaxSilent('excludecache-nginx-$serviceid','$page?top-buttons=$serviceid');";
...
function service_js():bool{
    ...
    $page       = CurrentPageName();
    return $tpl->js_dialog4("{exclude_cache}","$page?popup-main=$serviceid");
}
function rule_js():bool{
    ...
    return $tpl->js_dialog5("{exclude_cache}: $title","$page?popup-rule=$rule&serviceid=$serviceid&function=$function");
}
...
function rule_remove():bool{
    ...
    return admin_tracks("Remove cache exclude rule $pattern from reverse-proxy site $site");
...
function rule_enable():bool{
    ...
    $get_servicename=get_servicename($serviceid);
    echo refresh_global_no_close($serviceid);
    return admin_tracks("Enable/disable For reverse-proxy $get_servicename cache exclude rule $pattern");
}
...
function rule_popup():bool{
    ...
    $bt="{add}";
    $q=new lib_sqlite(NginxGetDB());
    $ligne=$q->mysqli_fetch_array("SELECT * FROM bypass_cache WHERE ID=$ruleid");
    if($ruleid>0){
        $bt="{apply}";
    }
...
function popup_main():bool{
    $q=new lib_sqlite(NginxGetDB());
    $q->QUERY_SQL("CREATE TABLE IF NOT EXISTS bypass_cache (ID INTEGER PRIMARY KEY AUTOINCREMENT,serviceid INTEGER NOT NULL DEFAULT 0, enabled INTEGER NOT NULL DEFAULT 1,pattern TEXT NOT NULL,regex INTEGER NOT NULL DEFAULT 1 )");
...
function popup_table():bool{
    ...
    $tpl        = new template_admin();
    echo "<div id='excludecache-nginx-$serviceid' style='margin-bottom:10px;margin-top:10px'></div>";
...
function rule_disable_all():bool{
    ...
    echo refresh_global_no_close($serviceid);
    echo "$function();";
    return admin_tracks("Disable all cache exclude rules For reverse-proxy $get_servicename");
}
function rule_enable_all():bool{
    ...
    echo refresh_global_no_close($serviceid);
    echo "$function();";
    return admin_tracks("Enable all cache exclude rules For reverse-proxy $get_servicename");
}
function popup_table2():bool{
    ...
    $searchQ="";
    $tableid    = time();
    $html[]="<div id='progress-compile-excludecache-$serviceid'></div>";
    $html[]="</div>";
...
";
    ...
    $query="SELECT * FROM bypass_cache WHERE serviceid=$serviceid$searchQ$searchE";
    ...
    $html[]="<tr id='def'>
				...
				<td style='width:100%'><code style='font-size: 14px;font-weight: bold;'>\.pagespeed\.([a-z]\.)?[a-z]{2}\.[^.]{10}\.[^.]+</code>&nbsp;&nbsp;&nbsp;$regex_span</td>
				<td style='width:1%'  nowrap >&nbsp;</td>
				<td style='width:1%'  nowrap >&nbsp;</td>
				</tr>";
        ...
        $html[]="LoadAjax('excludecache-nginx-$serviceid','$page?top-buttons=$serviceid&function=$function');";
        $html[]="</script>";
        echo $tpl->_ENGINE_parse_body($html);
        return true;
        }