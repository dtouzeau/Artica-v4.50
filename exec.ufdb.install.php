...
function install($filekey=0,$OS){
	...
	if($filekey<>0){
		$sock=new sockets();
		$ArticaTechNetRepo=unserializeb64($GLOBALS["CLASS_SOCKETS"]->GET_INFO("ArticaTechNetUFDBRepo"));
	...
	if(!$curl->GetFile("$tmpdir/$filename")){
		...
		if($GLOBALS["OUTPUT"]){echo "Starting......: ".date("H:i:s")." [INIT]: {$GLOBALS["TITLENAME"]}, Ufdbguardd unable to install....\n";}
	...
	if($GLOBALS["PROGRESS"]){
		...
		system("$php /usr/share/artica-postfix/exec.status.php --ufdb >/dev/null");
		system("/etc/init.d/artica-status restart --force");
		build_progress_idb("{restarting_service}",98);
		
		if(is_file("/etc/init.d/ufdb")){system("/etc/init.d/ufdb restart");}
		if(is_file("/etc/init.d/ufdbcat")){system("/etc/init.d/ufdbcat restart");}
	}
