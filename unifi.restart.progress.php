...
$GLOBALS["PROGRESS_FILE"]="/usr/share/artica-postfix/ressources/logs/unifi.restart.progress";
$GLOBALS["LOG_FILE"]=PROGRESS_DIR."/unifi.restart.progress.txt";
...
function js(){
	...
	$title=$tpl->javascript_parse_text("{restarting_service}");
...
function popup(){
	...
	$addon=null;
	if(isset($_GET["migration"])){$addon="&migration=yes";}
	$sock->getFrameWork("unifi.php?restart=yes$addon");
	$t=$_GET["t"];
	if(!is_numeric($t)){$t=time();}
	$text=$tpl->_ENGINE_parse_body("{restarting_service}...");
