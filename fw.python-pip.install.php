...
function js(){
	...
	$tpl->js_dialog6("{system}::{APP_PYTHON_PIP} {install} {$_GET["package"]}", "$page?popup=yes&package={$_GET["package"]}$function",650);
}

function popup(){
	if(isset($_GET["function"])){$function=";{$_GET["function"]}()";}
	$ARRAY["PROGRESS_FILE"]=PROGRESS_DIR."/python.progress";
	$ARRAY["LOG_FILE"]=PROGRESS_DIR."/python.progress.log";
	$ARRAY["CMD"]="python.php?pip-install={$_GET["package"]}";
	$ARRAY["TITLE"]="{system} {APP_PYTHON_PIP} {$_GET["package"]}";
