...
function GRAB_DATAS($ligne,$md5){
	...
	$md5_table=md5(__FUNCTION__."."."$from$to$USER_FIELD$searchuser$searchsites");
	...
	$SQLA[]="SELECT SUM(size) as size, SUM(rqs) as rqs,$TimeGroup,$USER_FIELD,proxyname FROM access_log";
	$SQLA[]="WHERE";
	...
	$SQLA[]="GROUP BY date_trunc('hour', zdate), $USER_FIELD,proxyname";
	...
	$sql=@implode(" ", $SQLA);
	$sql="INSERT INTO \"{$md5}report\" (size,rqs,zdate,$USER_FIELD,proxyname) $sql";
