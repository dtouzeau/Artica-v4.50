<?php
	...
	if($_SESSION["uid"]==null){
		if(count($_GET)>0){echo "<script>window.location.href ='logoff.php';</script>";die("DIE " .__FILE__." Line: ".__LINE__);}
		echo "window.location.href ='logoff.php';";die("DIE " .__FILE__." Line: ".__LINE__);
	}
...
<tbody class='tbody'>";		
		...
		while($ligne=@mysqli_fetch_array($results,MYSQLI_ASSOC)){	
			...
			$html=$html."
			<tr class=$classtr>
			<td nowrap style='width:1%;font-size:13px'>$date</td>
			<td nowrap style='width:1%;font-size:14px'><strong>{$ligne["IPPARAM"]}</strong></td>
