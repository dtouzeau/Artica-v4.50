...
function js(){
	...
	$tpl=new template_admin();
	$tpl->js_dialog6("{system}::qemu-guest-agent", "$page?popup=yes",650);
}
...
function popup(){
	...
	$ARRAY["CMD"]="services.php?apt-get-install=qemu-guest-agent";
	$ARRAY["TITLE"]="{system} {installing} qemu-guest-agent";
